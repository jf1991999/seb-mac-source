        ...
        if ([url.host isEqualToString:@"join"] && !_openingSettings) {
            ...
            if (code.length > 0) {
                ...
                if (studentName.length > 0) {
                    configComponents.queryItems = @[[NSURLQueryItem queryItemWithName:@"name" value:studentName]];
                }
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    // Show the About/splash window only for real session launches (a .seb config or a
    // blinkered:// URL). A bare launch hands off to the web app (welcome / student page) and
    // provides its own UI there — a splash on that path is just a dead wait on top of the
    // welcome screen. The paired bare-launch update-check path re-shows its own brief splash.
    _alternateKeyPressed = [self alternateKeyCheck];
    if (_alternateKeyPressed == NO && (_openingSettings || _openedURL)) {
        [self.aboutWindowController showAboutWindowForSeconds:10];
        // First visual = black covers + splash, never the desktop (launch cleanup #2).
        // The early covers may already exist (a blinkered:// URL arrives before this
        // runs) — in that case the splash was just shown at the non-elevated level,
        // i.e. BEHIND them, so raise it here; otherwise blinkeredEarlyCoverScreens
        // raises it after creating the covers.
        [self blinkeredEarlyCoverScreens];
        if (self.capWindows.count > 0 && self.aboutWindow.isVisible) {
        ...
        if (deviceId.length > 0 && token.length > 0 && base.length > 0) {
            ...
            [[[NSURLSession sessionWithConfiguration:config]
              ...
              completionHandler:^(NSData *d, NSURLResponse *resp, NSError *err) {
                if (err) DDLogWarn(@"Blinkered: home quit notification failed: %@", err.localizedDescription);
                else DDLogInfo(@"Blinkered: home session server notified (status %ld)", (long)((NSHTTPURLResponse *)resp).statusCode);
...
// If the main browser content is a Blinkered home session (/home/…), tell the page to open its
// styled exit-code modal and return YES so the caller skips SEB's native quit dialog. The page's
// exit flow validates the code via the server and navigates to the quit URL on success, which
// quits SEB normally. Returns NO for non-home sessions (e.g. class sessions) so they're unaffected.
- (BOOL)blinkeredShowHomeExitModal {
    NSWindow *mainWin = self.browserController.mainBrowserWindow;
    if (!mainWin) return NO;
    id mainAbstract = [mainWin valueForKey:@"webView"];
    if (!mainAbstract || ![mainAbstract respondsToSelector:@selector(nativeWebView)]) return NO;
    id mwk = [mainAbstract performSelector:@selector(nativeWebView)];
    if (![mwk isKindOfClass:NSClassFromString(@"WKWebView")]) return NO;
    NSString *url = [[mwk valueForKey:@"URL"] absoluteString] ?: @"";
    if ([url rangeOfString:@"/home/"].location == NSNotFound) return NO;   // not a home session
    NSString *js = @"window.blinkeredRequestExit && window.blinkeredRequestExit()";
    @try { [mwk performSelector:@selector(evaluateJavaScript:completionHandler:) withObject:js withObject:nil]; }
    @catch (__unused NSException *e) { return NO; }
    DDLogInfo(@"Blinkered: home-session Quit — showing in-page exit-code modal instead of native dialog");
    return YES;
}
    ...
    if (_browserController && [[NSUserDefaults standardUserDefaults] secureBoolForKey:@"org_safeexambrowser_SEB_examSessionClearCookiesOnEnd"]) {
        ...
        [cookieStore getAllCookies:^(NSArray<NSHTTPCookie *> *cookies) {
            ...
            for (NSHTTPCookie *cookie in cookies) {
                ...
                NSLog(@"BLINKERED exitSEB: saving cookie name=%@ domain=%@", cookie.name, cookie.domain);
            }
            ...
            if (focusChild.length > 0) {
                ...
                NSLog(@"BLINKERED exitSEB: saved %lu cookies to child %@ (skipped global cookies.json)", (unsigned long)cookies.count, focusChild);
                [NSApp terminate:nil];
                return;
            }
            ...
            NSLog(@"BLINKERED exitSEB: wrote %lu cookies to %@", (unsigned long)cookies.count, cookieFile.path);
            [NSApp terminate:nil];
        }];
    }
}
...
- (void) applicationWillTerminateProceed
{
    ...
    if (!_blinkeredSkipTerminateReturn && self.browserController.blinkeredFocusChildId.length == 0 && !self.browserController.blinkeredWebReturn) {
        ...
        if (target && target.bundleURL) {
            DDLogInfo(@"Blinkered: session teardown — surfacing %@ (student dashboard tab) via LaunchServices", target.bundleIdentifier);
            NSWorkspaceOpenConfiguration *cfg = [NSWorkspaceOpenConfiguration configuration];
            cfg.activates = YES;
            [[NSWorkspace sharedWorkspace] openApplicationAtURL:target.bundleURL
                                                  configuration:cfg
                                              completionHandler:nil];
        } else {
            DDLogInfo(@"Blinkered: session teardown — no running browser, opening the student dashboard fresh");
            [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:[self blinkeredStudentDashboardURLString]]];
        }
    }
