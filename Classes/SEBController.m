...
#import <stdatomic.h>   // M-C signing self-check: _Atomic live-population counters
...
#include <errno.h>
#include <libproc.h>
#import <os/log.h>   // BlinkeredThrashLog — a sink that survives Release; see its definition
#include <assert.h>
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    // A lock that died without unwinding — crash, force quit, power loss — leaves Blinkered's tint on
    // the child's desktop. This is the breadcrumb being cashed in: the flag and the original wallpaper
    // are PERSISTED precisely so the next launch can put it back even though the process that changed
    // it is gone. Runs unconditionally and early, before any lock can start: the wallpaper must be the
    // child's own again whether or not this launch turns into a session. No-op when nothing was left.
    [self blinkeredRestoreDesktopWallpaper];
...
#pragma mark - Desktop tint (the menu-bar strip over the aquarium)

// WHAT THIS IS. During a block-all the page fills the screen with the aquarium, but the top ~34pt
// belong to macOS. That strip is drawn by the OS from the DESKTOP PICTURE — measured with positive
// controls in docs/MENU_BAR_BACKDROP_PLAN.md: a window placed BEHIND the bar moved its colour by 0.4
// out of a possible 765, the same window raised ABOVE it by 139.6. Nothing this app draws can reach
// it. The only lever is the wallpaper, so that is what this does: while the water is up, the desktop
// becomes the water's own colours, and the seam stops registering.
//
// WHY A PROFILE AND NOT A COLOUR. The page's top row is not flat. Measured across the real renderer
// it runs from about rgb(18,99,121) at the screen edges to rgb(49,150,174) mid-screen under the sun
// glow. A single fill can only match the average and leaves the corners visibly light, so the page
// measures its own top row and sends the shape; this draws it as a horizontal gradient.
//
// THE SAFETY RULE, and it is the whole design: this changes a setting on a child's Mac, so it applies
// ONLY where it can prove it can put the setting back. A wallpaper that is a plain image file can be
// restored by URL. A shuffling album or a dynamic wallpaper CANNOT — `desktopImageURL` returns a
// container directory for those, and a naive save/restore would silently pin a rotating wallpaper to
// a single still, permanently. Where the check fails the feature simply does not run and the strip
// stays as it is today. Two probe findings sit behind that rule; both are in the plan doc.

static BOOL _blinkeredDesktopTintApplied = NO;   // this process's view; the persisted flag is the truth

// The display that owns the menu bar. Only that strip is in question, so only that desktop changes.
- (NSScreen *)blinkeredDesktopTintScreen
{
    return NSScreen.screens.firstObject;
}

// The reversibility gate. Returns the wallpaper's URL only when putting it back is a URL away.
- (NSURL *)blinkeredRestorableWallpaperURLForScreen:(NSScreen *)screen
{
    ...
    // A DIRECTORY is the tell for a shuffling album or a dynamic set — the case we must not touch.
    if (isDirectory) return nil;
    NSArray<NSString *> *restorable = @[@"jpg", @"jpeg", @"png", @"heic", @"heif", @"tiff", @"tif", @"gif", @"bmp"];
    if (![restorable containsObject:url.pathExtension.lowercaseString]) return nil;
    return url;
}

// Draw the profile as a horizontal gradient and write it out. A UNIQUE filename every time: the
// WindowServer caches the desktop picture BY URL, so rewriting the same path with new content can
// leave the old picture on screen and read as "the API did nothing" (seen during the probe).
- (NSURL *)blinkeredWriteTintImageForProfile:(NSArray *)profile
{
    if (profile.count < 2) return nil;
    NSMutableArray<NSColor *> *colors = [NSMutableArray arrayWithCapacity:profile.count];
    CGFloat locations[64];
    NSUInteger n = MIN(profile.count, (NSUInteger)64);
    for (NSUInteger i = 0; i < n; i++) {
        NSArray *rgb = profile[i];
        [colors addObject:[NSColor colorWithSRGBRed:[rgb[0] doubleValue] / 255.0
                                              green:[rgb[1] doubleValue] / 255.0
                                               blue:[rgb[2] doubleValue] / 255.0
                                              alpha:1.0]];
        locations[i] = (CGFloat)i / (CGFloat)(n - 1);
    }
    NSGradient *gradient = [[NSGradient alloc] initWithColors:colors atLocations:locations
                                                   colorSpace:[NSColorSpace sRGBColorSpace]];
    if (!gradient) return nil;

    // DRAW IT AT THE DISPLAY'S EXACT PIXEL SIZE. This was 1024x8, on the reasoning that the strip only
    // samples the top and the OS would stretch the rest — but that asks the scaler to blow 8 pixels up
    // to 1912 and 1024 across to 2940, and what came out on device was NOT the gradient we drew:
    // reported from the screen, the strip was darkest beside the notch where our image is brightest.
    // The image file itself is provably correct (sampled off the device: a clean single hump peaking
    // dead centre), so the distortion is downstream, and an extreme-aspect source is the one variable
    // worth removing before blaming the menu bar's own rendering. At native size there is no scaling
    // left to go wrong — every pixel maps 1:1.
    NSScreen *targetScreen = [self blinkeredDesktopTintScreen] ?: NSScreen.screens.firstObject;
    CGFloat scale = targetScreen.backingScaleFactor ?: 2.0;
    NSInteger pixelsWide = MAX(1, (NSInteger)lround(targetScreen.frame.size.width * scale));
    NSInteger pixelsHigh = MAX(1, (NSInteger)lround(targetScreen.frame.size.height * scale));

    // NOTCH COMPENSATION — an empirical fit, and it needs to be read as one.
    //
    // macOS shades its own menu bar around the notch. That is not our doing and nothing we draw can
    // remove it: proved on device by setting a wallpaper that is a single flat colour at EVERY pixel
    // and still seeing the strip darken near the notch. Because the water is brightest dead centre,
    // that shading sits against the brightest part of the scene, and the mismatch reads as a dark
    // patch beside the notch — which is what James reported from the moment the tint first worked.
    //
    // These two numbers were fitted by eye against a live aquarium on an M4 MacBook Air, macOS
    // 26.6.2, by bracketing: 60% was plainly too much, 25% at this reach was called "really good".
    // They are NOT derived, and they are not physics. If the strip ever looks wrong after a macOS
    // update, come back here first — Apple can change this shading whenever they like.
    //
    // Anchored to the MEASURED notch, never to pixels: auxiliaryTopLeftArea/auxiliaryTopRightArea
    // give its real bounds, so the curve lands correctly on a different notch. On a display without
    // one, both are nil, the lift is exactly zero, and the wallpaper is the plain water gradient —
    // which is what every external monitor and older Mac gets.
    const double kNotchLift = 0.25;      // brightness lift at the notch centre
    const double kNotchSpread = 3.15;    // how far it reaches, in multiples of the notch half-width
    double notchCentre = 0.5, notchHalf = 0.0;
    if (targetScreen.auxiliaryTopLeftArea.size.width > 0 && targetScreen.auxiliaryTopRightArea.size.width > 0) {
        CGFloat leftEdge = NSMaxX(targetScreen.auxiliaryTopLeftArea);
        CGFloat rightEdge = NSMinX(targetScreen.auxiliaryTopRightArea);
        if (rightEdge > leftEdge && targetScreen.frame.size.width > 0) {
            notchCentre = ((leftEdge + rightEdge) / 2.0) / targetScreen.frame.size.width;
            notchHalf = ((rightEdge - leftEdge) / 2.0) / targetScreen.frame.size.width;
        }
    }
    ...
    [gradient drawInRect:NSMakeRect(0, 0, pixelsWide, pixelsHigh) angle:0.0];
    [NSGraphicsContext restoreGraphicsState];

    // Apply the lift per column, over the drawn gradient. Done on the pixels rather than by adding
    // gradient stops so the compensation stays visibly separate from the water's own colours — the
    // profile remains exactly what the page measured, and this is a correction sitting on top of it.
    if (notchHalf > 0 && kNotchLift > 0) {
        unsigned char *bytes = rep.bitmapData;
        NSInteger bpr = rep.bytesPerRow, spp = rep.samplesPerPixel;
        for (NSInteger x = 0; x < pixelsWide; x++) {
            double f = (double)x / (double)MAX(1, pixelsWide - 1);
            double d = fabs(f - notchCentre) / (notchHalf * kNotchSpread);
            if (d >= 1.0) continue;
            double lift = 1.0 + kNotchLift * (1.0 - d * d);
            for (NSInteger y = 0; y < pixelsHigh; y++) {
                unsigned char *px = bytes + y * bpr + x * spp;
                for (int ch = 0; ch < 3; ch++) px[ch] = (unsigned char)MIN(255.0, px[ch] * lift);
            }
        }
    }
    DDLogInfo(@"[Blinkered] setDesktopTint: wallpaper %ldx%ld, notch lift %.0f%% at %.0f%% of width (spread %.2f)",
              (long)pixelsWide, (long)pixelsHigh, kNotchLift * 100, notchCentre * 100,
              notchHalf > 0 ? kNotchSpread : 0.0);
    NSData *png = [rep representationUsingType:NSBitmapImageFileTypePNG properties:@{}];
    if (!png) return nil;

    NSArray *appSupport = NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES);
    NSString *dir = [[appSupport firstObject] stringByAppendingPathComponent:@"Blinkered"];
    [[NSFileManager defaultManager] createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:nil error:nil];
    NSString *name = [NSString stringWithFormat:@"desktop-tint-%@.png", [[NSUUID UUID] UUIDString]];
    NSURL *out = [NSURL fileURLWithPath:[dir stringByAppendingPathComponent:name]];
    if (![png writeToURL:out atomically:YES]) return nil;
    return out;
}

// Delete tint images this app wrote. Never touches anything else in the directory.
- (void)blinkeredRemoveWrittenTintImages
{
    NSArray *appSupport = NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES);
    NSString *dir = [[appSupport firstObject] stringByAppendingPathComponent:@"Blinkered"];
    NSFileManager *fm = [NSFileManager defaultManager];
    for (NSString *entry in [fm contentsOfDirectoryAtPath:dir error:nil]) {
        if ([entry hasPrefix:@"desktop-tint-"] && [entry.pathExtension isEqualToString:@"png"]) {
            [fm removeItemAtPath:[dir stringByAppendingPathComponent:entry] error:nil];
        }
    }
}

// THE 1pt BAND. Measured on device with CGWindowListCopyWindowInfo during a live block-all:
//
//     level 27  Blinkered (0, 33, 1470, 923)   the page — the aquarium starts at y=33
//     level 26  Blinkered (0, 32, 1470, 924)   the COVER — one point higher
//     level 24  WindowServer (0, 0, 1470, 33)  the menu bar
//
// so in y = 32..33 the topmost thing is the cover, painted blinkeredBrandBackdropColor (#1a1a2e).
// Against the pink wallpaper nobody noticed it; against the tinted strip it reads as a black line
// ruled under the menu bar — which is the artefact this whole feature exists to remove.
//
// It comes from the `- 1` in NSScreen+SEBScreen's menuBarHeight, so the cover is sized one point
// taller than visibleFrame. Two other fixes were considered and rejected: growing the browser window
// changes main-window geometry every layout depends on, and removing the `- 1` changes cover geometry
// for EVERY lock kind — including ones with no tint, where that band would then expose a point of the
// child's own wallpaper, the exact artefact again. Recolouring the cover is the narrow fix: it is only
// ever visible in this band, and it is put back when the tint is.
//
// CapWindow instances are deliberately skipped. self.capWindows also holds the red lockdown-alert and
// the modal dimmer covers; those colours ARE their meaning, and a teal "alert" would be a lie.
// Build the profile as a horizontal gradient IMAGE, sized in points to the window it will back, so a
// pattern colour maps it 1:1 across the width — the same 0..1 mapping the wallpaper gradient uses.
- (NSImage *)blinkeredGradientImageForProfile:(NSArray *)profile width:(CGFloat)width
{
    if (profile.count < 2 || width < 1) return nil;
    NSMutableArray<NSColor *> *colors = [NSMutableArray arrayWithCapacity:profile.count];
    CGFloat locations[64];
    NSUInteger n = MIN(profile.count, (NSUInteger)64);
    for (NSUInteger i = 0; i < n; i++) {
        ...
        [colors addObject:[NSColor colorWithSRGBRed:[rgb[0] doubleValue] / 255.0
                                              green:[rgb[1] doubleValue] / 255.0
                                               blue:[rgb[2] doubleValue] / 255.0 alpha:1.0]];
        locations[i] = (CGFloat)i / (CGFloat)(n - 1);
    }
    NSGradient *gradient = [[NSGradient alloc] initWithColors:colors atLocations:locations
                                                   colorSpace:[NSColorSpace sRGBColorSpace]];
    ...
    [image lockFocus];
    [gradient drawInRect:NSMakeRect(0, 0, width, 1) angle:0.0];
    [image unlockFocus];
    return image;
}

// THE COVER MUST CARRY THE GRADIENT, NOT ITS AVERAGE. 3.6.214 tinted the cover with the profile's
// mean and the line got better but did not go: James, on device — "the black line fades as it goes
// to the end". That is the signature of a flat colour under a gradient, and the numbers say the same
// thing. The log recorded covers tinted rgb(32,125,148) while the water runs rgb(51,149,172) in the
// middle and rgb(18,99,121) at the edges, so the cover reads DARKER than the water mid-screen and
// LIGHTER at the ends — a line that fades out towards both sides, exactly as reported.
//
// A pattern colour is the whole fix: an image the width of the window, mapped 0..1 across it, which
// is the same mapping the wallpaper gradient gets when it is stretched to the display. The band then
// matches the water at every x rather than at one.
// THE LINE IS A WINDOW SHADOW, and this is what actually removes it.
//
// Two earlier attempts recoloured the cover window — first with the profile's mean, then with the
// full gradient — and neither made the line go, which is the evidence that mattered: something was
// painting OVER the cover, not the cover itself. The main browser window's top edge sits at y=33 and
// macOS gives every window a shadow unless told otherwise (hasShadow is set NO in exactly one place
// in this codebase, the menu-bar shield). That shadow spills upward into y=32 — precisely the band —
// and the WindowServer composites it above the cover, so no colour underneath can help.
//
// It also explains the shape James reported: a shadow is a constant black, so it looks denser over
// the bright middle of the water and weaker over the darker edges. The backdrop varies; the line
// does not.
//
// A full-screen lock window has nothing to cast a shadow onto, so switching it off costs nothing —
// but it is scoped to the tint anyway, and put back with everything else, so no other window's
// appearance changes.
// RE-ASSERTED, not set once. The first version applied this a single time when the tint arrived, and
// the line came straight back after a crash-relaunch on device: at that moment the app is still
// starting, so mainBrowserWindow is either nil (this returned early and the shadow was never turned
// off) or is replaced moments later by a window carrying macOS's default shadow. Shadow-off has to be
// a property that survives the window being recreated, not an event that happened once.
//
// Re-asserting on a short schedule is the idiom this codebase already uses for the same class of
// problem — see the active-tab re-assert, which exists because native window work lands async and can
// undo what was just set.
- (void)blinkeredSetMainWindowShadow:(BOOL)wantsShadow
{
    [self blinkeredApplyMainWindowShadow:wantsShadow];
    if (!wantsShadow) {
        for (NSNumber *delay in @[@0.5, @2.0, @5.0]) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay.doubleValue * NSEC_PER_SEC)),
                           dispatch_get_main_queue(), ^{
                // Only while the tint is still live — an unlock in the meantime must not have its
                // shadow quietly taken away by a timer from the previous session.
                if (_blinkeredDesktopTintApplied) [self blinkeredApplyMainWindowShadow:NO];
            });
        }
    }
}

- (void)blinkeredApplyMainWindowShadow:(BOOL)wantsShadow
{
    NSWindow *mainWindow = self.browserController.mainBrowserWindow;
    if (!mainWindow) return;
    if (mainWindow.hasShadow == wantsShadow) return;
    mainWindow.hasShadow = wantsShadow;
    // The shadow is cached by the window server; without this the change does not take until the
    // window is next resized or re-ordered, which under a lock may be never.
    [mainWindow invalidateShadow];
    DDLogInfo(@"[Blinkered] main-window shadow %@", wantsShadow ? @"restored" : @"off");
}

- (void)blinkeredTintCapWindowsWithProfile:(NSArray *)profile
{
    for (id window in [self.capWindows copy]) {
        if (![window isKindOfClass:[NSWindow class]]) continue;
        if ([window isKindOfClass:NSClassFromString(@"CapWindow")]) continue;   // alert/modal covers keep their colour
        NSWindow *capWindow = window;
        if (!profile) {
            capWindow.backgroundColor = [SEBBrowserWindow blinkeredBrandBackdropColor];
            continue;
        }
        NSImage *gradient = [self blinkeredGradientImageForProfile:profile width:capWindow.frame.size.width];
        // Pattern colours anchor at the window's bottom-left, and these covers start at x=0 spanning
        // the screen, so an image of exactly the window's width lands 1:1 with no phase to set.
        capWindow.backgroundColor = gradient ? [NSColor colorWithPatternImage:gradient]
                                             : [SEBBrowserWindow blinkeredBrandBackdropColor];
    }
}

- (void)blinkeredSetDesktopTint:(NSNotification *)notification
{
    NSArray *profile = notification.userInfo[@"profile"];
    if (![profile isKindOfClass:[NSArray class]] || profile.count < 2) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        NSScreen *screen = [self blinkeredDesktopTintScreen];
        NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];

        // First application of this lock: prove reversibility, then record what to go back to.
        // Re-applications (the page re-measures on resize) skip straight to redrawing.
        if (!_blinkeredDesktopTintApplied) {
            NSURL *original = [self blinkeredRestorableWallpaperURLForScreen:screen];
            if (!original) {
                DDLogInfo(@"[Blinkered] setDesktopTint: skipped — this wallpaper cannot be restored by URL "
                          @"(a shuffling album or dynamic wallpaper). The menu-bar strip is left as it is.");
                return;
            }
            NSDictionary *options = [[NSWorkspace sharedWorkspace] desktopImageOptionsForScreen:screen] ?: @{};
            NSData *archived = [NSKeyedArchiver archivedDataWithRootObject:options
                                                    requiringSecureCoding:NO error:nil];
            [preferences setPersistedSecureObject:original.path forKey:blinkeredDesktopTintOriginalURLKey];
            [preferences setPersistedSecureObject:(archived ?: [NSData data])
                                           forKey:blinkeredDesktopTintOriginalOptionsKey];
            // The flag goes down LAST and comes up FIRST on restore: a crash between recording the
            // original and setting the tint must not leave a flag pointing at nothing, and a crash
            // after setting it must always leave a flag that the next launch can act on.
            [preferences setPersistedSecureBool:YES forKey:blinkeredDesktopTintActiveKey];
            _blinkeredDesktopTintApplied = YES;
        }

        NSURL *image = [self blinkeredWriteTintImageForProfile:profile];
        if (!image) { DDLogError(@"[Blinkered] setDesktopTint: could not write the tint image"); return; }
        NSError *error = nil;
        ...
        if (![[NSWorkspace sharedWorkspace] setDesktopImageURL:image forScreen:screen options:options error:&error]) {
            DDLogError(@"[Blinkered] setDesktopTint: setDesktopImageURL failed — %@", error);
            return;
        }
        // The cover's 1pt band under the menu bar gets the SAME gradient, not its average (see the note
        // on blinkeredTintCapWindowsWithProfile: — the average is what left a line fading out to the sides).
        [self blinkeredTintCapWindowsWithProfile:profile];
        [self blinkeredSetMainWindowShadow:NO];

        DDLogInfo(@"[Blinkered] setDesktopTint: applied a %lu-stop gradient to the menu-bar display "
                  @"and to the cover band; main-window shadow off", (unsigned long)profile.count);
    });
}

- (void)blinkeredClearDesktopTint
{
    dispatch_async(dispatch_get_main_queue(), ^{ [self blinkeredRestoreDesktopWallpaper]; });
}

// Idempotent, and safe to call when no tint was ever applied — which is exactly what makes it usable
// from three places at once: the page's clearDesktopTint, the exit path, and the next launch after a
// lock died without unwinding.
- (void)blinkeredRestoreDesktopWallpaper
{
    NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];
    if (![preferences persistedSecureBoolForKey:blinkeredDesktopTintActiveKey]) {
        _blinkeredDesktopTintApplied = NO;
        return;
    }
    // persistedSecureObjectForKey:, not a ...StringForKey: — the category has no string variant, and
    // what comes back is whatever was stored, so it is class-checked before use.
    id storedPath = [preferences persistedSecureObjectForKey:blinkeredDesktopTintOriginalURLKey];
    NSString *path = [storedPath isKindOfClass:[NSString class]] ? storedPath : nil;
    id archived = [preferences persistedSecureObjectForKey:blinkeredDesktopTintOriginalOptionsKey];
    NSDictionary *options = nil;
    if ([archived isKindOfClass:[NSData class]] && [(NSData *)archived length] > 0) {
        NSSet *classes = [NSSet setWithObjects:NSDictionary.class, NSString.class, NSNumber.class, NSColor.class, nil];
        options = [NSKeyedUnarchiver unarchivedObjectOfClasses:classes fromData:archived error:nil];
    }
    if (path.length > 0 && [[NSFileManager defaultManager] fileExistsAtPath:path]) {
        NSError *error = nil;
        NSURL *url = [NSURL fileURLWithPath:path];
        if (![[NSWorkspace sharedWorkspace] setDesktopImageURL:url
                                                     forScreen:[self blinkeredDesktopTintScreen]
                                                       options:(options ?: @{}) error:&error]) {
            // Loud, not silent: this is the failure that leaves a child's Mac altered.
            DDLogError(@"Blinkered SECURITY: could NOT restore the desktop wallpaper to %@ — %@", path, error);
        } else {
            DDLogInfo(@"[Blinkered] desktop wallpaper restored to %@", path);
        }
    } else {
        DDLogError(@"Blinkered SECURITY: the recorded wallpaper (%@) is gone — cannot restore it.",
                   path.length ? path : @"<none>");
    }
    // Clear the breadcrumb whatever happened. Retrying forever cannot help — the recorded file is
    // either settable or missing — and a flag that never clears would re-run this on every launch.
    [preferences setPersistedSecureBool:NO forKey:blinkeredDesktopTintActiveKey];
    [preferences setPersistedSecureObject:@"" forKey:blinkeredDesktopTintOriginalURLKey];
    [preferences setPersistedSecureObject:[NSData data] forKey:blinkeredDesktopTintOriginalOptionsKey];
    _blinkeredDesktopTintApplied = NO;
    // Covers go back to the brand backdrop on exactly the paths the wallpaper does — including the
    // launch-recovery one, where there are no cover windows yet and this is a harmless no-op.
    [self blinkeredTintCapWindowsWithProfile:nil];
    [self blinkeredSetMainWindowShadow:YES];
    [self blinkeredRemoveWrittenTintImages];
}
...
- (void)startMainWindowBottomEdgeReveal {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(blinkeredFocusMainContent:)
                                               ...
                                               object:nil];
    // The locked page measured the aquarium's top row and wants the desktop under the menu-bar strip
    // to match it (and, on the way out, to be handed back). The reversibility gate lives in the
    // handler: a wallpaper that cannot be provably restored is never touched.
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(blinkeredSetDesktopTint:)
                                                 name:@"BlinkeredSetDesktopTint"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(blinkeredClearDesktopTint)
                                                 name:@"BlinkeredClearDesktopTint"
                                               object:nil];
...
- (void)blinkeredScreensDidWake:(NSNotification *)n {
    ...
    // #100: the DETERMINISTIC half of the Space remedy's retry. The observed 3.6.204 wedge was a
    // lock that landed while the screen was asleep — the one cover-screens attempt could not stick,
    // and nothing tried again until the owner clicked around. The enforcement tick retries too, but
    // NSTimers do not fire across system sleep, so on this edge the poll lands whenever the runloop
    // is coalesced back rather than at wake. This fires at wake, costs nothing when nothing is
    // wrong, and is rate-limited inside like every other entry point.
    [self blinkeredEnsureOrdinarySpace:@"screens-did-wake"];
}
...
- (void)blinkeredReportWakeNudge:(NSString *)snapVerdict snapshotMs:(NSInteger)snapMs raf:(NSString *)rafState
    ...
    // P7 — A MEASUREMENT, NOT A CONSTANT. This field shipped as a literal `@YES`, which is a claim
    // ("the display was asleep") dressed as an observation, on every one of the 443 wake_nudge rows
    // in production. A telemetry field that reads like a measurement and is a constant has already
    // misled one investigation on this rail: the sibling paint_fault reporter had the same field
    // computed from "a wake notification fired in the last 5 seconds", and a fleet of screen-off
    // captures reading `displaySlept:false` is what made them look like "screen on, window black".
    //
    // Same expression as that sibling now (blinkeredReportPaint, ~90 lines above), so the field means
    // ONE thing across both kinds. And it is worth reading rather than deleting: the wake edge fires
    // on dark wakes too, so the panel state at report time is exactly what separates a dark wake
    // (display still asleep — a snapshot of a page nobody can see) from a full one. Under `@YES` that
    // distinction was unreadable in every row ever collected.
    BOOL displaySlept = CGDisplayIsAsleep(CGMainDisplayID()) != 0;
    NSString *appVer = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"] ?: @"";
    NSMutableDictionary *body = [@{ @"token": devTok, @"kind": @"wake_nudge",
        ...
        @"reloadRan": @(reloadRan), @"displaySlept": @(displaySlept), @"lpm": @(lpm),
...
// D4 — NOT under .../Application Support/Blinkered. That directory is 0700 root:admin (created that way
// by BlinkeredUpdater/main.swift:322) and a non-root process CANNOT TRAVERSE IT, so a login item naming
// a program inside it is unrunnable by the very accounts that must run it. Measured on two devices; the
// first cut of §8.1 shipped there and was inert on the entire fleet while passing every check, because
// every check ran as root and root traverses anything. Worse, that mode is decided by a race —
// createDirectory applies attributes only at creation and `mkdir -p` leaves an existing dir alone — so
// whichever of the updater and the installer runs first sets it for ever. This path is created and
// hardened by ONE component (pkg/scripts/postinstall) with a 0755 chain, and the postinstall FAILS THE
// INSTALL unless an unprivileged uid can actually execute it.
static NSString * const kBlinkeredGuardLauncher = @"/Library/Application Support/BlinkeredGuard/BlinkeredGuard";
...
- (NSArray<NSString *> *)blinkeredAgentProgramArgumentsFor:(NSString *)agentBinary {
    // §8.1 — PREFER THE SIGNED LAUNCHER. Only when the pkg has installed it: a DMG/Cooperative install
    // never runs the postinstall, so on those devices it is absent and the /bin/zsh form below stays the
    // canonical one. Because this method is also what blinkeredPlistIsCanonical: compares against, a pkg
    // upgrade that lands the launcher makes the existing plist non-canonical, so the next full install
    // pass rewrites it — changing the executable path and renewing the BTM record. That is the fleet
    // repair, and it is why this check lives in the canonical definition rather than at the call site.
    if ([[NSFileManager defaultManager] isExecutableFileAtPath:kBlinkeredGuardLauncher]) {
        return @[kBlinkeredGuardLauncher, @"launch", agentBinary];
    }
...
- (NSDictionary *)blinkeredAgentPlistFor:(NSString *)agentBinary logPath:(NSString *)logPath {
    return @{
        ...
        // ATTRIBUTION, the documented way. This is the only Apple-DOCUMENTED lever over what System
        // Settings -> Login Items shows for a legacy plist, and we were not using it. From "Updating
        // helper executables from earlier versions of macOS": if a legacy LaunchAgent has no
        // AssociatedBundleIdentifiers key, System Settings shows "the organization name in the app's
        // signing certificate" rather than the app name; and if it cannot attribute the job at all and
        // the executable is unsigned, it shows the bare executable name. That last case is what the
        // fleet has today — `/bin/zsh`, so `Name: zsh`, `Developer Name: (null)`, "Unknown Developer".
        //
        // The key is CONDITIONAL ON §8.1. Apple's rule: "The Team Identifier of the Program or
        // ProgramArguments executable in the legacy property list must match that of the app bundle for
        // the AssociatedBundleIdentifiers key." /bin/zsh's lightweight code requirement carries NO
        // team-identifier at all (measured with `sfltool csinfo`: Team Identifier (null)), so on the
        // fallback form this key is discarded by the system — harmless, but inert. It only bites once
        // ProgramArguments[0] is our own signed launcher, which shares the app's team 6PRT29D483.
        //
        // Apple's own words on why a shell wrapper cannot be rescued any other way: attribution "relies
        // on the concept of responsible code, and the system has a very hard time tracking that when
        // you use a script… switch to using a Mach-O executable." A third-party interpreter does not
        // help either — it carries ITS vendor's team id, which also fails the match.
        @"AssociatedBundleIdentifiers": @[@"app.blinkered.Blinkered"],
    };
}
...
// M-A (docs/OS_UPDATE_RESILIENCE_PLAN.md §3): forward declarations — the predicate and the pre-kill
// cdhash helper are defined near the other SecCode telemetry helpers but used at the kill sites below.
static BOOL BlinkeredKillFailureIsUnkillableSystemProcess(pid_t pid, NSData *preKillCdhash, int killErrno);
static NSData *BlinkeredGuestCdhashForPid(pid_t pid);
static NSString *BlinkeredOffenderClass(pid_t pid);
// Returns YES only if a marker actually REACHED DISK — not merely "was not throttled", which is what
// an earlier version returned while BlinkeredWriteMarkerDict swallowed every write error. A caller
// that dedupes on "I reported this" must key on the truth, or a full disk retires the key exactly as
// a success does. The 15-minute throttle is keyed alertType:signal:demoted, identical for every
// thrashing path, so kill_thrash is exempt from it and self-limits per offender instead (§3.6).
static BOOL BlinkeredReportLockdownAlert(NSString *alertType, NSString *signal, NSString *offenderClass,
                                         BOOL demoted, int killErrno, BOOL firstScan, NSInteger screenCount,
                                         NSInteger thrashCount, NSInteger offenderOrdinal);
// M-A §3.6: note a SUCCESSFUL kill against the thrash counter. Telemetry only — never gates a kill.
static void BlinkeredNoteKillForThrash(NSString *execPath, NSString *offenderClass);
// The executable path of a LIVE pid, captured BEFORE the kill (§3.6). After the kill the pid is gone
// and proc_pidpath fails, and launchd respawns the target under a NEW pid — so the path is the only
// stable key a thrash can be counted on, and it must be read while the process still exists. Same
// shape as M-A's pre-kill cdhash capture, for the same reason.
static NSString *BlinkeredExecPathForPid(pid_t pid);
        ...
        if (bundleID) {
            ...
            if (matchingProhibitedApplications.count != 0) {
                ...
                if ([prohibitedProcess[@"strongKill"] boolValue] == YES) {
                    ...
                    NSData *preKillCdhash = BlinkeredGuestCdhashForPid(processPID);   // M-A §3.5a: before the kill
                    NSError *killError = nil;
                    if (![runningApplication killReturningError:&killError]) {
                        int killErrno = [[killError.userInfo objectForKey:SEBErrorKillProcessErrnoKey] intValue];
                        if (BlinkeredKillFailureIsUnkillableSystemProcess(processPID, preKillCdhash, killErrno)) {
                            // M-A §3.5a: an un-killable Apple system process must not BLOCK the session
                            // start (worse than the red screen — nothing even begins). Report, don't add.
                            DDLogError(@"M-A: demoting un-killable Apple system process (pid %d, errno %d) at session start — not blocking the session.", processPID, killErrno);
                            BlinkeredReportLockdownAlert(@"prohibited_process", @"unkillable_system_process",
                                                         BlinkeredOffenderClass(processPID), YES, killErrno, NO, (NSInteger)[NSScreen screens].count, 0 /*not a thrash*/, 0 /*no ordinal*/);
                        } else {
            ...
            if (filteredProcesses.count != 0) {
                ...
                if ([prohibitedProcess[@"strongKill"] boolValue] == YES) {
                    NSData *preKillCdhash = BlinkeredGuestCdhashForPid(processPID);   // M-A §3.5a: before the kill
                    NSError *killError = nil;
                    if (![NSRunningApplication killProcessWithPID:processPID error:&killError]) {
                        int killErrno = [[killError.userInfo objectForKey:SEBErrorKillProcessErrnoKey] intValue];
                        if (BlinkeredKillFailureIsUnkillableSystemProcess(processPID, preKillCdhash, killErrno)) {
                            DDLogError(@"M-A: demoting un-killable Apple system process (pid %d, errno %d) at session start — not blocking the session.", processPID, killErrno);
                            BlinkeredReportLockdownAlert(@"prohibited_process", @"unkillable_system_process",
...
// M-C signing self-check (docs/OS_UPDATE_RESILIENCE_PLAN.md §5) — counters + forward declarations here
// because windowWatcher (below) increments the counters and calls the reporter, but the definitions
// live near the other telemetry helpers. _Atomic: incremented on the watcher's queue, read on the
// self-check's background queue.
static _Atomic long blinkeredSigningLivePasses = 0;
static _Atomic long blinkeredSigningLiveFailures = 0;
static void BlinkeredRunSigningSelfCheckOnce(void);
static void BlinkeredMaybeReportLivePopulation(void);

// Start the windows watcher if it's not yet running
- (void)startWindowWatcher
{
    DDLogDebug(@"%s", __FUNCTION__);
    // M-C: one-shot signing self-check for this session — a canary, reports only, never acts.
    BlinkeredRunSigningSelfCheckOnce();
            ...
            if (windowLevel >= NSMainMenuWindowLevel+2) {
                ...
                if (windowOwnerPID != sebPID) {
                    if (![_systemProcessPIDs containsObject:windowOwnerPIDString]) {
                        ...
                        // Blinkered: never terminate our OWN bundled banner helper. The pre-lock
                        // warning (BlinkeredAgent → BlinkeredBanner) deliberately draws a small
                        // top-of-screen notice ABOVE the kiosk — "Bedtime starts in 10 minutes"
                        // over a live homework lock is the warning with the most at stake (a child
                        // mid-homework about to hit a block-all needs to save their work). Without
                        // this allowance the scanner would kill the helper the instant it drew.
                        // Identity is the executable's PATH INSIDE THIS SEALED BUNDLE, not a bundle
                        // id (the helper is a bare binary and has none): on an Enforced install the
                        // bundle is root-owned, so a child cannot plant an impostor there.
                        NSString *ownBannerHelperPath = [[[NSBundle mainBundle] bundlePath]
                                                         stringByAppendingPathComponent:@"Contents/MacOS/BlinkeredBanner"];
                        NSString *panelExecPath = appWithPanel.executableURL.path;
                        if (panelExecPath && [panelExecPath isEqualToString:ownBannerHelperPath]) {
                            DDLogWarn(@"Allowing Blinkered's own banner helper (pid %d) to keep its window — pre-lock warning.", windowOwnerPID);
                            [_systemProcessPIDs addObject:windowOwnerPIDString];
                            continue;
                        }
#ifndef DEBUG
                        ...
                        if (!_sessionState.allowSwitchToApplications && ![_preferencesController preferencesAreOpen]) {
                            if (appWithPanelBundleID && ![appWithPanelBundleID hasPrefix:@"com.apple."]) {
                                ...
                                if ([appWithPanelBundleID isEqualToString:FinderBundleID]) {
                                    ...
                                    // Without this continue, Finder fell through into the signature
                                    // check below and was killed a SECOND time by that branch
                                    // (docs/FOREIGN_APP_TERMINATION_PLAN.md §2.4). It must also not
                                    // fall through now that the check passes for Finder: caching it
                                    // in _systemProcessPIDs would exempt it from future scans.
                                    continue;
                                }
#endif
                                ...
                                if ([self signedSystemExecutable:windowOwnerPID]) {
                                    ...
                                    [_systemProcessPIDs addObject:windowOwnerPIDString];
                                    atomic_fetch_add(&blinkeredSigningLivePasses, 1);   // M-C live-population tally (telemetry only)
                                } else {
                                    atomic_fetch_add(&blinkeredSigningLiveFailures, 1); // M-C: a com.apple.* window that FAILED the check
                                    BlinkeredMaybeReportLivePopulation();
                                    // The app which opened the window or panel is no system process
                                    if (firstScan) {
                                        ...
                                        [self killProcessWithPID:windowOwnerPID];   // §3.6 thrash counting lives in killProcess:
                                        fishyWindowWasOpened = YES;
                                    }
                                }
                            }
...
- (BOOL)signedSystemExecutable:(pid_t)runningExecutablePID
{
    ...
    if (executablePath) {
        ...
        // "Is this Apple's own software?" must never be answered by pinning a certificate
        // fingerprint: Apple rotates the OS signing certificate (macOS 26.6.2 moved every
        // system binary to a "macOS Software Signing" leaf), and a stale pin makes this
        // method return NO for ALL system software — the window watcher then tries to kill
        // WindowServer, the kill fails with EPERM, and the session locks with the red
        // "Prohibited Process" screen on every click. `anchor apple` alone is also wrong:
        // Apple's own Mac App Store apps (Keynote, Pages, GarageBand) are signed
        // "Apple Mac OS Application Signing", FAIL it, and carry com.apple. bundle IDs, so
        // they would keep being killed in this branch. The union below admits both, and
        // still rejects Developer ID (Chrome, Firefox, Blinkered itself) — the measured
        // acceptance table is docs/FOREIGN_APP_TERMINATION_PLAN.md §2.5, prescription §5/M3.
        // On any error this returns NO, keeping the current kill behaviour: fail closed.
        static SecRequirementRef appleSoftwareRequirement = NULL;
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            NSString *reqStr = @"anchor apple or (anchor apple generic and certificate leaf[field.1.2.840.113635.100.6.1.9] exists)";
            SecRequirementRef createdReq = NULL;
            OSStatus createStatus = SecRequirementCreateWithString((__bridge CFStringRef)reqStr, kSecCSDefaultFlags, &createdReq);
            if (createStatus == noErr && createdReq != NULL) {
                appleSoftwareRequirement = createdReq;
            } else {
                DDLogError(@"Could not create the Apple-software code requirement (status %d) — every process will be treated as non-system software (fail closed).", (int)createStatus);
            }
        });

        if (appleSoftwareRequirement == NULL) {
            CFRelease(ref);
            DDLogDebug(@"Code signature suggests that %@ isn't correctly signed macOS system software.", executablePath);
            return NO;
        }

        status = SecStaticCodeCheckValidity(ref, kSecCSCheckAllArchitectures, appleSoftwareRequirement);
...
// STRICTLY SHORTER than the enforcement tick (8 x 0.25s = 2.0s). Equal periods meant ordinary timer
// jitter put roughly half the ticks marginally inside the window, so the real retry cadence
// oscillated between 2s and 4s — and any cover-screens or space-changed call landing just before a
// tick consumed that tick's entire budget, starving the two ORDERING-CRITICAL entry points in
// favour of the poll. 1.0s leaves headroom for both.
static const NSTimeInterval kBlinkeredSpaceRemedyMinInterval = 1.0;

// MONOTONIC. The retry engine must not be disableable by a clock correction: with wall-clock stamps
// a backward NTP step (or a restore-from-backup, or a manual Date & Time change) makes the delta
// negative, which passes the "< interval" test on every call and blocks the remedy from every entry
// point until wall time catches up — minutes or hours of exactly the wedge this module prevents.
static NSTimeInterval blinkeredMonotonicNow(void)
{
...
// Ask every OTHER app that is in native full-screen to leave it, via Accessibility.
//
// WHY THIS EXISTS, and why it is a PRE-STEP rather than a replacement for the Space switch.
// The Space switch changes which Space is on screen; it does NOT take the app out of full-screen.
// The app therefore stays full-screen on a Space nobody is looking at, and the device owner reports
// two symptoms that follow from that: the kiosk's LEFT-HAND menu region does not render (right-hand
// status items do), and a full-screen Chrome comes back in a poor state.
//
// The mechanism behind the menu-bar symptom is NOT established — two explanations were tested on
// the device and both were falsified (docs/FULLSCREEN_MENUBAR_OVERLAY_DIAGNOSIS.md). What IS
// established is the condition: while any app is in native full-screen, the left region does not
// render. Making the app genuinely leave full-screen removes that condition under every hypothesis,
// which is why this is worth doing even with the mechanism open.
//
// STRICTLY ADDITIVE. If Accessibility is not granted, or the AX calls fail, or the app ignores us,
// this returns and the Space switch below runs exactly as before. It can only remove the
// full-screen condition; it can never leave the kiosk worse off than not calling it.
//
// NEVER TOUCHES BLINKERED'S OWN WINDOWS: the kiosk's covers are borderless windows at elevated
// levels, not AX full-screen, but the pid check makes that explicit rather than incidental.
//
// The messaging timeout is load-bearing: AXUIElementCopyAttributeValue blocks on the TARGET app's
// run loop, and this runs on OUR main thread. An unresponsive or beachballed app would otherwise
// stall the kiosk — the one thing a lockdown must not do.
static BOOL blinkeredExitForeignFullScreen(pid_t ourPID)
{
    if (!AXIsProcessTrusted()) {
        DDLogWarn(@"Blinkered: Accessibility not granted — cannot ask full-screen apps to exit; falling back to the Space switch alone");
        return NO;
    }
    BOOL askedAny = NO;
    for (NSRunningApplication *app in [NSWorkspace sharedWorkspace].runningApplications) {
        if (app.processIdentifier == ourPID || app.processIdentifier <= 0) {
            continue;
        }
        if (app.activationPolicy != NSApplicationActivationPolicyRegular) {
            continue;   // agents and UI elements own no full-screen Space
        }
        AXUIElementRef axApp = AXUIElementCreateApplication(app.processIdentifier);
        if (!axApp) {
            continue;
        }
        AXUIElementSetMessagingTimeout(axApp, 0.5);   // see note above: never stall the kiosk
        ...
        if (AXUIElementCopyAttributeValue(axApp, kAXWindowsAttribute, &windows) == kAXErrorSuccess &&
            ...
            for (id w in (__bridge NSArray *)windows) {
                ...
                CFTypeRef isFull = NULL;
                if (AXUIElementCopyAttributeValue(win, CFSTR("AXFullScreen"), &isFull) == kAXErrorSuccess &&
                    isFull != NULL) {
                    if (CFGetTypeID(isFull) == CFBooleanGetTypeID() && CFBooleanGetValue(isFull)) {
                        AXError setErr = AXUIElementSetAttributeValue(win, CFSTR("AXFullScreen"), kCFBooleanFalse);
                        DDLogWarn(@"Blinkered: asking %@ to leave full-screen via Accessibility: %@",
                                  app.localizedName ?: @"(unnamed)",
                                  setErr == kAXErrorSuccess ? @"accepted" : ([NSString stringWithFormat:@"AXError %d", (int)setErr]));
...
// Why the kiosk must NOT act on the screen right now, or nil when it may. SINGLE SOURCE: the BOOL
// below derives from this, so the two can never disagree. The first version had them as two
// hand-maintained chains testing the same five conditions in the same order, with the reason
// falling through to an ASSUMED "levels-not-elevated" for anything unlisted — a confident wrong
// answer in a log written precisely so a wrong diagnosis could not happen twice.
- (NSString *)blinkeredKioskOwnershipBlockReason
{
    if (_blinkeredTeardownStarted) { return @"teardown"; }
    if (_isTerminating) { return @"terminating"; }
    if (self.settingsOpen) { return @"settings-open"; }
    if (_isAACEnabled || _wasAACEnabled) { return @"aac"; }
    if (_sessionState.allowSwitchToApplications) { return @"apps-allowed"; }
    // THE EARLY-STARTUP WINDOW IS THE ONE THAT MATTERS. While the session-launch covers are up,
    // settings have not been applied yet and elevateWindowLevels still reads NO — but -coverScreens
    // elevates the covers anyway (same condition). That window is precisely when the covers are
    // first CREATED, i.e. when being on the wrong Space does the damage, so gating on the
    // preference alone would skip the only pass that could prevent the bypass.
    if (_blinkeredEarlyCoversActive && _startingUp) { return nil; }
    // elevateWindowLevels YES == "third-party apps are NOT allowed", i.e. the covers are elevated
    // and the kiosk owns the screen. Read explicitly rather than inferred, so the reason is an
    // observation.
    if (![[NSUserDefaults standardUserDefaults] secureBoolForKey:@"org_safeexambrowser_elevateWindowLevels"]) {
        return @"levels-not-elevated";
    }
    return nil;
}
...
{
    return [self blinkeredKioskOwnershipBlockReason] == nil;
}
...
- (BOOL)blinkeredEnsureOrdinarySpace:(NSString *)reason
{
    ...
    NSTimeInterval now = blinkeredMonotonicNow();
    ...
    BOOL switched = NO;
    @try {
    // PRE-STEP: ask the full-screen app(s) to leave full-screen. STRICTLY ADDITIVE — see the note on
    // blinkeredExitForeignFullScreen. Deliberately NO waiting and NO early return here: the exit is
    // asynchronous and macOS animates it, but pumping the run loop to wait would re-enter this very
    // method from the window watcher, and an early return would skip the closed-loop re-read below.
    // So we ask, then fall through to the Space switch exactly as before. If the AX exit lands, the
    // Space collapses on its own and the switch is a harmless no-op; the closed-loop re-read at the
    // end reports what actually happened either way.
    BOOL askedExit = blinkeredExitForeignFullScreen(sebPID);
    switched = blinkeredSwitchDisplayToSpace(display, ordinarySpace);
    if (switched) {
        // Re-assert the SURFACES only. Not -reinforceKioskMode: that also performs session-state
        // transitions (re-reading SEB_allowSwitchToApplications, writing elevateWindowLevels,
        // clearing _blinkeredEarlyCoversActive) which, on the early-startup path this remedy is
        // specifically allowed to run on, could drop the covers to normal level and disable this
        // module for the rest of the session. See -blinkeredReassertKioskSurfaces.
        [self blinkeredReassertKioskSurfaces];
    }
    // CLOSED LOOP: `switched` so far is only "the private CGS call returned 0". Re-read the Space
    // to find out whether anything actually moved. Without this, a call that returns 0 without
    // effect — the likely case against a sleeping or locked display — logs "OK" forever while the
    // kiosk stays bypassed, which is worse than silence: it argues against the true cause.
    BOOL landed = switched && (blinkeredCurrentSpaceState(NULL, NULL) != BlinkeredSpaceStateFullScreen);
    DDLogWarn(@"Blinkered: foreign full-screen Space was current (%@) — AX-exit-asked=%@, switching display %@ to ordinary Space %llu: %@",
              reason, askedExit ? @"YES" : @"no", display, (unsigned long long)ordinarySpace,
              landed ? @"OK" : (switched ? @"CALL OK BUT SPACE DID NOT CHANGE" : @"FAILED"));
    switched = landed;
    }
    @finally {
        // @finally, not a trailing assignment. The body above reaches window creation, encrypted
        // defaults and AppKit calls that can raise; AppKit swallows main-runloop exceptions and
        // CONTINUES, so an escaping throw would have latched this flag YES for the life of the
        // process and silently disabled the remedy from all three entry points — the exact
        // "enforcement stopped retrying" failure this module exists to prevent.
...
//
// SPOOFING, and where the bound ACTUALLY comes from. kCGWindowOwnerName comes from the process
// itself, so an app can name itself "Dock" to be skipped here. An earlier version of this comment
// said the bundle-identifier check below made that harder; IT DOES NOT, and the claim was wrong.
// A bundle identifier is self-declared too — an unsigned bundle in /tmp declaring
// com.apple.OSDUIHelper is accepted by this predicate, demonstrated 11 Sep 2026.
//
// The real bound is two things, neither of them here:
//   * this list only affects the REPORT. The Space remedy and the cover levels do not consult it,
//     so nothing below can weaken enforcement itself; and
//   * -windowWatcher independently CODE-SIGNATURE-checks (signedSystemExecutable:) any process that
//     opens a window at windowLevel >= NSMainMenuWindowLevel+2, which is where the covers sit.
// An impostor therefore cannot get above the cover, whatever it calls itself. Keep it that way: if
// this list is ever consulted by a kill or a remedy path, that reasoning is void.
//
// A process that fails to resolve is treated as NOT chrome, i.e. as blocking.
static BOOL blinkeredWindowOwnerIsSystemChrome(NSString *ownerName, pid_t ownerPID)
{
    static NSSet *chromeNames;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // The last three were added 11 Sep 2026 from PRODUCTION EVIDENCE, not from a guess: they
        // account for 8 of the 31 not-enforced alerts ever raised, every one of them a transient
        // overlap of <= 90 s that told a parent "there's a problem with Blinkered" because a volume
        // HUD, a notification banner or the screenshot UI briefly sat over the cover. Both names
        // were measured on-device to resolve to com.apple.* bundle identifiers, and OSDUIHelper is
        // attested in production on macOS 14.8.9 and 26.6.2 — twelve major versions apart.
        //
        // "Screenshot" is the screenshot UI (com.apple.screencaptureui), which is NOT the same
        // process as `screencapture`, the CLI the prohibited-process kill targets. Exempting the UI
        // from the geometric REPORT does not exempt the CLI from being killed; they are different
        // subsystems and must not be conflated.
        //
        // Every member is pinned by value in tools/lockdown-tests/check-system-chrome-allowlist.sh,
        // which DRIVES the compiled predicate. Adding or removing a name without updating that gate
        // fails the build, deliberately: this list is the kind of thing a later reader tidies.
        chromeNames = [NSSet setWithArray:@[@"Window Server", @"Dock", @"loginwindow",
                                            ...
                                            @"OSDUIHelper", @"UserNotificationCenter",
                                            @"Screenshot"]];
    });
...
// Is this PID our OWN bundled banner helper? Identity is the executable's PATH INSIDE THIS BUNDLE
// — never a name or a bundle id. The helper is a bare binary with no bundle id of its own. This is
// the same test windowWatcher's allowance uses (search ownBannerHelperPath); the two must agree,
// because a process the scanner deliberately spares must not then be counted as the thing
// obscuring us.
//
// WHY A PATH COMPARE IS ENOUGH, AND WHY A SIGNATURE CHECK WOULD NOT ADD ANYTHING. The path is
// derived from mainBundle — the bundle THIS binary is executing from — so an impostor has to sit
// inside it. Anyone who can write there can equally replace Contents/MacOS/Blinkered, the binary
// computing this very verdict. The allowance is exactly co-extensive with "the child already owns
// the app", so it opens no new surface, and a code-signing check would be redundant against an
// attacker who can swap the checking binary.
//
// (An earlier version of this comment argued "the bundle is root-owned so a child cannot plant an
// impostor". That is true of the Enforced .pkg — postinstall chowns root:wheel, chmods go-w and
// FAILS the install if any file is left non-root-owned or group/other-writable — but it is FALSE on
// a .dmg or a dev install, where /Applications/Blinkered.app is user-owned. The argument above
// holds for every shape, so it is the one to keep.)
//
// The stake if a spoof did succeed is worth naming: skipping an impostor lets the loop reach SEB's
// own cover behind it and return ok:YES while the child is looking at the attacker's window — a
// false "ENFORCING", which is the direction this file elsewhere calls a lie to a parent.
//
// Fail-closed: an unreadable executable path returns NO, so an unidentifiable window still blocks
// the verdict. A false "not enforcing" is a bad alert; a false "enforcing" is a lie to a parent.
static BOOL blinkeredWindowOwnerIsOwnBannerHelper(pid_t ownerPID)
{
    if (ownerPID <= 0) {
        return NO;
    }
    NSRunningApplication *owner = [NSRunningApplication runningApplicationWithProcessIdentifier:ownerPID];
    NSString *execPath = owner.executableURL.path;
    if (execPath.length == 0) {
        return NO;
    }
    NSString *ownBannerHelperPath = [[[NSBundle mainBundle] bundlePath]
                                     stringByAppendingPathComponent:@"Contents/MacOS/BlinkeredBanner"];
    return [execPath isEqualToString:ownBannerHelperPath];
}

// Why no verdict can be given right now, or nil when one CAN be. Reporting is suppressed in more
// situations than enforcement is: -blinkeredKioskShouldOwnScreen governs whether we should ACT,
// this governs whether we can honestly SAY anything.
//
// It exists as a named reason rather than a bare BOOL because the first version of this module went
// completely SILENT when suppressed. A field report of "the lock did not take" then could not be
// told apart from "the screen was asleep" without going to macOS's own logs, which is what actually
// happened while diagnosing 3.6.204. A control that stops speaking in the one case that matters is
// not observable.
- (NSString *)blinkeredVerdictSuppressionReason
{
    NSString *blocked = [self blinkeredKioskOwnershipBlockReason];
    if (blocked) { return blocked; }
    // The OS is covering the screen for us — strictly MORE secure, but geometrically
    // indistinguishable from something obscuring the kiosk. Also true when fast-user-switched away.
    if (blinkeredScreenIsLockedOrSaverActive()) { return @"screen-locked-or-switched-away"; }
    // Both no-screen shapes, so the verdict has no nil path this function cannot name. The first
    // version modelled only `screens.firstObject == nil` and left `screenArea <= 0` uncovered,
    // which made the display-asleep case — the one it was written for — log "unknown".
    NSScreen *screen = NSScreen.screens.firstObject;
    if (screen == nil || (screen.frame.size.width * screen.frame.size.height) <= 0) { return @"no-screen"; }
    return nil;
}
...
- (NSDictionary *)blinkeredComputeEnforcementVerdictSuppression:(NSString **)suppressionOut
{
    // Computed ONCE and handed back, so the log names the reason that actually suppressed this
    // verdict rather than a second reading taken microseconds later that may disagree.
    NSString *suppression = [self blinkeredVerdictSuppressionReason];
    if (suppressionOut) { *suppressionOut = suppression; }
    if (suppression != nil) {
        return nil;
    }
    NSScreen *screen = NSScreen.screens.firstObject;
    ...
    for (NSDictionary *window in windowList) {
        ...
        if (fraction >= 0.02) {
            // Our own bundled banner helper draws a SMALL notice above the kiosk by design — the
            // pre-lock warning ("Bedtime starts in 10 minutes"), which is deliberately on top of a
            // live lock. It is our window, so it must not read as something obscuring us. Without
            // this, a sticky final-minute warning flipped the verdict to obscured-by-<helper> and
            // reported a false "NOT ENFORCING" to the parent.
            //
            // THE BUG IS DISPLAY-DEPENDENT, not universal — worth knowing before hunting it. The
            // panel is a fixed 460x84 = 38,640 pt^2, so it crosses the 0.02 threshold only below
            // ~1,932,000 pt^2 of screen: 1440x900 gives 0.0298 (fires), 1728x1117 sits on the line
            // at 0.0200, and 1920x1080 gives 0.0186 and never fired at all. A large-display device
            // would never have shown this.
            // windowWatcher already spares this process; the verdict has to agree with it.
            //
            // Tested here, inside the disqualifying branch, so the process lookup only runs for a
            // window that would otherwise block — not for every window on every evaluation.
            if (blinkeredWindowOwnerIsOwnBannerHelper(ownerPID)) {
                continue;
            }
            blockedBy = owner;
            break;
...
{
    // ENFORCEMENT FIRST, REPORTING SECOND — and never the other way round.
    //
    // This used to compute the verdict first and only remedy when the verdict said "not enforcing",
    // which coupled the SELF-HEAL to the REPORT: every condition that silenced reporting also
    // stopped the kiosk retrying — screen locked, screensaver up, display asleep. Which is exactly
    // when a parent locks a Mac that is not being used. Field-reported on 3.6.204 and reproduced
    // from the logs.
    //
    // The remedy is rate-limited, is a no-op unless a foreign full-screen Space is actually current,
    // and is independently gated on -blinkeredKioskShouldOwnScreen. Reporting stays suppressed
    // while the screen is locked; ENFORCEMENT DOES NOT.
    //
    // The wake edge (-blinkeredScreensDidWake:) drives this too, and is the deterministic half:
    // NSTimers do not fire across system sleep, so the poll alone would land whenever the runloop
    // is coalesced back rather than at wake.
    NSString *suppression = nil;
    NSDictionary *verdict = [self blinkeredComputeEnforcementVerdictSuppression:&suppression];

    // Remedy with the REAL reason, so the remedy's own log line still discriminates why the kiosk
    // lost the screen. Passing a bare @"enforcement-tick" here made every firing on the busiest
    // call site say only "a timer fired" — undoing the diagnosability this same change adds.
    if (verdict && ![verdict[@"ok"] boolValue]) {
        [self blinkeredEnsureOrdinarySpace:(verdict[@"reason"] ?: @"not-enforcing")];
    } else if (!verdict) {
        // Suppressed: we cannot honestly REPORT, but we must still ACT.
        [self blinkeredEnsureOrdinarySpace:(suppression ?: @"suppressed")];
    }

    if (!verdict) {
        NSString *key = [NSString stringWithFormat:@"suppressed|%@", suppression ?: @"unknown"];
        if (![key isEqualToString:_blinkeredLastEnforcementKey]) {
            _blinkeredLastEnforcementKey = key;
            DDLogInfo(@"Blinkered: enforcement verdict suppressed (%@) — still remedying, not reporting", suppression);
        }
        return;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
                  ...
                  @"detectedProhibitedProcess"])
        {
            ...
            if (!self.sessionState.processesDetected) {
                ...
                // M-D: make this red screen visible to the server (identity by code signature only, no
                // name/path/bundle-id; throttled). demoted:NO — M-A adds the demotion path, and
                // killErrno/thrashCount arrive with it.
                pid_t offenderPID = 0;
                for (NSDictionary *p in allRunningProhibitedProcesses) {
                    if ([p[@"PID"] isKindOfClass:[NSNumber class]]) { offenderPID = [p[@"PID"] intValue]; break; }
                }
                BlinkeredReportLockdownAlert(@"prohibited_process", @"unkillable_system_process",
                                             offenderPID > 0 ? BlinkeredOffenderClass(offenderPID) : @"unknown",
                                             NO, -1 /*killErrno unknown here*/, NO, (NSInteger)[NSScreen screens].count, 0 /*not a thrash*/, 0 /*no ordinal*/);
...
- (BOOL) killApplication:(NSRunningApplication *)application
{
    ...
    if (!self.sessionState.processCheckAllOverride && ![self isOverriddenProhibitedProcess:processDetails]) {
        ...
        NSData *preKillCdhash = BlinkeredGuestCdhashForPid(appPID);   // M-A §3: capture BEFORE the kill
        NSError *killError = nil;
        BOOL killSuccess = [application killReturningError:&killError];
        if (!killSuccess) {
            int killErrno = [[killError.userInfo objectForKey:SEBErrorKillProcessErrnoKey] intValue];
            if (BlinkeredKillFailureIsUnkillableSystemProcess(appPID, preKillCdhash, killErrno)) {
                // M-A: an un-killable Apple SYSTEM process. Report it and DO NOT lock the child out.
                // Return YES ("handled") so it is neither queued to restart nor treated as a failure.
                DDLogError(@"M-A: demoting un-killable Apple system process (pid %d, errno %d) from the red screen to telemetry.", appPID, killErrno);
                BlinkeredReportLockdownAlert(@"prohibited_process", @"unkillable_system_process",
                                             BlinkeredOffenderClass(appPID), YES, killErrno, NO, (NSInteger)[NSScreen screens].count, 0 /*not a thrash*/, 0 /*no ordinal*/);
                return YES;
            }
...
- (NSError * _Nullable) killProcess:(NSDictionary *)processDictionary
{
    ...
    NSError *error = nil;
    if (!self.sessionState.processCheckAllOverride && ![self isOverriddenProhibitedProcess:processDetails]) {
        NSData *preKillCdhash = BlinkeredGuestCdhashForPid(processPID);   // M-A §3: capture BEFORE the kill
        // §3.6 thrash counting lives HERE, not at the call sites, for two reasons the review found
        // the hard way. (1) This is the only place the REAL outcome is known: killProcess: returns
        // nil both when the kill succeeded and when the override path skipped it entirely
        // (processCheckAllOverride / isOverriddenProhibitedProcess:), so a caller testing `error ==
        // nil` would fire a thrash about a process it never touched — and burn the session's one
        // report slot doing it. (2) Every kill site funnels through here, so there is ONE place to
        // get the pre-kill capture right instead of one per caller.
        //
        // Captured before the kill for the same reason as the cdhash above: a dead pid has no
        // executable path and no code signature.
        NSString *preKillPath = BlinkeredExecPathForPid(processPID);
        NSString *preKillClass = BlinkeredOffenderClass(processPID);
        ...
        if (killSuccess) {
            ...
            BlinkeredNoteKillForThrash(preKillPath, preKillClass);
            if (appURL) {
                [_terminatedProcessesExecutableURLs addObject:appURL];
            }
        } else {
            int killErrno = [[error.userInfo objectForKey:SEBErrorKillProcessErrnoKey] intValue];
            if (BlinkeredKillFailureIsUnkillableSystemProcess(processPID, preKillCdhash, killErrno)) {
                // M-A: an un-killable Apple SYSTEM process (e.g. WindowServer). The kill has already
                // failed; the red screen removes nothing and only locks a child out of their own
                // machine over a process they did not start. Report it and DO NOT lock.
                DDLogError(@"M-A: demoting un-killable Apple system process (pid %d, errno %d) from the red screen to telemetry.", processPID, killErrno);
                BlinkeredReportLockdownAlert(@"prohibited_process", @"unkillable_system_process",
                                             BlinkeredOffenderClass(processPID), YES, killErrno, NO, (NSInteger)[NSScreen screens].count, 0 /*not a thrash*/, 0 /*no ordinal*/);
            } else {
...
{
    DDLogDebug(@"%s", __FUNCTION__);
    // ── SESSION-STATE HALF. Everything below the split is idempotent surface re-assertion; these
    // two lines are not, which is why -blinkeredApplyKioskPresentationOptionsAllowingApps: exists
    // as a separate entry point for callers (the #100 Space remedy) that must re-assert surfaces
    // WITHOUT performing a lifecycle transition. See -blinkeredReassertKioskSurfaces.
    //
    ...
    [self blinkeredApplyKioskPresentationOptionsAllowingApps:allowSwitchToThirdPartyApps
                                         overrideShowMenuBar:overrideShowMenuBar];
}


// The SURFACES half of the kiosk mode: presentation options (menu bar / Apple menu / Dock), the
// menu-bar click shield, and the levels of any modal or About window on screen.
//
// Split out of -startKioskModeThirdPartyAppsAllowed: so the #100 Space remedy can re-apply the
// presentation options — the step it used to omit, which cost the Apple/File menus after locking
// over a full-screen app — without also performing that method's two SESSION-STATE side effects
// (clearing _blinkeredEarlyCoversActive and writing elevateWindowLevels). Those were what turned
// "just call the proven path" into a change that could unlock the kiosk during startup.
//
// Everything here is idempotent and reads no session config beyond showMenuBar, so it is safe to
// call at any point in a session, including before settings are applied.
- (void)blinkeredApplyKioskPresentationOptionsAllowingApps:(BOOL)allowSwitchToThirdPartyApps
                                       overrideShowMenuBar:(BOOL)overrideShowMenuBar
{
    NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];
...
// Re-assert the kiosk's SURFACES only: bring the app forward, put the covers back, and re-apply
// the presentation options that own the menu bar, the Apple menu and the Dock.
//
// WHAT THIS DELIBERATELY DOES NOT DO — and why it exists at all. The #100 Space remedy first tried
// three of these steps inline, drifted out of step with -reinforceKioskMode and lost the
// presentation-options re-apply (field-reported on 3.6.204: no Apple/File menus after locking over a
// full-screen Safari). The obvious repair — "just call -reinforceKioskMode" — is WORSE, and
// adversarial review caught it before it shipped:
//
//   -reinforceKioskMode -> -setElevateWindowLevels        re-reads SEB_allowSwitchToApplications
//                       -> -switchKioskModeAppsAllowed:   -> _blinkeredEarlyCoversActive = NO
//
// -blinkeredKioskShouldOwnScreen returns YES during early startup precisely so the remedy can run
// BEFORE the covers are created — i.e. before this session's config is applied. A remedy firing
// there would have read the PREVIOUS session's allowSwitchToApplications, and if that was YES it
// would have written elevateWindowLevels = NO, dropped every cap window to NSNormalWindowLevel,
// restored the Dock and the Apple menu, and cleared the early-covers flag — after which
// -blinkeredKioskShouldOwnScreen returns NO and the whole module is dead for the session.
// The remedy for a bypass would have released the lock and then disabled itself.
//
// So: surfaces are idempotent and safe to re-assert from anywhere, at any point in the session.
// SESSION-STATE TRANSITIONS ARE NOT, and stay in -reinforceKioskMode where the config is known to
// be applied. Anything added here must be safe to run before settings load.
- (void)blinkeredReassertKioskSurfaces
{
    [[NSRunningApplication currentApplication] activateWithOptions:(NSApplicationActivateAllWindows | NSApplicationActivateIgnoringOtherApps)];
    // Guarded on isVisible: a main window that is deliberately ordered out (mid session-reset, or
    // while the offline panel owns the screen) must not be dragged back on top of the covers.
    if (self.browserController.mainBrowserWindow.isVisible) {
        [self.browserController.mainBrowserWindow makeKeyAndOrderFront:self];
    }
    [self coverScreens];
    // The presentation options ARE the menu bar / Apple menu / Dock. Re-applying them is the step
    // the remedy used to omit. The surfaces-only entry point performs no lifecycle transition.
    [self blinkeredApplyKioskPresentationOptionsAllowingApps:_sessionState.allowSwitchToApplications
                                         overrideShowMenuBar:NO];
    if (NSApp.mainWindow) {
        NSDictionary *userInfo = @{
            NSAccessibilityUIElementsKey: @[NSApp.mainWindow],
            NSAccessibilityFocusedWindowAttribute: NSApp.mainWindow
...
- (void)reinforceKioskMode
{
    if (!self.settingsOpen) {
        ...
        if (_isAACEnabled == NO && _wasAACEnabled == NO) {
            ...
            // The SESSION-STATE half. -setElevateWindowLevels re-derives
            // _sessionState.allowSwitchToApplications from the persisted config and writes
            // elevateWindowLevels; -switchKioskModeAppsAllowed: hands leveling over to the loaded
            // settings and clears _blinkeredEarlyCoversActive. Both are correct HERE — a reinforce
            // is a session-lifecycle event and the config is applied by the time it runs — and both
            // are why the #100 Space remedy must NOT call this method. See
            // -blinkeredReassertKioskSurfaces below.
            [self setElevateWindowLevels];
            [self blinkeredReassertKioskSurfaces];
            [self switchKioskModeAppsAllowed:_sessionState.allowSwitchToApplications overrideShowMenuBar:NO];
            [self blinkeredReassertKioskSurfaces];
...
//
// On an agent-minted OFFLINE lock the signed config's start URL is
// `blinkered-offlinecover://<untilMs>`. Recovery must go where LAUNCH goes — launch itself uses
// -loadURL: (SEBOSXBrowserController.m:218) — so for those schemes it takes the same route.
//
// THE ORIGINAL REASON FOR THIS IS NO LONGER TRUE, and is corrected here rather than left to be
// re-derived. It used to read: load(_:) "intercepts and renders from a compiled-in string; a raw
// loadRequest: of that URL simply fails, so a wedged covered device would have stayed wedged."
// Since the cover became SERVED by a WKURLSchemeHandler instead of injected, a raw loadRequest: of
// the cover URL DOES render it — the handler answers, provided the URL is the configured start URL.
// So routing is no longer what makes the cover renderable. It is kept because recovery taking the
// same route as launch is the property this is for, and the one the gates pin.
//
...
// Write one marker dict to the crash-markers dir the agent flushes. The caller owns the ALLOWLIST —
// this only stamps the common fields and writes the file.
// Returns whether the marker REACHED DISK. Callers that dedupe on "we reported this" must key on
// that, not on "we tried": every error here used to be swallowed (error:nil twice, and the write's
// own BOOL discarded), so a full disk or an unwritable container looked exactly like a success.
static BOOL BlinkeredWriteMarkerDict(NSMutableDictionary *marker)
{
    ...
    marker[@"os"] = [NSString stringWithFormat:@"macOS %ld.%ld.%ld", (long)v.majorVersion, (long)v.minorVersion, (long)v.patchVersion];
    marker[@"at"] = @([[NSDate date] timeIntervalSince1970]);
    marker[@"attempts"] = @0;
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtURL:dir withIntermediateDirectories:YES attributes:nil error:nil]) {
        return NO;   // YES if it already exists, so this is a real failure
    }
    ...
    if (!data) { return NO; }
    return [data writeToURL:file atomically:YES];
}
...
// ── M-A: demote an un-killable Apple SYSTEM process from the red screen to telemetry ─────────────
// (docs/OS_UPDATE_RESILIENCE_PLAN.md §3). These are the sliceable core the kill-demotion probe
// compiles against real live PIDs (§3.8) — self-contained C functions, no SEBController state. The
// predicate + its Security.framework path were proven UNPRIVILEGED (uid 503) on a 26.6.2 device:
// WindowServer/launchd/coreaudiod DEMOTE; an own-uid process and a fabricated exited-PID KEEP.
//
// The `anchor apple`-only requirement — condition (2). ONE construction site (G-A pins this): the bare
// string, no `generic`, no MAS OID. NOT signedSystemExecutable:'s union — M-A answers "is this the OS
// itself?", a different, narrower question (§3.3a).
static SecRequirementRef BlinkeredAppleAnchorRequirement(void)
{
    static SecRequirementRef req = NULL;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ SecRequirementCreateWithString(CFSTR("anchor apple"), kSecCSDefaultFlags, &req); });
    return req;
}

// The cdhash (kSecCodeInfoUnique) of a dynamic guest code — condition (3)'s identity. Readable
// cross-uid via the guest SecCode → static code → signing information (verified on device).
static NSData *BlinkeredGuestCdhashFromCode(SecCodeRef code)
{
    SecStaticCodeRef staticCode = NULL;
    if (SecCodeCopyStaticCode(code, kSecCSDefaultFlags, &staticCode) != errSecSuccess || !staticCode) return nil;
    CFDictionaryRef info = NULL;
    OSStatus s = SecCodeCopySigningInformation(staticCode, kSecCSDefaultFlags, &info);
    CFRelease(staticCode);
    if (s != errSecSuccess || !info) return nil;
    NSData *cdhash = [(__bridge NSDictionary *)info objectForKey:(id)kSecCodeInfoUnique];
    NSData *copy = cdhash ? [cdhash copy] : nil;
    CFRelease(info);
    return copy;
}

// The caller uses THIS to capture the pre-kill cdhash BEFORE the kill, and passes it into the predicate.
static NSData *BlinkeredGuestCdhashForPid(pid_t pid)
{
    SecCodeRef code = NULL;
    if (SecCodeCopyGuestWithAttributes(NULL, (__bridge CFDictionaryRef)@{ (__bridge id)kSecGuestAttributePid: @(pid) }, kSecCSDefaultFlags, &code) != errSecSuccess || !code) return nil;
    NSData *cd = BlinkeredGuestCdhashFromCode(code);
    CFRelease(code);
    return cd;
}

// THE PREDICATE. Returns YES only when a FAILED kill was against an un-killable Apple system process
// that should be demoted from the red screen to telemetry. All three conditions must hold, every step
// must succeed; anything else is NO (fail-closed → the current kill-and-lock behaviour stands).
//   (1) killErrno == EPERM  — a non-admin child can only spawn same-uid (killable) processes, so a
//       child-controlled process never produces EPERM and never enters this predicate (§3.4 A1/A3).
//   (2) the live PID's dynamic guest SecCode satisfies `anchor apple` — "is this the OS?", forgeable
//       only with Apple's signing key. NO uid read (the v1 defect: uid is unreadable cross-uid).
//   (3) the cdhash re-read now equals the pre-kill cdhash — a recycled PID hosting a different binary
//       is caught (A6). EPERM guarantees this call did not itself free the PID.
static BOOL BlinkeredKillFailureIsUnkillableSystemProcess(pid_t pid, NSData *preKillCdhash, int killErrno)
{
    if (killErrno != EPERM) return NO;
    if (preKillCdhash.length == 0) return NO;
    SecCodeRef code = NULL;
    if (SecCodeCopyGuestWithAttributes(NULL, (__bridge CFDictionaryRef)@{ (__bridge id)kSecGuestAttributePid: @(pid) }, kSecCSDefaultFlags, &code) != errSecSuccess || !code) return NO;
    SecRequirementRef appleReq = BlinkeredAppleAnchorRequirement();
    BOOL demote = NO;
    if (appleReq && SecCodeCheckValidity(code, kSecCSDefaultFlags, appleReq) == errSecSuccess) {
        NSData *postKillCdhash = BlinkeredGuestCdhashFromCode(code);
        demote = (postKillCdhash.length > 0 && [postKillCdhash isEqualToData:preKillCdhash]);
    }
    CFRelease(code);
    return demote;
}

// ── M-D lockdown-alert telemetry (docs/OS_UPDATE_RESILIENCE_PLAN.md §6) ──────────────────────────
// Classify the offending process by its CODE SIGNATURE — never its name, bundle id, or path. This is
// the one identity signal that crosses the wire, and it cannot encode a child's name/document/habit.
// The dynamic guest SecCode is taken from the live PID (the same mechanism M-A uses); ordered most- to
// least-privileged so the first match wins.
static NSString *BlinkeredOffenderClass(pid_t pid)
{
    SecCodeRef code = NULL;
    NSDictionary *attrs = @{ (__bridge id)kSecGuestAttributePid: @(pid) };
    if (SecCodeCopyGuestWithAttributes(NULL, (__bridge CFDictionaryRef)attrs, kSecCSDefaultFlags, &code) != errSecSuccess || !code) {
        return @"unknown";
    }
    NSArray<NSArray<NSString *> *> *checks = @[
        @[ @"anchor apple", @"apple_platform" ],                                                            // the OS itself
        @[ @"anchor apple generic and certificate leaf[field.1.2.840.113635.100.6.1.9] exists", @"apple_mas" ],  // Mac App Store
        @[ @"anchor apple generic and certificate leaf[field.1.2.840.113635.100.6.1.13] exists", @"developer_id" ], // Developer ID
    ];
    NSString *result = nil;
    for (NSArray<NSString *> *c in checks) {
        SecRequirementRef req = NULL;
        if (SecRequirementCreateWithString((__bridge CFStringRef)c[0], kSecCSDefaultFlags, &req) == errSecSuccess && req) {
            OSStatus st = SecCodeCheckValidity(code, kSecCSDefaultFlags, req);   // dynamic guest code (M-A §3.2 uses default flags)
            CFRelease(req);
            if (st == errSecSuccess) { result = c[1]; break; }
        }
    }
    if (!result) {
        // Signed at all (ad-hoc / self-signed) vs unsigned.
        result = (SecCodeCheckValidity(code, kSecCSDefaultFlags, NULL) == errSecSuccess)
                 ? @"adhoc" : @"unsigned";
    }
    CFRelease(code);
    return result;
...
// Report one lockdown alert to the server via the marker rail. `detail` MUST already be enum/num/bool
// only — no names, paths, or bundle ids (the leak gate enforces this). Throttled to at most one marker
// per (alertType, signal) per 15 minutes so a 4 Hz thrash cannot fill the disk.
// ── M-A §3.6: THE SILENT THRASH ─────────────────────────────────────────────────────────────────
// The other half of the 26.6.2 incident, and the half that stayed invisible. Dock and Control Centre
// FAILED the signing check, were killed SUCCESSFULLY, respawned by launchd, and killed again — at
// 4 Hz, for as long as the device was locked. Nothing was posted, nothing logged at error level,
// nothing reached the server, because a SUCCESSFUL kill posts nothing. 3.6.209 stopped those two
// failing the check; it did not stop the CLASS. A new Apple overlay process, or a future
// signing-format change, reproduces it exactly.
//
// DETECT AND REPORT ONLY — the behaviour is unchanged. Auto-exempting a thrashing process was
// explicitly rejected by the plan: "killed it 5 times, stop trying" is a fail-open exemption any
// respawning app can buy, and a child could install a self-restarting login item to earn a permanent
// one. Nothing here may ever gate a kill.
//
// KEYED ON THE EXECUTABLE PATH, not the pid: launchd respawns the target under a NEW pid each time,
// so the pid is different on every tick and only the path is stable across a thrash.
static NSInteger const kBlinkeredThrashN = 5;          // kills of one key…
static NSTimeInterval const kBlinkeredThrashWindow = 60.0;   // …within this many seconds
static NSInteger const kBlinkeredThrashMaxKeys = 256;        // memory bound (see the eviction note)
static NSInteger const kBlinkeredThrashMaxOrdinal = 16;      // distinct server signatures per session
static NSTimeInterval const kBlinkeredThrashRetry = 60.0;    // per-key floor between report ATTEMPTS
static NSInteger const kBlinkeredThrashMaxReported = 1024;   // retired-key set, also bounded
static NSMutableDictionary<NSString *, NSMutableArray<NSNumber *> *> *blinkeredThrashHits = nil;
static NSMutableDictionary<NSString *, NSNumber *> *blinkeredThrashAttempted = nil;
static NSMutableDictionary<NSString *, NSNumber *> *blinkeredThrashOrdinal = nil;
static NSMutableSet<NSString *> *blinkeredThrashReported = nil;
static NSInteger blinkeredThrashNextOrdinal = 1;
static BOOL blinkeredThrashSaturated = NO;

// The executable path of a LIVE pid. MUST be called BEFORE the kill: a dead pid has no path, so a
// post-hoc read returns nil — and BlinkeredNoteKillForThrash then early-returns and counts NOTHING.
// (An earlier comment here claimed a post-hoc read would key every thrash under the empty string.
// That was wrong: the counter would go silent, not collapse keys. Same conclusion, wrong mechanism.)
static NSString *BlinkeredExecPathForPid(pid_t pid)
{
    if (pid <= 0) { return nil; }
...
// The local record of a thrash, to BOTH sinks — they fail in opposite conditions. See the call site.
static void BlinkeredThrashLog(NSInteger count, NSTimeInterval window)
{
    NSString *line = [NSString stringWithFormat:
        @"BLINKERED thrash: a process was force-terminated %ld times in %.0fs and keeps coming back "
         "(behaviour unchanged - it is still being killed)", (long)count, (double)window];
    // os_log survives a Release build with logging disabled; the file log is the half a child account
    // can actually READ (`log show` is refused without privileges). An explicit %{public}@ is required:
    // a runtime-built string handed to NSLog as the whole format reaches the device as "<private>".
    os_log(OS_LOG_DEFAULT, "%{public}@", line);
    DDLogWarn(@"%@", line);
}

// Note one SUCCESSFUL kill against the thrash counter.
//
// ONE ROW PER KEY PER SESSION — but only once a row has ACTUALLY REACHED DISK. The first version
// retired the key before asking, and the reporter's 15-minute throttle keys on
// alertType:signal:demoted, which is identical for every path. So a second thrashing process
// crossing the threshold inside that window retired its key, called a reporter that silently
// returned, and could never report again for the rest of the session. The incident this measure
// exists for had TWO thrashers — Dock and Control Centre, both at 4 Hz, seconds apart — so it would
// have reported exactly one of them, with nothing on the row saying a second existed. Note that
// scoping the throttle by offenderClass would NOT have fixed that: both of them are apple_platform.
// kill_thrash is therefore exempt from the shared floor (see the reporter), and rate-limited instead
// by its own two limiters, which are strictly stronger and do not conflate distinct offenders:
// one row per executable path per session, and kBlinkeredThrashRetry between attempts on one path.
static void BlinkeredNoteKillForThrash(NSString *execPath, NSString *offenderClass)
{
    if (execPath.length == 0) { return; }   // unreadable path: count nothing rather than key on ""
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        blinkeredThrashHits = [NSMutableDictionary dictionary];
        blinkeredThrashAttempted = [NSMutableDictionary dictionary];
        blinkeredThrashOrdinal = [NSMutableDictionary dictionary];
        blinkeredThrashReported = [NSMutableSet set];
    });
    NSInteger count = 0;
    NSInteger ordinal = 0;
    NSString *reportClass = nil;
    BOOL firstAttempt = NO;
    BOOL nowSaturated = NO;
    @synchronized (blinkeredThrashHits) {
        if ([blinkeredThrashReported containsObject:execPath]) { return; }
        NSTimeInterval now = [NSDate date].timeIntervalSince1970;
        NSMutableArray<NSNumber *> *hits = blinkeredThrashHits[execPath];
        if (!hits) {
            // BOUND THE TABLE. Pruning inside a key bounds that key's array; it never removes the key
            // itself, so a stream of distinct never-repeating paths grew the dictionary without limit
            // — measured at 200,000 keys / 26.6 MB with zero markers emitted. §3.6 concedes the key is
            // child-influenceable, which is exactly who would produce such a stream.
            if (blinkeredThrashHits.count >= kBlinkeredThrashMaxKeys) {
                NSMutableArray<NSString *> *dead = [NSMutableArray array];
                [blinkeredThrashHits enumerateKeysAndObjectsUsingBlock:^(NSString *k, NSMutableArray<NSNumber *> *v, BOOL *stop) {
                    if (v.count == 0 || now - v.lastObject.doubleValue > kBlinkeredThrashWindow) { [dead addObject:k]; }
                }];
                [blinkeredThrashHits removeObjectsForKeys:dead];
                [blinkeredThrashAttempted removeObjectsForKeys:dead];
                // Still full of LIVE keys? EVICT THE LEAST-RECENTLY-HIT ONE — never refuse the new one.
                // Refusing (the previous shape) meant a genuinely thrashing process was not counted at
                // all whenever 256 other paths happened to be active, and nothing recorded that it had
                // been dropped: a fail-silent in the measure whose whole purpose is to stop failures
                // being silent, and one an adversarial key-stream could aim. Evicting instead keeps the
                // bound while always admitting the newest evidence.
                if (blinkeredThrashHits.count >= kBlinkeredThrashMaxKeys) {
                    NSString *coldest = nil;
                    NSTimeInterval coldestAt = INFINITY;
                    for (NSString *k in blinkeredThrashHits) {
                        NSTimeInterval t = blinkeredThrashHits[k].lastObject.doubleValue;
                        if (t < coldestAt) { coldestAt = t; coldest = k; }
                    }
                    if (coldest) {
                        [blinkeredThrashHits removeObjectForKey:coldest];
                        [blinkeredThrashAttempted removeObjectForKey:coldest];
                    }
                    if (!blinkeredThrashSaturated) { blinkeredThrashSaturated = YES; nowSaturated = YES; }
                }
            }
            hits = [NSMutableArray array];
            blinkeredThrashHits[execPath] = hits;
        }
        [hits addObject:@(now)];
        NSUInteger firstInWindow = 0;
        while (firstInWindow < hits.count && now - hits[firstInWindow].doubleValue > kBlinkeredThrashWindow) {
            firstInWindow++;
        }
        if (firstInWindow > 0) { [hits removeObjectsInRange:NSMakeRange(0, firstInWindow)]; }
        if ((NSInteger)hits.count < kBlinkeredThrashN) { return; }
        // A key past the threshold is re-entered on EVERY subsequent tick (4 Hz) until a row reaches
        // disk, so the attempt — and the local log line with it — must be floored per key. Without
        // this a thrasher whose write kept failing logged ~3,600 lines over a 15-minute window.
        NSNumber *lastAttempt = blinkeredThrashAttempted[execPath];
        if (lastAttempt && now - lastAttempt.doubleValue < kBlinkeredThrashRetry) { return; }
        firstAttempt = (lastAttempt == nil);
        blinkeredThrashAttempted[execPath] = @(now);
        // The offender's ORDINAL for this session — see the reporter for why the row is invisible
        // without it. Sticky per path, so a retry after a failed write reuses the same signature
        // rather than burning a second one. Allocation STOPS at the cap and the table therefore holds
        // at most kBlinkeredThrashMaxOrdinal entries: paths past the cap share the last ordinal and so
        // dedupe together, which is this measure's own founding defect reappearing beyond the cap —
        // so the cap must sit ABOVE any plausible real thrash, not merely at a comfortable number.
        // 8 was chosen for headroom that was never measured. The quota only becomes reachable around
        // 29 distinct signatures (the client can mint at most cap + 2 M-A signatures, and inserts are
        // gated by the same dedup), while a platform-wide signing break kills every com.apple.*
        // process that owns a window — Dock, ControlCenter, SystemUIServer, NotificationCenter, the
        // text-input and menu-extra agents — and every launchd-respawned one of those reaches N=5.
        // Ordinals are also burned at ATTEMPT time, so failed writes spend them. 16 clears the
        // realistic population with ~13 signatures of quota still spare.
        NSNumber *ord = blinkeredThrashOrdinal[execPath];
        if (!ord) {
            if (blinkeredThrashNextOrdinal <= kBlinkeredThrashMaxOrdinal) {
                ord = @(blinkeredThrashNextOrdinal++);
                blinkeredThrashOrdinal[execPath] = ord;
            } else {
                ord = @(kBlinkeredThrashMaxOrdinal);
            }
        }
        ordinal = ord.integerValue;
        count = (NSInteger)hits.count;
        reportClass = offenderClass.length ? offenderClass : @"unknown";
    }
    // Reported OUTSIDE the lock: the reporter takes its own token and does file I/O.
    //
    // BOTH SINKS, DELIBERATELY. The os_log line survives a Release build with logging disabled, where
    // DDOSLogger is compiled out (#ifdef DEBUG) and the file logger is removed. The DDLog line is the
    // one that is actually READABLE on a locked-down device: `log show` needs privileges the child
    // account does not have and returns "Operation not permitted", while the CocoaLumberjack file log under
    // ~/Library/Logs/Blinkered is world-readable and was confirmed live on the test Mac. Neither sink
    // alone covers both cases, which is why BlinkeredThrashLog writes to each.
    //
    // ONE LINE PER KEY PER SESSION: only on the first attempt for this path.
    if (nowSaturated) {
        // Local only. `signal` is a server-side ALLOW-LIST (crashReport.js sanitizeLockdownDetail) and
        // unlisted detail keys are dropped, so a new signal would land as null until seb-classroom
        // ships the enum value first. Saturation needs 256 distinct executables force-killed inside
        // 60 s — adversarial-only — and with eviction it no longer suppresses anything, so it did not
        // justify a cross-repo change. Promoting it to a real signal is a one-line server addition.
        os_log(OS_LOG_DEFAULT, "%{public}@", @"BLINKERED thrash: key table saturated - evicting least-recently-hit paths");
        DDLogWarn(@"BLINKERED thrash: key table saturated - evicting least-recently-hit paths");
    }
    if (firstAttempt) { BlinkeredThrashLog(count, kBlinkeredThrashWindow); }
    BOOL wrote = BlinkeredReportLockdownAlert(@"prohibited_process", @"kill_thrash", reportClass,
                                              NO, -1, NO, (NSInteger)[NSScreen screens].count, count, ordinal);
    // RETIRE ONLY ON A ROW THAT REACHED DISK. BlinkeredWriteMarkerDict now propagates that; it used to
    // swallow a failed mkdir, a failed serialisation and the write's own BOOL, so "wrote" meant no more
    // than "was not throttled" and a full disk retired the key exactly as a success did.
    if (wrote) {
        @synchronized (blinkeredThrashHits) {
            // BOUND THIS TOO. It is the last table in this family that grew without limit: one entry
            // per path that has successfully reported, never pruned. It is far harder to drive than
            // the 200k-key case fixed earlier (every entry costs a successful marker write), but it
            // is the same shape. Evicting an arbitrary member costs at most one duplicate marker for
            // that path, which the server's six-hour dedup on the signature absorbs.
            if (blinkeredThrashReported.count >= kBlinkeredThrashMaxReported) {
                [blinkeredThrashReported removeObject:[blinkeredThrashReported anyObject]];
            }
            [blinkeredThrashReported addObject:execPath];
            [blinkeredThrashHits removeObjectForKey:execPath];
            [blinkeredThrashAttempted removeObjectForKey:execPath];
            // blinkeredThrashOrdinal is deliberately NOT cleared: the ordinal is this path's identity
            // for the session, and releasing it would let a later path reuse the signature.
        }
    }
}
...
static BOOL BlinkeredReportLockdownAlert(NSString *alertType, NSString *signal, NSString *offenderClass,
                                         ...
                                         NSInteger thrashCount, NSInteger offenderOrdinal)
{
    static NSMutableDictionary<NSString *, NSDate *> *lastReport = nil;
    static dispatch_once_t once; dispatch_once(&once, ^{ lastReport = [NSMutableDictionary dictionary]; });
    // Key on (alertType, signal, demoted) so a demoted:true→false M-A regression is not swallowed by
    // the 15-minute throttle (it is the exact transition this telemetry exists to catch).
    //
    // kill_thrash IS EXEMPT. Its key carries nothing that distinguishes one offender from another, so
    // the floor let a boring thrasher — a third-party respawner firing in the first seconds of a
    // session — silence the 26.6.2-class case for the following 15 minutes, which is the most
    // diagnostic window there is. Scoping the key by offenderClass would not have helped: Dock and
    // Control Centre are both apple_platform. The counter carries its own limiters instead (one row
    // per executable path per session, plus a per-path retry floor), and they are strictly stronger
    // than this one because they are keyed on the offender rather than on the signal name.
    BOOL sharedFloorApplies = ![signal isEqualToString:@"kill_thrash"];
    NSString *key = [NSString stringWithFormat:@"%@:%@:%d", alertType, signal, demoted ? 1 : 0];
    if (sharedFloorApplies) {
        @synchronized (lastReport) {
            NSDate *prev = lastReport[key];
            if (prev && [[NSDate date] timeIntervalSinceDate:prev] < 15 * 60) return NO;   // 15-minute floor per key
            lastReport[key] = [NSDate date];
        }
    }
    // The discriminator (signal AND demoted) rides in `method` so the server's signature separates
    // distinct events and a demoted:true→false M-A regression is not deduped away (plan N6).
    NSString *method = [NSString stringWithFormat:@"%@:%@:demoted=%d", alertType, signal, demoted ? 1 : 0];
    // THE OFFENDER DISCRIMINATOR, and the reason this measure did not work end to end without it.
    // The server stores one row per (device, sha1(exceptionType|method)) per SIX HOURS, and for a
    // thrash both of those inputs were constant: the executable path is deliberately never on the row
    // and offenderClass rides in `detail`, which the signature does not read. So every thrashing path
    // on every device produced ONE signature, and the second thrasher's marker was accepted, answered
    // 200, deleted by the agent and never inserted. Removing the client-side floor (review 1) only
    // moved that silence downstream. An ORDINAL — "the Nth distinct thrashing path this session" —
    // separates them while carrying nothing about the offender: unlike a hash of the path it needs no
    // salt and cannot be brute-forced back to a path, which for a child-spawned binary can contain a
    // username. It is capped, because once signatures differ they stop deduping and start consuming
    // the 30/device/hour lockdown_alert budget that M-A's red-screen row also draws on.
    if (offenderOrdinal > 0) {
        // The capped value is rendered "16+", not "16", so a reader can tell the sixteenth thrasher
        // from "sixteen or more" — everything at or past the cap shares this ordinal and therefore
        // this signature, and without the marker that collapse is invisible on the row.
        method = (offenderOrdinal >= kBlinkeredThrashMaxOrdinal)
            ? [method stringByAppendingFormat:@":offender=%ld+", (long)kBlinkeredThrashMaxOrdinal]
            : [method stringByAppendingFormat:@":offender=%ld", (long)offenderOrdinal];
    }
    NSMutableDictionary *detail = [@{
        @"alertType":     alertType,
        @"signal":        signal,
        @"offenderClass": offenderClass ?: @"unknown",
        @"demoted":       @(demoted),
        @"firstScan":     @(firstScan),
        @"screenCount":   @(MAX((NSInteger)1, screenCount)),
    } mutableCopy];
    if (killErrno >= 0) detail[@"killErrno"] = @(killErrno);   // absent (-1) on the pre-M-A red-screen path
    // Absent on every signal but kill_thrash, so a null here means "not a thrash report" rather than
    // "a thrash of zero" — the server clamps 0..1000 and stores null when the key is missing.
    if (thrashCount > 0) detail[@"thrashCount"] = @(thrashCount);
    return BlinkeredWriteMarkerDict([@{
        @"exceptionType": @"LockdownAlert",
        @"method":        method,
        @"stack":         @[],
        @"phase":         @"lockdown",
        @"kind":          @"lockdown_alert",
...
// ── M-C signing self-check (docs/OS_UPDATE_RESILIENCE_PLAN.md §5) ─────────────────────────────────
// A CANARY, never an actor: it evaluates the "is this Apple system software?" requirement against
// known-good controls and reports if the predicate looks BROKEN (a system binary fails — the 26.6.2
// incident's signature) or OVERBROAD (Blinkered's own binary passes — a security signal). Its result
// is read by NOTHING but the reporter below — no kill site, no signedSystemExecutable: branch, no lock
// decision. That fail-closed rule is the round-1 ruling this section preserves.
//
// The live-population counters (blinkeredSigningLivePasses/Failures) are defined earlier in the file,
// before windowWatcher which increments them (plan N8). THE union string is duplicated from
// signedSystemExecutable: deliberately — that method's copy may not
// leave its body (the M-B gate pins it there), and the G6 gate asserts THIS copy is byte-identical so a
// drift is caught. Keep the two literals identical.
static NSString * const kBlinkeredAppleSoftwareRequirement =
    @"anchor apple or (anchor apple generic and certificate leaf[field.1.2.840.113635.100.6.1.9] exists)";

// Report one signing-self-check finding via the marker rail (kind:'signing_selfcheck'). detail is
// enum/numeric only. method carries the signal so broken vs overbroad get distinct signatures (N6-style).
static void BlinkeredReportSigningSelfcheck(NSString *signal, NSString *source, long passes, long failures)
{
    BlinkeredWriteMarkerDict([@{
        @"exceptionType": @"SigningSelfcheck",
        @"method":        [NSString stringWithFormat:@"SigningSelfcheck:%@", signal],
        @"stack":         @[],
        @"phase":         @"signing-selfcheck",
        @"kind":          @"signing_selfcheck",
        @"detail":        @{
            @"signal":       signal,
            @"source":       source,
            @"livePasses":   @(MAX(0L, passes)),
            @"liveFailures": @(MAX(0L, failures)),
        },
    } mutableCopy]);
}

// Static-path evaluation, matching signedSystemExecutable:'s flags (kSecCSCheckAllArchitectures) so
// this is a canary for the SHIPPED check, not a different one (plan N8). Returns YES only on a clean pass.
static BOOL BlinkeredStaticPathSatisfies(NSString *path, SecRequirementRef req)
{
    if (!req || ![[NSFileManager defaultManager] fileExistsAtPath:path]) return NO;
    SecStaticCodeRef code = NULL;
    NSURL *url = [NSURL fileURLWithPath:path];
    if (SecStaticCodeCreateWithPath((__bridge CFURLRef)url, kSecCSDefaultFlags, &code) != errSecSuccess || !code) return NO;
    OSStatus st = SecStaticCodeCheckValidity(code, kSecCSCheckAllArchitectures, req);
    CFRelease(code);
    return st == errSecSuccess;
}

// The one-shot static self-check: runs once per session on a background queue. Positive control — an
// Apple system binary must PASS the shipped union; if EVERY present candidate fails, the predicate is
// broken. Negative control — Blinkered's own binary must FAIL the tight `anchor apple` (not the union:
// a hypothetical Mac-App-Store Blinkered would satisfy the union's MAS disjunct, plan Q4); if it passes,
// the predicate is overbroad. Reports only; never acts.
static void BlinkeredRunSigningSelfCheckOnce(void)
{
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
            SecRequirementRef unionReq = NULL, appleOnlyReq = NULL;
            SecRequirementCreateWithString((__bridge CFStringRef)kBlinkeredAppleSoftwareRequirement, kSecCSDefaultFlags, &unionReq);
            SecRequirementCreateWithString(CFSTR("anchor apple"), kSecCSDefaultFlags, &appleOnlyReq);

            // Positive control: ordered candidates, checked only if present. INCONCLUSIVE (report
            // nothing) if none is on disk — a hard-coded path must not itself generate false alarms.
            NSArray<NSString *> *positives = @[ @"/usr/bin/true", @"/bin/sh",
                                                @"/System/Library/CoreServices/Finder.app" ];
            BOOL anyPresent = NO, anyPassed = NO;
            for (NSString *p in positives) {
                if (![[NSFileManager defaultManager] fileExistsAtPath:p]) continue;
                anyPresent = YES;
                if (BlinkeredStaticPathSatisfies(p, unionReq)) { anyPassed = YES; break; }
            }
            if (anyPresent && !anyPassed) {
                DDLogError(@"M-C signing self-check: every present system-binary control FAILED the requirement — predicate looks BROKEN.");
                BlinkeredReportSigningSelfcheck(@"system_signing_broken", @"static_control", 0, 0);
            }

            // Negative control: Blinkered's own executable must FAIL `anchor apple`.
            NSString *ownPath = [NSBundle mainBundle].executableURL.path;
            if (ownPath && BlinkeredStaticPathSatisfies(ownPath, appleOnlyReq)) {
                DDLogError(@"M-C signing self-check: Blinkered's OWN binary satisfied 'anchor apple' — predicate is OVERBROAD (security signal).");
                BlinkeredReportSigningSelfcheck(@"system_signing_overbroad", @"static_control", 0, 0);
            }

            if (unionReq) CFRelease(unionReq);
            if (appleOnlyReq) CFRelease(appleOnlyReq);
        });
    });
}

// Evaluated from windowWatcher after it classifies a com.apple.* window: once there is a clear
// live-population signal (some failed, NONE passed), report it once. Cheap integer test, safe at 4 Hz.
static void BlinkeredMaybeReportLivePopulation(void)
{
    static dispatch_once_t reportedOnce;
    long passes = atomic_load(&blinkeredSigningLivePasses);
    long failures = atomic_load(&blinkeredSigningLiveFailures);
    if (failures > 0 && passes == 0 && (failures) >= 3) {
        dispatch_once(&reportedOnce, ^{
            DDLogError(@"M-C signing self-check: %ld com.apple.* windows classified this session, all FAILED, none passed — predicate looks BROKEN (live population).", failures);
            BlinkeredReportSigningSelfcheck(@"system_signing_broken", @"live_population", passes, failures);
        });
    }
}
...
- (void) applicationWillTerminateProceed
{
    ...
    // Give the child their wallpaper back. HERE, in the restore cluster, and not in
    // blinkeredAtomicVisualTeardown: that method is deliberately only the visual cut, and this is a
    // system setting, exactly like restoreScreenCapture and restoreSystemSettings below. Idempotent
    // and a no-op when no tint was applied, so the ordinary unlock (which already cleared it via the
    // page's clearDesktopTint) costs nothing, and a lock that ended any other way is still put right.
    [self blinkeredRestoreDesktopWallpaper];
