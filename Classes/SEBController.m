                  ...
                  @"detectedProhibitedProcess"])
        {
            ...
            if (!self.sessionState.processesDetected) {
                ...
                BlinkeredReportLockdownAlert(@"prohibited_process", @"unkillable_system_process",
                                             ...
                                             NO, NO, (NSInteger)[NSScreen screens].count);
...
static void BlinkeredReportLockdownAlert(NSString *alertType, NSString *signal, NSString *offenderClass,
                                         BOOL demoted, BOOL firstScan, NSInteger screenCount)
{
    ...
    NSString *key = [NSString stringWithFormat:@"%@:%@", alertType, signal];
    @synchronized (lastReport) {
        NSDate *prev = lastReport[key];
        if (prev && [[NSDate date] timeIntervalSinceDate:prev] < 15 * 60) return;   // 15-minute floor per (alertType, signal)
