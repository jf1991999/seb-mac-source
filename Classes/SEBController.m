...
// BEGIN blinkeredHomeRetryGates
    ...
    // guard to build.
    //
    // Fix 2 also does NOT dismiss the panel on success. Dismissal is a UX repair, not a correctness
    // requirement — the device is recovered either way and a parent returning to a stale panel can
    // press either button without harm — and it is hazardous: -abortModal ends the INNERMOST modal
    // session, which after "Enter Exit Code…" is the dialog the parent is typing their Master Exit
    // Code into. Executed, not reasoned about. Adding it later means adding all three of
    // (a) dismiss only when NSApp.modalWindow == the panel alert's own window, (b) deliver it on a
    // modal-mode run-loop callback (this method already is one), and (c) hold that NSAlert in state
    // this method can reach — none of which exists today, so no half of it can be added alone.
