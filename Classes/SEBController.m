...
- (void)blinkeredScheduleHomeSessionSanityCheck
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(60 * NSEC_PER_SEC)),
        ...
        if (!strongSelf || [strongSelf blinkeredHomeSessionInfo] != nil) {
            return;
        }
        DDLogError(@"Blinkered SECURITY: home lock session running with NO home_session.json a minute after "
                   @"launch — the start-URL write did not happen. Offline re-lock and native-quit reporting "
                   @"are degraded for this session.");
        ...
        [strongSelf blinkeredReportSecurityEvent:@"home_session_missing"];
    });
}
...
- (void)blinkeredReportSecurityEvent:(NSString *)eventType {
    ...
    DDLogInfo(@"Blinkered: reported %@ to server", eventType);
}
