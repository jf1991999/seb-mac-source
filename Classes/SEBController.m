...
// that swallow every click on the macOS menu bar while an elevated (locked) session shows it.
// The bar stays visible (clock, Wi-Fi, battery) but nothing up there is interactive:
// third-party status items (clicking the Microsoft Defender shield wedged a locked MacBook,
// 11 Aug 2026), the Wi-Fi toggle (induced-offline attack — OFFLINE_UNLOCK review §B.5),
// Control Center and the app menus are all inert. The mechanism is the one the exam covers
// already rely on (an elevated window over the strip receives the clicks instead of the
// status items); this is that cover, transparent and menu-bar-sized.
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    if (_openingSettings || _openedURL) {
        if (_blinkeredSavePair) {
            ...
            // blinkeredInstallLegacyLaunchAgent always does unload+load (a real restart) — NOT the
            // idempotent health-check, which would no-op a "running but stale" agent. So the very next
            // poll runs on fresh creds with a clean state and picks up any pending lock.
...
- (void)blinkeredRefreshAgentIfPaired {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:agentJson]) {
        ...
        if ([[self blinkeredPersistedMode] isEqualToString:@"cooperative"]) {
            [self blinkeredEnforceCooperativeNoRoot];
        } else {
            [self blinkeredRefreshUpdaterDaemonPlistIfNeeded];
        }
    }
}

// After an app update the updater daemon's launchd registration still uses the OLD bundled plist until it
// is re-registered — so plist changes in a new build (WatchPaths, StartInterval) wouldn't take effect. Once
// per app version, if the daemon is already Enabled, re-register it so the new plist is adopted. register()
// on an already-approved service is idempotent and does not re-prompt the user.
- (void)blinkeredRefreshUpdaterDaemonPlistIfNeeded {
    if (@available(macOS 13.0, *)) {
        SMAppService *svc = [SMAppService daemonServiceWithPlistName:@"app.blinkered.updater.plist"];
        if (svc.status != SMAppServiceStatusEnabled) return;   // not enabled yet — the setup card handles that
        NSString *ver = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"] ?: @"?";
        NSString *key = [@"blinkeredUpdaterPlistRefreshed_" stringByAppendingString:ver];
        if ([[NSUserDefaults standardUserDefaults] boolForKey:key]) return;   // already refreshed this build
        NSError *err = nil;
        [svc registerAndReturnError:&err];
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:key];
        DDLogInfo(@"Blinkered: refreshed updater daemon registration for build %@ (err=%@)", ver, err.localizedDescription);
    }
}
...
- (void)blinkeredEnforceCooperativeNoRoot {
    if (@available(macOS 13.0, *)) {
        ...
        // REPORTS it as Enabled and unregisterAndReturnError: would target it and FAIL ("lacks required
        // entitlement" — you can't unregister a /Library/LaunchDaemons job via SMAppService). Verified via
        // E2E on a pkg install. So if the pkg plist exists, this is an Enforced device: skip entirely. (An
...
- (void)blinkeredSiteWindowBecameKey:(NSNotification *)note {
    ...
    if (mainAbstract && [mainAbstract respondsToSelector:@selector(nativeWebView)]) {
        ...
        if ([mwk isKindOfClass:NSClassFromString(@"WKWebView")]) {
            NSString *urlEsc = [[url stringByReplacingOccurrencesOfString:@"\\" withString:@"\\\\"]
                                 stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"];
            NSString *js = [NSString stringWithFormat:@"window.__blinkeredWindowFocused && window.__blinkeredWindowFocused('%@')", urlEsc];
...
- (void)blinkeredPeriodicPaintCheck:(NSTimer *)t { [self blinkeredCheckPaintForTrigger:@"periodic"]; }
...
- (void)blinkeredReportPaint:(NSString *)trigger frame:(BPRFrame)f
    ...
    if (![devId isKindOfClass:[NSString class]] || ![devTok isKindOfClass:[NSString class]]) return;
    NSTimeInterval nowT = [NSDate timeIntervalSinceReferenceDate];
    BOOL displaySlept = (nowT - _blinkeredLastWakeAt) < 5.0;
    ...
    NSDictionary *body = @{ @"token": devTok, @"kind": @"paint_fault",
        ...
        @"rungRecovered": @(rung), @"recovered": @(recovered), @"displaySlept": @(displaySlept),
...
- (void)blinkeredWakeEdgeProbeWithWakeEpochMs:(double)wakeEpochMs startedAt:(NSTimeInterval)startedAt
    ...
    void (^judge)(void) = ^{
        ...
        BOOL reloadRan = NO;
        if (wedged && !_blinkeredOfflinePanelShowing && _bweSessionReloads < kBWEMaxReloadsSession) {
            ...
            NSURL *target = wk.URL;
            if (!target.absoluteString.length) {
                NSString *s = [[NSUserDefaults standardUserDefaults] secureStringForKey:@"org_safeexambrowser_SEB_startURL"];
                target = s.length ? [NSURL URLWithString:s] : nil;
            }
            if (target) {
                _bweSessionReloads++;
                reloadRan = YES;
...
- (void)blinkeredHandleSavePair:(NSURL *)url {
    ...
    // when the pkg daemon is present we're Enforced and must NOT touch it (blinkeredEnforceCooperativeNoRoot
...
// Earlier builds dropped the agent plist into ~/Library/LaunchAgents and loaded it with
// launchctl. macOS attributed that to the signing developer ("James Robert Fisher"). We now
// register the agent bundled INSIDE the app via SMAppService (attributed to "Blinkered"), so
// the legacy copy must be unloaded + deleted on first run of the new build — otherwise BOTH
// agents run and could double-launch a locked session.
- (void)blinkeredRemoveLegacyLaunchAgent {
    NSString *plistPath = [self blinkeredLegacyLaunchAgentPath];
    if (![[NSFileManager defaultManager] fileExistsAtPath:plistPath]) { return; }
    NSTask *unload = [NSTask new];
    unload.launchPath = @"/bin/launchctl";
    unload.arguments = @[@"unload", plistPath];
    @try { [unload launch]; [unload waitUntilExit]; }
    @catch (__unused NSException *e) {}
    [[NSFileManager defaultManager] removeItemAtPath:plistPath error:nil];
    DDLogInfo(@"Blinkered: removed legacy LaunchAgent at %@", plistPath);
}
...
//
// Idempotent: if the plist is present AND launchd already has the agent loaded, this no-ops — so
...
- (void)blinkeredInstallLaunchAgentReportRepair:(BOOL)reportRepair {
    ...
    if (plistPresent && wasRunning && logCurrent) {
        DDLogVerbose(@"Blinkered: LaunchAgent installed, running, log-path current — no-op");
        return;   // healthy — nothing to do (and don't restart a live agent mid-lock)
    }
    // (Re)install. On the first run of this build the plist is absent (older builds used SMAppService
    // and deleted the legacy copy), so this migration installs it and retires SMAppService below. It
    // also fires when the agent is registered-but-dead, or still on the old /tmp log path.
    DDLogInfo(@"Blinkered: installing/repairing LaunchAgent (plist:%d running:%d logCurrent:%d)", plistPresent, wasRunning, logCurrent);
    [self blinkeredInstallLegacyLaunchAgent];
    BOOL nowRunning = [self blinkeredAgentRunningWithin:5.0];
    DDLogInfo(@"Blinkered: LaunchAgent (re)install complete — running=%d", nowRunning);
    // Repair report: the device was paired and the agent had genuinely gone missing, and we just
    // brought it back. Fire once (server dedupes) so a silently-unlockable stretch is visible.
...
- (BOOL)blinkeredAgentRunningWithin:(NSTimeInterval)timeout {
    ...
    do {
        NSTask *task = [NSTask new];
        task.launchPath = @"/bin/launchctl";
        task.arguments = @[@"list", @"app.blinkered.agent"];
        NSPipe *pipe = [NSPipe pipe];
        task.standardOutput = pipe;
        task.standardError = [NSPipe pipe];   // discard
        @try {
            ...
            NSData *data = [[pipe fileHandleForReading] readDataToEndOfFile];
            NSString *output = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
            // `launchctl list <label>` prints a `"PID" = 1234;` line only while the job is alive.
            if (output && [output rangeOfString:@"\"PID\""].location != NSNotFound) return YES;
        } @catch (__unused NSException *e) {}
...
// Fallback for macOS < 13 (no SMAppService) or if registration fails: the original
// ~/Library/LaunchAgents install loaded via launchctl.
- (void)blinkeredInstallLegacyLaunchAgent {
    NSFileManager *fm = [NSFileManager defaultManager];
    NSString *agentBinary = [[NSBundle mainBundle].bundlePath
        stringByAppendingPathComponent:@"Contents/MacOS/BlinkeredAgent"];

    // Retire any SMAppService registration from an earlier build BEFORE loading the LaunchAgent, so
    // the two mechanisms never both run (double agent = double-launched locks). Idempotent: no-op
    // when nothing is registered.
    if (@available(macOS 13.0, *)) {
        SMAppService *svc = [SMAppService agentServiceWithPlistName:@"app.blinkered.agent.plist"];
        if (svc.status == SMAppServiceStatusEnabled) {
            [svc unregisterAndReturnError:nil];
            DDLogInfo(@"Blinkered: retired SMAppService registration in favour of the durable LaunchAgent");
        }
    }

    NSArray *libraryDirs = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
    NSString *launchAgentDir = [[libraryDirs firstObject] stringByAppendingPathComponent:@"LaunchAgents"];
  ...
  withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    // Unload any running instance, then load the updated plist.
    NSTask *unload = [NSTask new];
    unload.launchPath = @"/bin/launchctl";
    unload.arguments = @[@"unload", plistPath];
    [unload launch]; [unload waitUntilExit];

    NSTask *load = [NSTask new];
    load.launchPath = @"/bin/launchctl";
    load.arguments = @[@"load", plistPath];
    [load launch]; [load waitUntilExit];
    DDLogInfo(@"Blinkered: legacy LaunchAgent installed and started from %@", plistPath);
}
...
- (void)blinkeredLayoutMenuBarShieldWindows
{
    ...
    // Frame each shield over its screen's menu-bar strip, leveled above the status items
    // (status-item windows sit at NSStatusWindowLevel = NSMainMenuWindowLevel+1; +2 matches
    // the cap windows). setFrame: takes global coordinates, so derive from screen.frame.
...
- (IBAction) requestedQuit:(id)sender
{
    ...
    NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];
    // Check for a per-class quit hash written by the /seb-setquit URL interceptor.
    ...
    if ([preferences secureBoolForKey:@"org_safeexambrowser_SEB_allowQuit"] == YES || noActiveClass) {
        ...
        if (![hashedQuitPassword isEqualToString:@""]) {
            ...
            if (!passwordMatches && isHomeQuit && hashedQuitPassword) {
                ...
                NSString *normalised = [[[password componentsSeparatedByCharactersInSet:separators]
                                         componentsJoinedByString:@""] uppercaseString];
                passwordMatches = [hashedQuitPassword caseInsensitiveCompare:[keychainManager generateSHAHashString:normalised]] == NSOrderedSame;
            }
            if (passwordMatches) {
                // if the correct quit password was entered
                DDLogInfo(@"Correct quit password entered");
...
- (void)notifyServerQuitWithCompletion:(void (^)(void))completion
{
    ...
    if (homeData) {
        ...
        if (deviceId.length > 0 && token.length > 0 && base.length > 0) {
            ...
            NSString *quitSessionId = [homeInfo[@"sessionId"] isKindOfClass:[NSString class]] ? homeInfo[@"sessionId"] : nil;
            NSString *quitMethod = _blinkeredQuitMethod ?: @"master_code";
            ...
            request.HTTPBody = [[NSString stringWithFormat:@"{\"token\":\"%@\"}", token] dataUsingEncoding:NSUTF8StringEncoding];
            [request setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
            DDLogInfo(@"Blinkered: notifying server of home session quit (device %@)", deviceId);
              ...
              completionHandler:^(NSData *d, NSURLResponse *resp, NSError *err) {
                ...
                if (delivered) {
                    ...
                    if (quitSessionId.length > 0) {
                        ...
                        // the code itself.
                        NSString *proof = [self blinkeredExitProofForCode:quitTypedCode session:quitSessionId];
                        NSMutableDictionary *marker = [@{ @"sessionId": quitSessionId,
                                                          @"exitedAt": @((long long)([NSDate date].timeIntervalSince1970 * 1000)),
                                                          @"method": quitMethod } mutableCopy];
...
// [R6] HMAC-SHA256(key: the normalised typed code, message: sessionId), lowercase hex.
// Normalisation must match BOTH the dialog's own comparison (hyphens/whitespace stripped, uppercased —
// the master code is DISPLAYED as XXXX-XXXX-XXXX, so parents type the hyphens) and the server's, or a
// genuine exit produces a digest the server can never reproduce and the parent is refused.
- (NSString *)blinkeredExitProofForCode:(NSString *)code session:(NSString *)sessionId
{
    ...
    [strip addCharactersInString:@"-\uFEFF"];   // ASCII hyphen + a BOM riding along on a paste (escaped: an invisible literal here would be unreviewable)
    [strip formUnionWithCharacterSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    NSString *key = [[[code componentsSeparatedByCharactersInSet:strip] componentsJoinedByString:@""] uppercaseString];
...
- (void)blinkeredMaybeShowOfflinePanel:(NSDictionary *)info
{
    ...
    [alert addButtonWithTitle:@"Enter Exit Code…"];
    ...
    void (^offlinePanelAnswer)(NSModalResponse) = ^void (NSModalResponse answer) {
        ...
        if (answer == NSAlertFirstButtonReturn) {
            DDLogInfo(@"Blinkered: offline panel — Retry");
            [[self blinkeredMainWebView] reload];
    ...
    [self runModalAlert:alert conditionallyForWindow:mainWin completionHandler:offlinePanelAnswer];
}
...
// a denial (errAEEventNotPermitted -1743) just returns NO and the caller falls back. Returns YES if a
// matching tab was found + selected.
