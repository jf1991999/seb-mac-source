...
- (NSString *)blinkeredChromiumTabScript:(NSString *)bundleId
{
    return [NSString stringWithFormat:
        ...
        "  end repeat\n"
        "end tell\n"
        "return false\n", bundleId, kBlinkeredTabURLMatch];
}
...
- (void) applicationWillTerminateProceed
{
    ...
    // (and matches the _blinkeredSkipTerminateReturn flag's intent: never open a duplicate tab).
    {
        ...
        if (target) {
            // Blinkered: select the kid's EXISTING dashboard tab in the running browser — the correct
            // tab, no new tab — validated on-device for Chrome/Safari (AppleScript by URL, select+reload)
            // and Firefox (Accessibility by title). Done HERE in the foreground app (which holds the TCC
            // grants), not the background agent. Best-effort: if it can't (unsupported browser / grant
            // missing) the activate-only path below still runs. The agent then brings the browser forward.
            [self blinkeredSelectDashboardTabInBrowser:target.bundleIdentifier pid:target.processIdentifier];
