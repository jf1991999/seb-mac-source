...
#import <os/log.h>   // BlinkeredClearLog — see its definition for why NSLog is not usable there
...
//
// [R3-1] ORIGIN IS NOT ENOUGH — THE SENDER MUST ALSO BE A MAIN FRAME.
//
// The origin check above answers "whose content is this?" and nothing else. Frame POSITION is a
// separate property, and until this line existed it was never checked, so the comment above named
// "a cross-origin iframe" as a threat the gate did not actually stop: an iframe at ANY
// *.blinkered.com.au host, in ANY window, reached the full verb switch — quit and updateURLFilter
// included. `hasSuffix:@".blinkered.com.au"` (SEBAbstractWebView.m:87) trusts every subdomain, and
// the handler is installed on the ONE shared _wkWebViewConfiguration (:1345) every window is built
// from, so nothing else downstream narrowed it either.
//
// A BLANKET main-frame rule is correct here because EVERY legitimate poster is a main-frame
// document — enumerated from the pages themselves, not assumed (seb-classroom@origin/main):
//
//   public/home-content.html   the home lock shell   — the main window's own document
//   public/content.html        the class lock shell  — the main window's own document
//   public/student.html        the student dashboard — a top-level page (showAutoUpdateSetup only)
//
// Those three are the complete set of files posting to window.webkit.messageHandlers.blinkered.
// The main window DOES host iframes, and they are the reason to check rather than a reason not to:
// #contentFrame carries teacher-pushed content, and the per-site keep-alive frames carry
// third-party sites. NEITHER posts, and `isEmbeddable()` returns a hard `false` in both shells
// today, so the site-frame path is dead code — the "an embeddable site was selected in the main
// window's iframe" comment on focusMainContent below describes a design that no longer runs, and
// even when it did, the poster was the SHELL reacting to a click, never the framed site.
//
// So this costs nothing that works today, and it is the property that survives a future author:
// a verb added later is main-frame-gated by default, exactly as the allow-list below makes it
// origin-gated by default. masterCodeExit already carried its own `frameInfo.mainFrame` check
// ([P2R2-13]); that check stays where it is — it is per-verb defence in depth, and the whole point
// of this one is that the next verb should not have to remember.
//
// Note this is STRICTER than Windows, deliberately. Windows attributes a message to the sending
// WINDOW's committed main-frame URL (BlinkeredBridge.ResolveSenderMainFrameUrl) rather than to the
// posting frame, so a SAME-ORIGIN subframe there passes the origin gate; what stops an untrusted
// frame is that the renderer never creates CefSharp.PostMessage outside the trusted origin
// (JavascriptBindingApiAllowOrigins). WebKit gives us no equivalent per-origin injection control —
// a script message handler is exposed to every frame — so the frame check is where we buy it back.
// BEGIN blinkeredBridgeFrameGate
// Load-bearing markers: tools/lockdown-tests/run-bridge-frame-gate-test.sh SLICES the region
// between them and drives REAL WKScriptMessages (a main frame and a same-origin iframe, from a live
// WKWebView) through this exact source — so the frame rule is tested as shipped, not as a copy that
// can drift. Moving or deleting a marker fails that test loudly rather than silently testing nothing.
- (BOOL)blinkeredTrustedBridgeOrigin:(WKScriptMessage *)message
{
    // Fail closed: `frameInfo` is nonnull by API contract, but an absent or ambiguous frame signal
    // must never resolve to "more capability" on a device whose job is to stop a child leaving.
    if (!message.frameInfo.mainFrame) return NO;
      ...
      didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    // buys literally nothing. [R3-1] "each always a main frame" was for a long time an OBSERVATION
    // about the pages rather than a rule about the gate — true of our code, enforced nowhere, and
    // therefore worth nothing against an iframe. blinkeredTrustedBridgeOrigin: now requires it. The four types once proposed for exemption were not inert either:
    ...
    if (![self blinkeredTrustedBridgeOrigin:message]) {
        ...
        // [R3-1] Name WHICH half refused. The gate now has two independent reasons to say no, and a
        // refusal that reports only the origin would describe a subframe at our OWN host as an
        // "untrusted origin" — sending whoever reads this log hunting a host problem that is not
        // there. This is the diagnostic that tells a legitimate poster which rule it broke.
        NSString *reason = message.frameInfo.mainFrame ? @"untrusted origin" : @"a SUBFRAME (main frame required)";
        ...
        //
        // The throttle key carries the frame position too, so a subframe cannot mask the report of
        // a main-frame refusal at the same host (and vice versa) by getting there first.
        NSString *originKey = message.frameInfo.mainFrame ? originDesc
                                                          : [originDesc stringByAppendingString:@" (subframe)"];
        if ([self blinkeredShouldReportTamperKey:[NSString stringWithFormat:@"log|%@|%@", type ?: @"?", originKey]]) {
            DDLogError(@"Blinkered SECURITY: rejected bridge message '%@' from %@ %@ — "
                       @"no handler ran. (Further refusals for this type+origin are throttled.)",
                       type ?: @"<none>", reason, originDesc);
        }
        [self blinkeredReportBridgeTamper:type origin:originKey];
        return;
    }
    ...
    if ([type isEqualToString:@"setQuitPassword"]) {
        ...
        //
        // HIDDEN site windows are closed too. This used to `continue` on !isVisible, so a
        // keep-alive window that clearSiteWindows had orderOut'd SURVIVED a teacher Clear /
        // block-all / unlock — still loaded, still holding its session — and a later
        // focusWindow re-surfaced it. The page fires this verb precisely so that nothing
        // survives; a teardown that skips the windows it cannot see is a teardown that fails
        // OPEN, on the path that ends a locked session. Windows already closed every
        // additional window (BlinkeredWindowController.CloseAdditionalWindows over
        // AdditionalWindows(), which does not consult IsHidden); this makes Mac match.
        dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // Return a JSON array for EVERY SEBBrowserWindow except the main window — visible or not —
        // each carrying an explicit `visible` flag. Called by home-content.html every second to
        // populate the window-switcher tab bar. Result is delivered by evaluating
        // window.__blinkeredWindowsCallback(json) in the main view.
        //
        // The payload is LOSSLESS BY CONTRACT, and both platforms emit the same field set with the
        // same meaning. This loop used to `continue` on !isVisible, so an orderOut'd keep-alive
        // window (clearSiteWindows on a lane/mode switch) was simply ABSENT — while Windows
        // included it, under a comment claiming that matched us. It did not, so identical page code
        // behaved differently per platform, and neither side emitted anything a consumer could use
        // to tell the two apart. Filtering is now the PAGE's job, explicitly, at each call site
        // (home-content.html `winIsVisible`): existence questions ("has the site's window opened
        // yet?") read the whole list; visibility questions ("what can the child see right now?")
        // filter on `visible`.
        dispatch_async(dispatch_get_main_queue(), ^{
            NSWindow *mainWin = [self valueForKey:@"mainBrowserWindow"];
            ...
            for (NSWindow *win in allWindows) {
                ...
                // `visible` is the field that makes the list lossless: NO for a keep-alive window
                // this session has orderOut'd, and for a window that has not been ordered front yet.
                // `active` stays visible-only by construction — an ordered-out window is never the
                // key window — and is ANDed with isVisible so the two fields can never disagree.
                BOOL isVisible = win.isVisible;
                // splitRole is the page's ONLY view of the split: it renders the control's state
                // from this and holds no authoritative copy, so there is no pair identity and no
                // counter on either side to disagree about. 0 = none, 1 = left, 2 = right.
                NSNumber *splitRole = @(((SEBBrowserWindow *)win).blinkeredSplitRole);
    ...
    } else if ([type isEqualToString:@"setDesktopTint"]) {
        // The macOS menu-bar strip above the locked page is drawn by the OS and samples the DESKTOP
        // PICTURE, not the window stack (measured with controls — docs/MENU_BAR_BACKDROP_PLAN.md, B0
        // and "B — MEASURED VIABLE"). Nothing this app draws can reach it, so the only way to stop it
        // reading as a seam above the aquarium is to set the wallpaper to the water's own colour.
        // The PAGE sends the colour because the page owns the art, and the art is not a constant: the
        // water's top row runs bright mid-screen under the sun glow and dark at the edges, so this
        // carries a PROFILE across the width rather than a single value.
        //
        // Everything past validation happens in SEBController, which owns the reversibility gate:
        // the tint is applied only where the original wallpaper can be provably put back.
        //
        // NOT in the __blinkeredNative.capabilities list, deliberately. That list is pinned to an
        // identical one in blinkered-win, and declaring a verb the other platform does not implement
        // is the direction its own comment calls unsafe — Windows has no menu bar and will never have
        // this. The page does not gate on the capability either: it posts on any non-Windows build and
        // an older Mac simply logs the unknown type. (The release gate catches a one-sided edit here;
        // it caught this one.)
        NSArray *profile = body[@"profile"];
        if (![profile isKindOfClass:[NSArray class]] || profile.count < 2 || profile.count > 64) return;
        NSMutableArray *clean = [NSMutableArray arrayWithCapacity:profile.count];
        for (id entry in profile) {
            if (![entry isKindOfClass:[NSArray class]] || [(NSArray *)entry count] != 3) return;
            NSMutableArray *rgb = [NSMutableArray arrayWithCapacity:3];
            for (id channel in (NSArray *)entry) {
                if (![channel isKindOfClass:[NSNumber class]]) return;
                NSInteger v = [channel integerValue];
                [rgb addObject:@(MAX(0, MIN(255, v)))];      // clamp: the page is not trusted to be in range
            }
            [clean addObject:rgb];
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:@"BlinkeredSetDesktopTint"
                                                            object:nil
                                                          userInfo:@{@"profile": clean}];

    } else if ([type isEqualToString:@"clearDesktopTint"]) {
        // The water is gone (a site is showing, or the lock ended) — hand the wallpaper back. Over
        // the grey browsing bar a teal strip is worse than the child's own wallpaper ever was.
        [[NSNotificationCenter defaultCenter] postNotificationName:@"BlinkeredClearDesktopTint" object:nil];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (NSDictionary *it in items) {
                ...
                // class, message, site, report, freetime, search, back, switch, exit); map it to an SF
                // Symbol. Template images, so the menu tints them to match its text in light/dark and
                // when highlighted.
    ...
    } else if ([type isEqualToString:@"setSplit"]) {
        // Assign the two halves of a 50/50 split, or — with no ids — END the split.
        //
        // ATOMIC, and named to a thread: the whole body runs in ONE main-queue turn. Window
        // destruction is main-thread too, so nothing can die between resolving the ids and
        // applying the roles. Both roles are set BEFORE either frame is computed, because the
        // reverter (windowDidResize: -> blinkeredEnforceKioskFrame -> setCalculatedFrame) will
        // re-enter synchronously off the first setFrame: and must read the FINAL roles, not a
        // half-applied pair.
        //
        // Both ids must resolve or neither role is applied: a half-assigned split is one
        // half-width window overlapped by a full-width one, with nothing to correct it.
        //
        // NOT capability-declared yet. Declaring setSplit is a lockstep edit across three repos
        // (both native declarations plus all three capability pins), so it lands last — until
        // then no page renders a control and this verb is unreachable in the field.
        NSInteger leftNumber = [body[@"left"] isKindOfClass:[NSNumber class]] ? [body[@"left"] integerValue] : 0;
        NSInteger rightNumber = [body[@"right"] isKindOfClass:[NSNumber class]] ? [body[@"right"] integerValue] : 0;
        dispatch_async(dispatch_get_main_queue(), ^{
            SEBOSXBrowserController *browserController = (SEBOSXBrowserController *)self;
            if (leftNumber <= 0 || rightNumber <= 0) {
                // The exit verb. Same ordered clear every other path uses.
                DDLogInfo(@"[Blinkered] setSplit{}: ending the split.");
                [browserController blinkeredClearSplitRolesDyingWindow:nil];
                return;
            }
            NSWindow *mainWin = [self valueForKey:@"mainBrowserWindow"];
            SEBBrowserWindow *leftWindow = nil;
            SEBBrowserWindow *rightWindow = nil;
            for (NSWindow *win in [[NSApplication sharedApplication].windows copy]) {
                if (win == mainWin || ![win isKindOfClass:NSClassFromString(@"SEBBrowserWindow")]) {
                    continue;
                }
                if (win.windowNumber == leftNumber) { leftWindow = (SEBBrowserWindow *)win; }
                if (win.windowNumber == rightNumber) { rightWindow = (SEBBrowserWindow *)win; }
            }
            if (!leftWindow || !rightWindow || leftWindow == rightWindow) {
                DDLogWarn(@"[Blinkered] setSplit(%ld, %ld) refused: left %@, right %@ — applying "
                          @"NEITHER role. A half-assigned split has no way to correct itself.",
                          (long)leftNumber, (long)rightNumber,
                          leftWindow ? @"ok" : @"unresolved", rightWindow ? @"ok" : @"unresolved");
                return;
            }
            // Both roles first, then both frames — see the re-entrancy note above.
            leftWindow.blinkeredSplitRole = SEBBlinkeredSplitRoleLeft;
            rightWindow.blinkeredSplitRole = SEBBlinkeredSplitRoleRight;
            [leftWindow setCalculatedFrame];
            [rightWindow setCalculatedFrame];
            DDLogInfo(@"[Blinkered] setSplit: window %ld = left, window %ld = right.",
                      (long)leftNumber, (long)rightNumber);
        });
...
- (void) transferCookiesToWKWebViewWithCompletionHandler:(void (^)(void))completionHandler
{
    ...
    // preserved: same config key → we don't re-read → the picker's child stands.
    {
        ...
        if (newConfig) {
            ...
            // [B'] blinkeredClearChildData IS DELIBERATELY IGNORED. It used to delete each named child's
            // jar here. The server (blinkered-classroom #734) stopped emitting the key on 21 Aug 2026 and
            // per-child clears now ride the POLL, performed by the agent and retired only when the agent
            // confirms them — so this branch has no server behind it any more.
            //
            // Reading it would be strictly worse than dead code, which is why it is unwired rather than
            // left to rot. A .seb is CACHED and REPLAYED: by the crash watchdog, by an offline re-lock, by
            // a reboot, and on this platform by offline-lock.seb, a byte-copy of the last real lock config
            // that no unlock deletes. A config already on a device still carries the key, and the server
            // can no longer send a corrected array — so acting on it would let a stale file replay a
            // deletion nobody can withdraw. Ignoring it is the only thing that closes that replay.
            // (blinkered-win did the same in the same release; the agent additionally strips the key from
            // offline-lock.seb as it stages it, so the payload does not sit on disk either.)
            //
            // Logged, not silent: a device still holding a pre-B' config is worth knowing about when
            // reading a log, and this line is the only place that fact is visible.
            // DO NOT RE-WIRE THIS TO CONFIG. tools/lockdown-tests/run-seb-clear-key-unwired-test.sh scans
            // every source file in the tree for a jar deletion driven by this key and fails the build.
            id retiredClearList = [prefs secureObjectForKey:@"org_safeexambrowser_SEB_blinkeredClearChildData"];
            if ([retiredClearList isKindOfClass:[NSArray class]] && ((NSArray *)retiredClearList).count > 0) {
                BlinkeredClearLog(@"BLINKERED transferCookies: this config names %lu child(ren) under the RETIRED blinkeredClearChildData key - ignoring it (per-child clears ride the poll now)",
                      (unsigned long)((NSArray *)retiredClearList).count);
            }
    ...
    if (self.blinkeredFocusChildId.length > 0) {
        ...
        if (!self.blinkeredCookieOwnerChecked) {
            ...
            // NOTE THE ORDERING, because this file is now load-bearing for a DELETION (R2 §F6.2).
            // The owner is claimed here, before the wipe below — which is asynchronous — completes.
            // A force-quit inside that window leaves cookie-store-owner.txt naming a child whose data
            // is not actually in the store, and the AGENT deletes the live store on exactly that
            // attribution. Pre-existing ordering, new consequence.
            //
            // Left as-is deliberately: the loss is bounded and self-healing. The mis-attributed child
            // loses only live-store data that the interrupted wipe was about to destroy anyway; their
            // jar is intact, and their next adoption wipes and restores from it. Claiming the owner
            // AFTER the wipe would trade that for a worse failure — a crash mid-wipe leaving the file
            // naming the PREVIOUS child, so the next launch's owner-mismatch branch does not fire and
            // one kid inherits another's residual cookies, which is the Rupert/Rory leak this whole
            // block exists to close.
            [self blinkeredWriteCookieStoreOwner:self.blinkeredFocusChildId];
            // [B'] A CLEAR LANDED FOR THIS CHILD → WIPE, exactly as an owner change does.
            //
            // The jar is a BACKUP. The child's live cookies are in this persistent store, and nothing
            // on a normal launch empties it: examSessionClearCookiesOnStart is unconditionally <false/>
            // and the owner check below fires only when the owner CHANGES — which on an assigned Mac
            // it never does. So a clear that deleted only the jar left the child signed in and the
            // exit save wrote the same pre-clear cookies back (MAC_CHILD_CLEAR_REVIEW_R1 §F1).
            //
            // The agent deletes this store itself when it can attribute it (the owner file names the
            // cleared child). This is the case it cannot: a device where the owner was unrecorded, or
            // where a session was running when the clear landed. Reuses the wipe below rather than
            // adding a second deletion primitive — it is already shipped and proven (the Rupert/Rory
            // fix), and it goes through WKWebsiteDataStore's own API, so it survives WebKit moving its
            // files. Consuming the tombstone here is also what lets the SAVE path decide on presence
            // alone: one that reaches a save can only have been written after this adoption.
            // DDLogInfo, not NSLog, and that is not cosmetic. NSLog reaches only the unified log,
            // which needs privileged access to read back — so on the first device run of this feature
            // (MAC-B'-1, 25 Aug) these four lines were the ONLY ones that could not be recovered, and
            // the run could not say which branch had wiped the store. The agent's decisions go to a
            // plain file anyone can read; the app's half, where D9 actually lives, did not.
            BOOL clearedSinceLastSession = [self blinkeredConsumeClearTombstoneForChild:self.blinkeredFocusChildId];
            if (clearedSinceLastSession) {
                // The migration re-seeds an ABSENT jar from the legacy global cookies.json — and an
                // absent jar is exactly what a clear produces, so the clear ARMS it. Suppressing it
                // here is what stops the parent's deletion being undone by a file the wipe below does
                // not reach (R1 §F2).
                BOOL deviceIsAssigned = self.blinkeredCookieMigrateFromConfig;
                self.blinkeredCookieMigrateFromConfig = NO;
                // ...and suppression ALONE is not enough, because it is one-shot against a DURABLE
                // file (R2 §F3). blinkeredCookieMigrate is emitted on every lock for an assigned
                // device, so the flag returns with the next config; the migration's trigger is a
                // LOADED COUNT of zero, not file absence, and a session that ends with no cookies
                // writes a valid EMPTY jar that reads back as zero. So any later session could
                // re-fire the migration and re-seed pre-clear logins from cookies.json — after the
                // clear was retired.
                //
                // The flag being set is the server telling us this is an ASSIGNED device, i.e. that
                // cookies.json is THIS child's logins. That is the one moment we are entitled to
                // delete it, so delete it, rather than only declining to read it once. The agent
                // does the same when it can attribute the store by name; this covers the case it
                // could not (owner unrecorded, or a session was running).
                //
                // Deliberately NOT done when the flag is clear: on a shared or reassigned device
                // cookies.json may be a DIFFERENT child's, and deleting an unattributed file is the
                // glob wipe this workstream removed. That residual case is recorded in
                // MAC_CHILD_CLEAR_REVIEW_R2 §F3 and is not closed here.
                if (deviceIsAssigned) {
                    [self blinkeredDeleteGlobalCookieFile];
                    BlinkeredClearLog(@"BLINKERED transferCookies: child %@ was cleared and this is an ASSIGNED device → deleted the legacy global cookies.json so a later migration cannot re-seed their pre-clear logins (R2 §F3)", BlinkeredSanitizeChildId(self.blinkeredFocusChildId));
                }
                BlinkeredClearLog(@"BLINKERED transferCookies: child %@ was cleared since their last session → wiping the live store and skipping the legacy migration", BlinkeredSanitizeChildId(self.blinkeredFocusChildId));
            }
            if (clearedSinceLastSession || (owner.length > 0 && ![owner isEqualToString:self.blinkeredFocusChildId])) {
                // [R3 F6] SANITISED, and not for privacy — a real id is a UUID, so the sanitiser is a
                // no-op on it. blinkeredFocusChildId is PAGE-SUPPLIED on a shared device
                // (body[@"childId"] → blinkeredProfileSwitch:), and this line now lands in the flat
                // file the device-run method reads to attribute a wipe. A newline in that value would
                // forge lines in the very log the next run scores from.
                BlinkeredClearLog(@"BLINKERED transferCookies: wiping the live store before restore (owner %@, session child %@)", owner.length ? BlinkeredSanitizeChildId(owner) : @"<none>", BlinkeredSanitizeChildId(self.blinkeredFocusChildId));
...
- (NSImage *)blinkeredSessionMenuIconForName:(NSString *)name
{
    ...
    dispatch_once(&once, ^{
        map = @{ @"calendar": @"calendar",                            // My day
                 ...
                 @"report":   @"exclamationmark.triangle",            // Report a problem
                 @"freetime": @"gamecontroller.fill",                 // Free time (the approved games)
                 @"search":   @"magnifyingglass",                     // Research / ask to research
                 @"back":     @"arrow.left",                          // Back to homework
                 @"switch":   @"arrow.left.arrow.right",              // Switch user
                 @"exit":     @"rectangle.portrait.and.arrow.right" }; // Exit / leave / free-time doors
    });
...
- (void)blinkeredProfileSwitch:(NSString *)action childId:(NSString *)childId url:(NSString *)navUrl webView:(WKWebView *)webView
{
    ...
    if ([action isEqualToString:@"leave"]) {
        ...
        {
            // [B'] This path already performs the full wipe below, so a pending clear for this child
            // is satisfied by it. Spend the tombstone so it cannot go on to refuse the save at the end
            // of a session whose cookies are entirely post-clear.
            (void)[self blinkeredConsumeClearTombstoneForChild:childId];
...
// The child-id → filename rule that NAMES every per-child jar on this Mac.
//
// [B'] The agent deletes these files by name and cannot call this method — it is a separate binary
// that links none of the app — so blinkeredSanitizeChildId() in BlinkeredAgent/ChildDataClears.swift
// duplicates it. That duplication is the dangerous kind: a drift means the agent deletes a file
// nothing ever created and then reports success to the server, which retires the request. The
// BEGIN/END markers are load-bearing — tools/lockdown-tests/run-child-clear-parity-test.sh SLICES
// this region out, compiles it, and runs it against the Swift side over the same vectors, so
// neither can change without the other going red.
// [R3 F1] Every line that records a clear DECISION goes through here, and it writes to BOTH sinks.
//
// The first cut of this moved them from NSLog to DDLogInfo so they would reach the flat file the
// device-run method reads. That was half right and the commit claiming "reaches the file AND the
// unified log, so nothing is lost" was wrong: DDOSLogger — the unified-log sink — is added only
// inside `#ifdef DEBUG` (SEBController.m), and Release defines NDEBUG, not DEBUG. Worse, when
// org_safeexambrowser_SEB_enableLogging is NO the file logger is REMOVED, so DDLog has zero sinks
// and all six lines are discarded. Under the original NSLog they at least always reached the unified
// log — which is exactly where MAC-B'-1 went looking and found nothing.
//
// So: NSLog for a sink that no config can switch off, DDLogInfo for the file that can be read back
// without privileges. Neither alone is sufficient and the pair costs one duplicated line in a debug
// build. tools/lockdown-tests/run-child-clear-parity-test.sh pins that all six sites use this.
static void BlinkeredClearLog(NSString *format, ...) NS_FORMAT_FUNCTION(1,2);
static void BlinkeredClearLog(NSString *format, ...)
{
    va_list args;
    va_start(args, format);
    NSString *line = [[NSString alloc] initWithFormat:format arguments:args];
    va_end(args);
    // os_log with an EXPLICIT %{public}@, not NSLog. Measured on macOS 26.5, DEBUG undefined:
    //
    //   NSLog(@"%@", line)            -> <private>. A runtime-built NSString passed as the whole
    //                                    format argument is redacted wholesale, so the device gets
    //                                    the literal text "<private>" and nothing else.
    //   NSLog(@"%{public}@", line)    -> <decode: missing data>. The annotation is not honoured here.
    //   os_log(…, "%{public}@", line) -> the full line. This one.
    //
    // The first cut of this helper used the first form and so lost the line entirely — which, with
    // enableLogging NO, left the six decisions readable NOWHERE, reproducing the very gap this helper
    // was added to close, for the third time (R3 §F1, then its own confirmation read). It was also a
    // regression against shipped 3.6.206, where these were literal formats whose fixed text survived.
    //
    // PUBLIC IS DELIBERATE, and it is an exposure change from 3.6.206, where the `%@` arguments were
    // redacted. The id is sanitised and is a UUID with no personal data in it; it is already on disk
    // in the flat log and in the tombstone's own filename; and a decision line whose child reads
    // "<private>" cannot attribute a wipe, which is the only reason these lines exist.
    // BARE os_log(), i.e. DEFAULT level — not os_log_info(). `log show --predicate
    // 'process == "Blinkered"'` returns Default and NOT Info without `--info`. At Info level these
    // lines would be invisible to the documented run while looking perfectly healthy in Console.app.
    // The parity gate pins this exact call, so switching the level turns it red — read that failure as
    // "the device run can no longer see these", not as a style nit.
    //
    // READING THESE BACK IS AN ADMIN OPERATION, and this comment used to imply otherwise. The
    // persisted store /var/db/diagnostics is `drwxr-x--- root:admin` and /usr/bin/log is not setuid,
    // so the child's account cannot read it. Measured on the Kid's Mac as the child: BOTH the
    // predicate query and a bare `log show --last 60s` exit 77 (EX_NOPERM) with
    //   log: Could not open local log store: Operation not permitted
    // MAC-B'-2's `| wc -l` discarded that line and its 0 read as "the app logged nothing". (What
    // actually fired that night is unrecoverable — the zsh `log` BUILTIN produces the same 0, and its
    // probe was never committed. Which is moot: that account cannot read the log by any invocation.
    // Note the trap this leaves — `/usr/bin/log` over the builtin is NECESSARY BUT NOT SUFFICIENT.)
    // Two consequences for anyone reading this sink on a device:
    //
    //   * Run the query from an ADMIN account on that Mac — `su <admin>` from the child's Terminal
    //     needs no logout. The unified log is system-wide, so an admin sees the child's Blinkered
    //     process without switching users.
    //   * A ZERO IS NOT AN EMPTY RESULT. A query that runs and matches nothing still prints its
    //     one-line header, so `| wc -l` gives 1. A `log show` that FAILS prints nothing on stdout and
    //     the reason on stderr, so `| wc -l` gives 0. Never pipe this to wc without keeping stderr —
    //     1 and 0 mean opposite things. Observed both ways on the same device (26.5.2): header absent
    //     as the child, present as an admin minutes later; and separately on 26.6.2.
    //
    // And read it back the SAME DAY. Default-level messages live in /var/db/diagnostics/Persist under
    // a SIZE cap, not a time one — measured at ~10h on one Mac and ~35h on another, so it is whatever
    // that machine's log volume makes it. `log stats` reports a much older archive start and will
    // mislead you; measure Persist. The flat file has no such horizon, which is the other half of why
    // both sinks exist.
    //
    // Read back off a device on 27 Aug 2026, admin account, all four decision lines at Default and in
    // full — the same four the flat file had. See docs/OSLOG_SINK_READBACK.md §4.
    os_log(OS_LOG_DEFAULT, "%{public}@", line);   // never config-gated
    DDLogInfo(@"%@", line);                       // the flat file, when enableLogging is on
}

// BEGIN blinkeredChildIdSanitisation
static NSString *BlinkeredSanitizeChildId(NSString *childId)
{
...
// END blinkeredChildIdSanitisation

- (NSString *)blinkeredProfileCookieDir
{
...
- (NSString *)blinkeredCookiePathForChild:(NSString *)childId
{
    return [[self blinkeredProfileCookieDir] stringByAppendingPathComponent:
            [BlinkeredSanitizeChildId(childId) stringByAppendingPathExtension:@"plist"]];
}

// [B'] The clear tombstone for one child: profile-cookies/.cleared-<sanitized>, dropped by the agent
// immediately BEFORE it unlinks that child's jar. Cannot collide with a jar (those end in .plist, and
// the sanitiser above cannot emit a leading dot). Its contents are the clear's wall-clock time in
// milliseconds, as text. See -blinkeredSaveCookies:forChild: for what it is for.
- (NSString *)blinkeredClearTombstonePathForChild:(NSString *)childId
{
    return [[self blinkeredProfileCookieDir] stringByAppendingPathComponent:
            [@".cleared-" stringByAppendingString:BlinkeredSanitizeChildId(childId)]];
}

// [B'] THE CLEAR TOMBSTONE — the app's half of a per-child data clear.
//
// The agent drops profile-cookies/.cleared-<sanitized> immediately BEFORE it unlinks that child's jar.
// The tombstone carries TWO instructions at once, and that is the thing to keep hold of when changing
// anything here:
//
//   (i)  "refuse this child's next jar write", and
//   (ii) "wipe this child's LIVE STORE when you next adopt them".
//
// (ii) is the one that matters most, because the jar is only a BACKUP — the child's actual cookies,
// LocalStorage and IndexedDB live in the shared persistent WKWebsiteDataStore, and on an assigned Mac
// nothing on a normal launch empties it (MAC_CHILD_CLEAR_REVIEW_R1 §F1).
//
// SO PEEKING AND CONSUMING ARE DIFFERENT ACTS, and which one a site performs is decided by ONE RULE:
//
//   ** only a site that actually performs the wipe may CONSUME the tombstone. **
//
// Everything else PEEKS. Getting this wrong is not a style point: until R2 §F1 the save consumed it,
// so in the one situation the tombstone exists for — a clear landing while a session is somehow
// running — the save refused the jar write (protecting the copy), deleted the tombstone, and left
// nothing to wipe the original. The child stayed signed in, with the clear already confirmed to the
// server and reported to the parent as done.
//
// The three sites, and why each does what it does:
//
//   ADOPTION (-transferCookiesToWKWebViewWithCompletionHandler:)          CONSUMES
//     It wipes the live store and suppresses the legacy migration in the same breath. This is what
//     makes a clear the agent could NOT attribute to the store land anyway, the moment the child next
//     opens Blinkered.
//
//   THE PICKER's login (-blinkeredProfileSwitch:)                          CONSUMES
//     It performs its own comprehensive wipe of every data type unconditionally, so the tombstone's
//     instruction is discharged. Spending it here is what stops it going on to refuse the save at the
//     end of a session whose cookies are entirely post-clear.
//
//   THE SAVE (-blinkeredSaveCookies:forChild:, the one choke point for every jar write)   PEEKS
//     It refuses the write and LEAVES THE TOMBSTONE STANDING, because it has wiped nothing. The
//     instruction is still owed, and the next adoption is what discharges it.
//
// WHY PEEKING AT THE SAVE CANNOT STRAND A CHILD, which is the property "spend on first use" existed
// to protect. An unspent tombstone would refuse that child's every future save and discard their
// logins on every quit — worse than the defect it guards against. It cannot happen, because a save
// for a child is only reachable once that child has been adopted, and every adoption consumes:
//
//   * -blinkeredSaveCookies: needs a childId. Both callers take it from blinkeredFocusChildId
//     (SEBController.m's exitSEB) or from the picker's own childId (the `leave` path), and that
//     property is only ever set in transferCookies or in -blinkeredProfileSwitch: — the two consuming
//     sites.
//   * The once-per-launch blinkeredCookieOwnerChecked flag can skip the adoption consume for a child
//     picked up by the "best-effort first fill" branch. That merely DEFERS the wipe: every fresh
//     lock/focus session is a new config, which re-arms the flag, so the next lock consumes and wipes.
//     No save is wrongly allowed in the meantime — the peek refuses them all — and the failure
//     direction is "the parent's deletion is still pending", never "the deletion was undone".
//
// The end-to-end suite pins both halves: Timeline 4 asserts the tombstone SURVIVES the save and is
// then spent by a real adoption which empties the live store, and Timeline 6 asserts the child is not
// stranded afterwards.
//
// SO THERE IS NO TIMESTAMP, AND THAT IS THE POINT. The first cut compared the tombstone's stamp against
// this process's start, which needed a wall clock shared by two processes and was wrong for a second
// reason besides: on a shared device one process outlives many child sessions, so process start is not
// when THIS child's cookies were adopted (MAC_CHILD_CLEAR_REVIEW_R1 §F5), and a backward clock step
// between the two inverted the comparison (§S5). Consuming at adoption carries the ordering by
// construction, so both classes are closed rather than mitigated, and no clock is read at all.
//
// The stamp the agent writes is retained as CONTENT for the log — it says when the clear happened —
// and nothing decides on it.
// The whole mechanism lives between these markers as FREE FUNCTIONS over a path, so the lockdown
// suite compiles and drives THE SHIPPED CODE rather than a transcription of it. The methods below are
// one-line wrappers that supply the path and log.
//
// This shape is not decoration. R2 §F2 put a single `return NO;` at the top of the old method — which
// disabled the adoption wipe, the migration suppression and the save refusal simultaneously — and
// EVERY GATE STAYED GREEN, because nothing in the tree compiled it: the vectors linked only
// BlinkeredClearSupersedesSave, the session probe transcribed the read-and-spend, and the end-to-end
// test faked the adoption consume with `rm -f`. The mechanism carrying the whole of R1 §F1's fix was
// the one thing no test could hold in place.
// BEGIN blinkeredClearTombstoneDecision
static BOOL BlinkeredClearSupersedesSave(NSString *tombstoneContents)
{
    // PRESENCE, not value. nil means the file was not there; anything else — including an empty or
    // unreadable one — means a clear landed after this session adopted the child.
    return tombstoneContents != nil;
}

// PEEK. Is there a standing clear instruction for this child? Reads the tombstone and LEAVES IT.
//
// This is what a site that has not wiped anything must call. It answers (i) — "refuse this jar write"
// — without discharging (ii), which is still owed to whoever next adopts the child.
static BOOL BlinkeredPeekClearTombstone(NSString *tombPath, NSString **outContents)
{
    if (tombPath.length == 0) return NO;
    NSString *contents = [NSString stringWithContentsOfFile:tombPath encoding:NSUTF8StringEncoding error:nil];
    if (outContents) *outContents = contents;
    return BlinkeredClearSupersedesSave(contents);
}

// CONSUME. Peek, then SPEND — the instruction is discharged and will not be seen again.
//
// ONLY a caller that performs the live-store wipe may call this. A caller that consumes without
// wiping destroys the parent's instruction and leaves the child's actual data in place; that is
// exactly R2 §F1, and it is why the save above this line peeks instead.
static BOOL BlinkeredConsumeClearTombstone(NSString *tombPath, NSString **outContents)
{
    if (!BlinkeredPeekClearTombstone(tombPath, outContents)) return NO;
    [[NSFileManager defaultManager] removeItemAtPath:tombPath error:nil];
    return YES;
}
// END blinkeredClearTombstoneDecision

// Is a clear standing for this child? Does NOT spend it — see the rule above.
- (BOOL)blinkeredHasClearTombstoneForChild:(NSString *)childId
{
    if (childId.length == 0) return NO;
    return BlinkeredPeekClearTombstone([self blinkeredClearTombstonePathForChild:childId], NULL);
}

// Read and SPEND this child's tombstone. Returns YES if one was there. Callers must wipe.
- (BOOL)blinkeredConsumeClearTombstoneForChild:(NSString *)childId
{
    if (childId.length == 0) return NO;
    NSString *contents = nil;
    if (!BlinkeredConsumeClearTombstone([self blinkeredClearTombstonePathForChild:childId], &contents)) return NO;
    BlinkeredClearLog(@"BLINKERED cookie clear: consumed the clear tombstone for child %@ (stamped %@)",
          BlinkeredSanitizeChildId(childId),
...
- (void)blinkeredSaveCookies:(NSArray<NSHTTPCookie *> *)cookies forChild:(NSString *)childId
{
    ...
    // [B'] The ONE choke point for every write of a per-child jar (session exit and the picker's
    // `leave` both land here), which is why the clear backstop lives here and not at each caller.
    // A tombstone still present at a SAVE was written after this session adopted the child — adoption
    // consumes it — so the cookies about to be written predate the clear. Refuse; the clear stands.
    //
    // PEEK, NOT CONSUME. This path wipes nothing: it declines to write the jar and returns. The
    // tombstone's other instruction — wipe this child's LIVE store — is still owed, and the next
    // adoption is what discharges it. Spending it here (which is what shipped until R2 §F1) protected
    // the backup and destroyed the only route to the original, in precisely the case the tombstone
    // exists for. See the rule at -blinkeredConsumeClearTombstoneForChild:.
    if ([self blinkeredHasClearTombstoneForChild:childId]) {
        BlinkeredClearLog(@"BLINKERED cookie clear: NOT writing child %@'s jar back - it was cleared after this session adopted them (tombstone LEFT STANDING for the next adoption to act on)", BlinkeredSanitizeChildId(childId));
        return;
    }
