    ...
    // preserved: same config key → we don't re-read → the picker's child stands.
    {
        ...
        if (newConfig) {
            ...
            if ([retiredClearList isKindOfClass:[NSArray class]] && ((NSArray *)retiredClearList).count > 0) {
                NSLog(@"BLINKERED transferCookies: this config names %lu child(ren) under the RETIRED blinkeredClearChildData key - ignoring it (per-child clears ride the poll now)",
    ...
    if (self.blinkeredFocusChildId.length > 0) {
        ...
        if (!self.blinkeredCookieOwnerChecked) {
            ...
            if (clearedSinceLastSession) {
                ...
                if (deviceIsAssigned) {
                    [self blinkeredDeleteGlobalCookieFile];
                    NSLog(@"BLINKERED transferCookies: child %@ was cleared and this is an ASSIGNED device → deleted the legacy global cookies.json so a later migration cannot re-seed their pre-clear logins (R2 §F3)", self.blinkeredFocusChildId);
                }
                NSLog(@"BLINKERED transferCookies: child %@ was cleared since their last session → wiping the live store and skipping the legacy migration", self.blinkeredFocusChildId);
            }
            if (clearedSinceLastSession || (owner.length > 0 && ![owner isEqualToString:self.blinkeredFocusChildId])) {
                NSLog(@"BLINKERED transferCookies: wiping the live store before restore (owner %@, session child %@)", owner.length ? owner : @"<none>", self.blinkeredFocusChildId);
...
- (BOOL)blinkeredConsumeClearTombstoneForChild:(NSString *)childId
{
    ...
    NSLog(@"BLINKERED cookie clear: consumed the clear tombstone for child %@ (stamped %@)",
...
- (void)blinkeredSaveCookies:(NSArray<NSHTTPCookie *> *)cookies forChild:(NSString *)childId
{
    ...
    if ([self blinkeredHasClearTombstoneForChild:childId]) {
        NSLog(@"BLINKERED cookie clear: NOT writing child %@'s jar back - it was cleared after this session adopted them (tombstone LEFT STANDING for the next adoption to act on)", BlinkeredSanitizeChildId(childId));
        return;
    }
