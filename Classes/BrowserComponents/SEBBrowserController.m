...
{
    // Blinkered per-child isolation: in a kid focus session, restore ONLY this child's saved
    // cookies and IGNORE the global cookies.json. Otherwise SEB re-injects the previous user's
    // cookies into every new window (e.g. the Alto popup), leaking the last kid's logins.
    ...
    // Read cookies saved by exitSEB from the JSON file written atomically on last quit.
    ...
    NSLog(@"BLINKERED transferCookies: cookie file %@", data ? @"found" : @"not found");

    if (data) {
        NSArray *cookieArray = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        NSLog(@"BLINKERED transferCookies: restoring %lu cookies", (unsigned long)cookieArray.count);
...
- (void)blinkeredProfileSwitch:(NSString *)action childId:(NSString *)childId url:(NSString *)navUrl webView:(WKWebView *)webView
{
    ...
    if (childId.length > 0) { self.blinkeredFocusChildId = childId; }
