      ...
      didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    if ([type isEqualToString:@"setQuitPassword"]) {
        ...
        // Bring a specific open SEBBrowserWindow to the front, matched by URL.
        // Tries exact URL match first; falls back to matching by host so the tab bar
        // still works even if the page navigated slightly since the last poll.
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSArray *allWindows  = [[NSApplication sharedApplication].windows copy];
            for (NSWindow *win in allWindows) {
                // NOTE: do NOT skip !win.isVisible here. The red-close button hides
                // additional windows (orderOut) rather than closing them, so a window
                // the user "closed" is invisible but still alive — we must still match
                // and re-show it. Match each tab to ITS OWN window via the window's
                // ORIGINAL url (and current url), so with several 🪟 sites open clicking
                // one tab never lands on a different site's window.
            ...
            // Match this tab to its own window by exact/original/host URL. No "any open
            // window" fallback — that focused the wrong site when several were open.
            // Only reopen when this site genuinely has no window.
            ...
            if (target) {
                ...
                if (mainWk) {
                    NSString *urlEsc = [targetUrl stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"];
                    NSString *js = [NSString stringWithFormat:@"window.__blinkeredReopenTab && window.__blinkeredReopenTab('%@')", urlEsc];
