...
// [R1-1] Only our own served pages may send MUTATING bridge messages. Script-message handlers are
// exposed to EVERY frame of EVERY origin in webviews sharing this configuration — so without this
// gate, any JavaScript running inside a lock (an allow-listed site with an XSS/eval sink, a kid's
// own allow-listed domain, a cross-origin iframe) could postMessage setQuitPassword and overwrite
// the quit hash — the exact offline exit boundary (OFFLINE_EXIT_CODE_PLAN.md §2.0). The check uses
...
- (BOOL)blinkeredTrustedBridgeOrigin:(WKScriptMessage *)message
{
    ...
    BOOL trusted = [origin.protocol isEqualToString:@"https"] &&
        ([host isEqualToString:@"blinkered.com.au"] || [host hasSuffix:@".blinkered.com.au"]);
#ifdef DEBUG
    // Local dev server only in debug builds — never in a release binary.
    trusted = trusted || [host isEqualToString:@"localhost"] || [host isEqualToString:@"127.0.0.1"];
#endif
    return trusted;
}

// Best-effort security event to the server (parent alert, deduped server-side) when a page is
// caught sending a mutating bridge message from an untrusted origin. Device creds from agent.json.
      ...
      didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    // [R1-1] BLOCKING gate: mutating messages (they change enforcement state) only from our origin.
    // `quit` ENDS the lockdown, so it belongs here with the other enforcement-mutating messages: the
    // frame-origin check means an allow-listed third-party page, an XSS sink inside one, or an
    // embedded iframe can never post it.
    if ([type isEqualToString:@"setQuitPassword"] || [type isEqualToString:@"updateURLFilter"] ||
        [type isEqualToString:@"quit"]) {
        if (![self blinkeredTrustedBridgeOrigin:message]) {
            WKSecurityOrigin *o = message.frameInfo.securityOrigin;
            DDLogError(@"Blinkered SECURITY: rejected mutating bridge message '%@' from untrusted origin %@://%@ — quit hash / URL filter unchanged", type, o.protocol, o.host);
            [self blinkeredReportBridgeTamper:type origin:[NSString stringWithFormat:@"%@://%@", o.protocol, o.host]];
            return;
        }
    }
    ...
    if ([type isEqualToString:@"setQuitPassword"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *targetHost = [NSURL URLWithString:targetUrl].host ?: @"";
            // Matcher: does this window belong to the tab? Exact current/original URL, or same
            // host (current or original) — so with several 🪟 sites open clicking one tab never
            // lands on a different site's window.
            BOOL (^matchesTab)(NSWindow *) = ^BOOL(NSWindow *win) {
                ...
                if ([winUrl isEqualToString:targetUrl] || [origUrl isEqualToString:targetUrl]) return YES;
            ...
            // GROUP: exact/host matches seed the tab's window group, then windows OPENED BY a
            // group member join it (blinkeredOpenerWindowNumber chain, transitively). A document
            // opened from the OneDrive file list lives on another host entirely (tab =
            // www.onedrive.com, list = onedrive.live.com, doc = an Office host — Surface,
            // 12 Aug 2026), so the opener chain is the only tie between the doc and its tab.
            NSArray *allWindows = [[NSApplication sharedApplication].windows copy];
            ...
            // PASS 1 — MRU: scan orderedWindows (front-to-back, visible windows only) and take the
            // FIRST group member. The front-most one is the window the student was last using. The
            // old exact-URL-first rule always surfaced the tab's ORIGINAL window here, which buried
            // an open document under the file list on every tab switch ("my file closed").
            NSWindow *target = nil;
            for (NSWindow *win in [[NSApplication sharedApplication].orderedWindows copy]) {
                if (win == mainWin) continue;
                if (![win isKindOfClass:NSClassFromString(@"SEBBrowserWindow")]) continue;
                if ([groupNumbers containsObject:@(win.windowNumber)]) { target = win; break; }
            }
            // PASS 2 — revival: nothing visible matched, so scan ALL windows including hidden
            // ones. The red-close button hides additional windows (orderOut) rather than
            // closing them, so a window the user "closed" is invisible but still alive — we
            // must still match and re-show it (exact match preferred, then host, then any
            // hidden group member).
            if (!target) {
                ...
                for (NSWindow *win in allWindows) {
                    ...
                    NSString *winUrl = wkView.URL.absoluteString ?: @"";
                    NSString *origUrl = [win valueForKey:@"blinkeredOriginalURLString"] ?: @"";
                    if ([winUrl isEqualToString:targetUrl] || [origUrl isEqualToString:targetUrl]) { exactMatch = win; break; }
                    if (!hostMatch) hostMatch = win;
                }
                target = exactMatch ?: (hostMatch ?: groupMatch);
            }
        ...
        // SAME already-allowed URL, so it can't be used to escape the lockdown. Matching mirrors
        // focusWindow (exact URL / original URL / host).
        NSString *targetUrl = body[@"url"];
        if (![targetUrl isKindOfClass:[NSString class]] || targetUrl.length == 0) return;
        dispatch_async(dispatch_get_main_queue(), ^{
            NSWindow *mainWin = [self valueForKey:@"mainBrowserWindow"];
            ...
            for (NSWindow *win in [[NSApplication sharedApplication].windows copy]) {
                ...
                if ([winUrl isEqualToString:targetUrl] || [origUrl isEqualToString:targetUrl]) { exactMatch = win; break; }
                NSString *origHost = [NSURL URLWithString:origUrl].host ?: @"";
                if (!hostMatch && ([[wkView.URL host] isEqualToString:targetHost] ||
                                   (targetHost.length && [origHost isEqualToString:targetHost]))) {
                    hostMatch = win;
                }
            }
            NSWindow *target = exactMatch ?: hostMatch;
            if (target) {
        ...
        // The home tab bar's ‹ / › back/forward buttons — move the matching site window through its
        // history. Same window-matching as reloadWindow; goBack/goForward are no-ops at the ends of
        // history, and history holds only already-allowed pages, so this can't escape the lockdown.
        NSString *targetUrl = body[@"url"];
...
- (void)blinkeredSessionMenuPicked:(NSMenuItem *)sender
{
    ...
    NSString *esc = [[itemId stringByReplacingOccurrencesOfString:@"\\" withString:@"\\\\"]
                      stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"];
    NSString *js = [NSString stringWithFormat:@"window.__blinkeredMenuPick && window.__blinkeredMenuPick('%@')", esc];
...
- (void)blinkeredProfileSwitch:(NSString *)action childId:(NSString *)childId url:(NSString *)navUrl webView:(WKWebView *)webView
{
    ...
    void (^navigate)(void) = ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *esc = [[navUrl stringByReplacingOccurrencesOfString:@"\\" withString:@"\\\\"]
                              stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"];
            [webView evaluateJavaScript:[NSString stringWithFormat:@"location.replace('%@')", esc] completionHandler:nil];
        });
    };
