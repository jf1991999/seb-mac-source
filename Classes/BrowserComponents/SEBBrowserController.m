...
static void BlinkeredClearLog(NSString *format, ...)
{
    ...
    // BARE os_log(), i.e. DEFAULT level — not os_log_info(). Confirmed from the shipping binary:
    // `log show --predicate 'process == "Blinkered"'`, the command MAC-B'-2 step 6 tells the operator
    // to run, returns Default and NOT Info without `--info`. At Info level these lines would be
    // invisible to the documented run while looking perfectly healthy in Console.app. The parity gate
    // pins this exact call, so switching the level turns it red — read that failure as "the device run
    // can no longer see these", not as a style nit.
