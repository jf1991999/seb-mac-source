                                 ...
                                 mainBrowserWindow:(BOOL)mainBrowserWindow temporaryWindow:(BOOL)temporaryWindow
{
    ...
    // Remember the URL this 🪟 window was opened with, so a site tab can be matched
    // back to its own window even after the page redirects (e.g. office.com login).
    if (url) newBrowserWindow.blinkeredOriginalURLString = url.absoluteString;
    // Stamp which window's page opened this one (0 = tab-opened). The home page uses this to
    // group a document window under its opener's site tab even when the doc is cross-host.
    newBrowserWindow.blinkeredOpenerWindowNumber = self.blinkeredPendingOpenerWindowNumber;
    self.blinkeredPendingOpenerWindowNumber = 0;
    ...
    // Blinkered: full-window site windows (Office, Canvas, BBC Bitesize…) are navigated via
    // the top tab bar and fill the screen below it. Make them BORDERLESS — like the main
    // window — so they have no title bar/traffic lights AND square corners that go right to
    // the screen edge (a titled window has small rounded corners). canBecomeKey/Main are
    // overridden to YES, so a borderless site window still works normally.
    if (!mainBrowserWindow && !temporaryWindow) {
        [newBrowserWindow setStyleMask:NSWindowStyleMaskBorderless];
        // A borderless kiosk window must never be movable: NSWindow defaults to
        // movable=YES even without a title bar, which leaves the window eligible
        // for the macOS 15+ system window-management actions (Window menu /
        // Fn-Ctrl shortcuts: Center, Fill, Move & Resize, tiling) — a kid could
        // displace the locked window with them.
        newBrowserWindow.movable = NO;
    }
    ...
    [newBrowserWindow setBackgroundColor:[NSColor whiteColor]];
    [newBrowserWindow.windowController showWindow:self];
    [newBrowserWindow makeKeyAndOrderFront:self];
    // Rig diagnostic for the tab window-group feature: names the new window and the window whose page
    // opened it (0 = the home page itself). The Windows build of this feature shipped once with opener
    // attribution silently broken and NOTHING in the log said so, which cost a release to diagnose.
    // Logged here rather than at stamp time because an NSWindow has no valid windowNumber until it has
    // been ordered in.
    DDLogInfo(@"[Blinkered] site window %ld opened for %@ (opener window %ld).",
              (long)newBrowserWindow.windowNumber,
              newBrowserWindow.blinkeredOriginalURLString ?: @"(none)",
              (long)newBrowserWindow.blinkeredOpenerWindowNumber);
                                    ...
                                    mainWebView:(BOOL)mainWebView
{
    ...
    NSView *webViewWrapper = [(NSView *)webView superview];
    if (webViewWrapper) {
...
{
    [self.sebController conditionallyLockExam:startURL.absoluteString configKey:self.configKey];
    ...
    // Launch cleanup #4/#5: the window is ordered front below before any content
    // exists — keep it black until the start page painted, so the launch reads as
    // covers -> content with no white pre-paint rectangle.
    [newBrowserWindow blinkeredHoldContentUntilFirstPaint];
    ...
    if (mainBrowserWindowShouldBeFullScreen) {
        ...
        // Fullscreen kiosk window: opt out of user/OS window management (the
        // macOS 15+ Window-menu Center/Fill/Move & Resize/tiling actions act on
        // any movable window, even a borderless one, and would push the tab-bar
        // header off-screen).
        self.mainBrowserWindow.movable = NO;
    }
...
// ── Blinkered split screen: end the split ────────────────────────────────────────────────
//
// TWO steps, in ONE order that is load-bearing rather than stylistic:
//
//   1. clear the role on every site window;
//   2. re-frame each of them — WHILE THEY ARE STILL VISIBLE.
//
// -setCalculatedFrame passes self.screen, and that is NIL for an ordered-out window, so the
// whole geometry body is skipped. Re-frame-then-hide works; hide-then-re-frame silently does
// nothing and leaves a half-width window that NOTHING later corrects — no Mac path re-frames
// a site window outside windowDidMove:/windowDidResize:, and setTopInset only writes a global.
// Every caller must therefore run this BEFORE any orderOut:.
//
// ALL roles clear together, not just the dying window's: a one-sided split is a state with no
// meaning, and clearing both removes the orphaned-survivor case without tracking partners.
//
// dyingWindow (may be nil) is skipped for the re-frame only — it is mid-teardown.
- (void) blinkeredClearSplitRolesDyingWindow:(NSWindow *)dyingWindow
{
    NSMutableArray *clearedWindows = [NSMutableArray array];
    for (NSWindow *win in [[NSApplication sharedApplication].windows copy]) {
        if (![win isKindOfClass:[SEBBrowserWindow class]]) {
            continue;
        }
        SEBBrowserWindow *browserWindow = (SEBBrowserWindow *)win;
        if (browserWindow.blinkeredSplitRole == SEBBlinkeredSplitRoleNone) {
            continue;
        }
        browserWindow.blinkeredSplitRole = SEBBlinkeredSplitRoleNone;
        [clearedWindows addObject:@(browserWindow.windowNumber)];
        if (browserWindow != dyingWindow) {
            [browserWindow setCalculatedFrame];
        }
    }
    if (clearedWindows.count > 0) {
        // Rig diagnostic: a split that ends without this line means a clear path was missed,
        // which is how the survivor gets stranded half-width.
        DDLogInfo(@"[Blinkered] split ended: cleared and re-framed window(s) %@ (dying window %ld).",
                  [clearedWindows componentsJoinedByString:@", "],
                  (long)(dyingWindow ? ((SEBBrowserWindow *)dyingWindow).windowNumber : 0));
    }
}
...
// Blinkered: release (or restore) audio in every open web view. Called when the locked device goes
// idle / active so macOS can idle-sleep instead of a media site's audio pinning it awake. Runs the
// page-side hooks injected in SEBAbstractModernWebView. Only silent audio is touched (see that script),
// so audible playback the child is using is never interrupted. Best-effort per view.
- (void)blinkeredSetWebViewAudioSuspended:(BOOL)suspended
{
    // When the locked device goes idle, RELEASE the pages' media so macOS's audio daemon (coreaudiod)
    // stops holding the Mac awake and it can idle-sleep. Use SEB's own tested path,
    // stopMediaPlaybackInAllWebViews → WKWebView.pauseAllMediaPlayback — NOT
    // stringByEvaluatingJavaScriptFromString:, a legacy WebKit1 API that silently does nothing on a
    // modern WKWebView (which is why the first attempt never released anything). pauseAllMediaPlayback
    // has no un-pause, so 'resume' is a no-op — the child just presses play again when they return.
    if (suspended) {
...
{
    // Hide every browser window in one pass BEFORE the (sequential) teardown, so
    // the session disappears as a single screen: the black covers behind stay up
    // until the app exits. Closing windows one by one visibly peeled the UI apart
    // on unlock (tab bar and site window vanishing at different moments).
    NSArray *openWindowDocuments = [[NSDocumentController sharedDocumentController] documents];
    for (SEBBrowserWindowDocument *openWindowDocument in openWindowDocuments) {
        [openWindowDocument.mainWindowController.window orderOut:self];
    }
    [self.mainBrowserWindow orderOut:self];
...
- (BOOL) isReconfiguringAllowedFromURL:(NSURL *)url
{
    if (![super isReconfiguringAllowedFromURL:url]) {
        // Blinkered is already running this session. A second .seb open here is a
        // harmless duplicate (e.g. the home agent re-opening the current session's
        // config) — the session is already up and working. Ignore it silently rather
        // than popping the disruptive "Loading New Settings Not Allowed" alert.
        self.startingUp = false;
        self.sebController.openingSettings = false;
        DDLogInfo(@"Blinkered: reconfiguring not allowed (already in a session) — ignoring the duplicate config open silently");
        return NO;
    } else {
        return YES;
    }
