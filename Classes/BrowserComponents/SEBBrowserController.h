...
@interface SEBBrowserController : NSObject <NSURLSessionTaskDelegate, WKScriptMessageHandler> {
...
// Blinkered per-child cookie isolation (shared-device focus sessions). When set, transferCookies
// restores ONLY this child's saved cookies (ignoring the global cookies.json) and exitSEB saves
// the session's cookies to this child's file instead of the global one.
@property (strong, nonatomic) NSString *_Nullable blinkeredFocusChildId;
// YES when blinkeredFocusChildId came from the .seb config on an ASSIGNED device (the server's
// blinkeredCookieMigrate key). Triggers the one-time copy of the legacy global cookies.json into that
// child's jar so they don't re-login. NEVER set by a runtime profileSwitch (a shared-device login), whose
// global jar may belong to a different kid.
@property (nonatomic) BOOL blinkeredCookieMigrateFromConfig;
// Guard so the "did the cookie store change hands?" wipe check runs ONCE per loaded config (the first
// transferCookies of a session), not on every new window. Re-armed whenever a NEW .seb loads — the app
// process & this controller survive across lock→unlock→lock, so a per-launch bool would never re-fire.
@property (nonatomic) BOOL blinkeredCookieOwnerChecked;
// The config key (org_safeexambrowser_configKey) under which blinkeredFocusChildId was last read from the
// .seb. When the live config key differs, a NEW config loaded → re-read the child + re-arm the owner check.
@property (strong, nonatomic) NSData *_Nullable blinkeredCookieConfigKey;
// The child id the LAST loaded .seb named (not a runtime picker child). A change here also marks a new
// session — belt-and-suspenders in case blinkeredFocusChildId is excluded from the config-key hash so two
// kids' shared-device configs would otherwise hash identically.
@property (strong, nonatomic) NSString *_Nullable blinkeredCookieConfigChild;
// Set when the web app is itself returning the user to student.html (a focus/class session's
// launching tab, or a lockdown's openDashboard). When set, applicationWillTerminateProceed must NOT
// also open student.html, or the kid gets a duplicate browser tab on exit.
@property (nonatomic) BOOL blinkeredWebReturn;
