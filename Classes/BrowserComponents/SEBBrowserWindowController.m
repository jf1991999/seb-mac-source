...
- (void)windowDidMove:(NSNotification *)notification
{
    ...
    dragStarted = false;
    [self blinkeredEnforceKioskFrame];
}


- (void)windowDidResize:(NSNotification *)notification
{
    DDLogDebug(@"BrowserWindow %@ did resize", self.window);
    [self blinkeredEnforceKioskFrame];
}
...
{
    DDLogWarn(@"BrowserWindow %@ was miniaturized", self.window);
    if ([self blinkeredIsKioskManagedWindow:self.browserWindow]) {
        // No legitimate path miniaturizes a kiosk window (no Minimize menu item,
        // no miniaturizable styleMask bit, Cmd+M unbound) — but an AX-driven
        // miniaturize would drop the lock UI into the hidden Dock with nothing
        // to restore it. Deminiaturize on the next runloop turn rather than
        // synchronously from inside the notification.
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.browserWindow deminiaturize:nil];
        });
    }
}


// A kiosk-managed browser window is borderless (neither Titled nor Resizable
// styleMask bits): the windows Blinkered uses when locked/focused (main + site
// windows). A borderless window has no title bar, so there is no legitimate
// user-move path for it — any move/resize/miniaturize is OS window management.
// Titled windows (unlocked browsing, windowed exam mode / Focus Time) are
// legitimately movable — leave them alone.
- (BOOL)blinkeredIsKioskManagedWindow:(SEBBrowserWindow *)browserWindow
{
    return browserWindow != nil
        && (browserWindow.styleMask & (NSWindowStyleMaskTitled | NSWindowStyleMaskResizable)) == 0;
}


// Self-heal for the kiosk frame: a fullscreen-kiosk browser window (borderless
// main or site window) has exactly one legitimate frame — the calculated one.
// The macOS 15+ window-management actions (Window menu / Fn-Ctrl shortcuts:
// Center, Fill, Move & Resize, tiling) can move such a window programmatically,
// bypassing the mouse-drag watcher, and push the tab-bar header off-screen.
// Prevention (window.movable = NO) closes the known vectors; this snaps the
// window straight back for any window-management path we didn't anticipate.
// No loop: setCalculatedFrame's setFrame: is a no-op (posts no move/resize
// notifications) once the frame is already correct, and the re-entrancy flag
// guards the correcting pass itself.
- (void)blinkeredEnforceKioskFrame
{
    if (blinkeredReassertingKioskFrame) {
        return;
    }
    SEBBrowserWindow *browserWindow = self.browserWindow;
    if (![self blinkeredIsKioskManagedWindow:browserWindow]) {
        return;
    }
    blinkeredReassertingKioskFrame = YES;
    if (browserWindow == _browserController.mainBrowserWindow) {
        // Main browser window: use the 3-arg form with mainBrowserWindow:YES so
        // the frame is computed for the main screen — plain setCalculatedFrame
        // would pin the window full-screen to whatever display a programmatic
        // move landed it on. Site windows keep their current screen (wrong-
        // screen recovery stays with windowDidChangeScreen / the covers).
        [browserWindow setCalculatedFrameOnScreen:browserWindow.screen mainBrowserWindow:YES temporaryWindow:NO];
    } else {
        [browserWindow setCalculatedFrame];
    }
    blinkeredReassertingKioskFrame = NO;
}
...
- (void)windowWillClose:(NSNotification *)notification
{
    ...
    // Blinkered split screen: the PROGRAMMATIC close paths (chip x, closeAdditionalWindows,
    // JS window.close). The child's own red button / Cmd-W does NOT come here — it hides via
    // -windowShouldClose:, which clears roles itself. Both hooks are needed; neither covers
    // the other. Runs before the window leaves NSApp.windows, so the helper skips it by
    // identity rather than re-framing a window mid-teardown.
    // Same guard as -windowShouldClose:: only a window that HOLDS a role ends the split. Closing a
    // document (chip x, JS window.close) must leave the pairing alone.
    if ([self.window isKindOfClass:NSClassFromString(@"SEBBrowserWindow")] &&
        ((SEBBrowserWindow *)self.window).blinkeredSplitRole != SEBBlinkeredSplitRoleNone) {
        [self.browserController blinkeredClearSplitRolesDyingWindow:self.window];
    }
