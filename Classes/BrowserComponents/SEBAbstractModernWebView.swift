...
@objc public class SEBAbstractModernWebView: NSObject, SEBAbstractBrowserControllerDelegate, SEBAbstractWebViewNavigationDelegate, WKScriptMessageHandler {
    ...
    public var wkWebViewConfiguration: WKWebViewConfiguration {
        ...
        // Blinkered: expose whether macOS Accessibility is granted, so the locked home
        // page can report it to the parent dashboard (no prompt — read-only check).
        let accessibilityGranted = AXIsProcessTrusted()
        // Blinkered: whether the current macOS user is an administrator — i.e. a member of the
        // admin group (gid 80). The app runs as the user, so its own group list reflects the user's.
        // An admin user can remove Blinkered, so the dashboard warns + suggests a Standard account.
        let isAdminUser: Bool = {
            let count = getgroups(0, nil)
            if count <= 0 { return false }
            var groups = [gid_t](repeating: 0, count: Int(count))
            if getgroups(count, &groups) < 0 { return false }
            return groups.contains(80)
        }()
        // Blinkered — THE CAPABILITY DECLARATION. `window.__blinkeredNative` states what THIS BUILD
        // understands, so the page can gate a control on what the native in front of it can actually
        // do instead of on version arithmetic. Read the long note in Content/Blinkered.js
        // (blinkered-win) for the full contract; the essentials:
        //
        //   • It rides a DOCUMENT-START injection on both platforms, so it is present before the page
        //     renders anything. Here that is the WKUserScript below; on Windows it is the bridge shim
        //     (Page.addScriptToEvaluateOnNewDocument), NOT Api.js — Api.js assigns `SafeExamBrowser`
        //     wholesale over an IPC a warm-cache page can outrun, which is why this cannot live beside
        //     `masterCodeBridge` and be trusted at first render.
        //   • It is a SEPARATE GLOBAL for that same reason: nothing another injection assigns can
        //     clobber it.
        //   • ABSENT MEANS UNSUPPORTED. An older build declares nothing and the page hides the
        //     control. Never the other way round.
        //   • It gates PAGE UI, never enforcement. The declaration is client-side and therefore
        //     forgeable; all a forged entry buys is a button that posts a message this build has no
        //     handler for (which the bridge logs and drops). Nothing that decides what a child is
        //     allowed to do may ever be gated on it.
        //
        // Adding a verb? Add it here. OMITTING one is safe (the page hides the control); listing one
        // this build does not implement is not.
        let jsCapabilities = [
            "focusWindow",              // surface a site window by id or URL
            "focusWindow.reopen",       // ... and its reopenIfMissing option (was: version >= 3.6.24)
            "closeWindow",              // close ONE site window by id (the x on a window chip)
            "clearSiteWindows",         // hide every site window, keep-alive
            "closeAdditionalWindows",   // close every site window
            "reloadWindow",             // reload the active site window (was: version >= 3.6.92)
            "navigateWindow",           // back/forward in a site window (was: version >= 3.6.95)
            "showSessionMenu",          // native NSMenu over the site (was: version >= 3.6.112)
            "setTopInset",              // reserve the tab bar's height
            "masterCodeExit",           // on-device Master Exit Code validation
            "setSplit",                 // put two site windows side by side, 50/50 (and end it)
        ...
        window.SafeExamBrowser = {
          ...
          isAdmin: \(isAdminUser),
          // Blinkered §14.2 item 6 — THE CAPABILITY ANNOUNCE. This build can validate the Master Exit
          // Code on the device (the `masterCodeExit` bridge message), so the lock page may route its
          // master-code box through the bridge instead of the server. Announced as a plain constant
          // because it is a property of the BUILD, not of any session: a page that finds it absent —
          // every build before this one — keeps the Phase-1 server field and behaves exactly as it
          // does today. The page ALSO requires a live server-side masterDoor flag and applies its own
          // timeout before trusting this, so a stale or forged announce cannot strand a parent.
          masterCodeBridge: true,
          security: {
            browserExamKey: '',
            configKey: '',
        ...
        window.__blinkeredNative = { contract: 1, capabilities: [\(jsCapabilities)] }
"""
...
//            userContentController.addUserScript(controlSpellCheckUserScript)
        ...
        // Re-register the "blinkered" handler (getOpenWindows / focusWindow). We build a
        // fresh user content controller above, which drops the handler the shared config
        // had — without this, window.webkit.messageHandlers.blinkered is undefined in the
        // page and 🪟 site tabs reopen instead of switching back.
        // `?? nil` flattens the double-optional from the @optional protocol property.
        // removeScriptMessageHandler first so a duplicate "blinkered" can never raise
        // the "handler already exists" exception (which crashes the whole app).
        if let blinkeredHandler = navigationDelegate?.blinkeredScriptMessageHandler ?? nil {
            userContentController.removeScriptMessageHandler(forName: "blinkered")
            userContentController.add(blinkeredHandler, name: "blinkered")
        }
    ...
    // 2.0 — SAY WHICH BRANCH A REFRESH TOOK, AND WHAT SCHEME IT COMMITTED TO.
    //
    // Record: docs/WHITE_LOCK_SCREEN_REMEDIATION_PLAN.md Stage 0, incident
    // docs/WHITE_LOCK_SCREEN_INCIDENT_2026_09_08.md §10.3.
    //
    // On 8 September a child pressed ⌘R on the compiled-in offline cover. The cover is rendered with
    // `loadHTMLString(html, baseURL: about:blank)`, so `sebWebView.url` is `about:blank`, and the
    // first branch below navigated there: a white window, no chrome, and no way back to the cover.
    // THE APP LOG SAID NOTHING. Its complete record of that refresh is three lines, none naming a
    // destination and all byte-identical to a healthy refresh — `Reloading current webpage`
    // (SEBBrowserWindow.m, no URL), `reload()`/`load(_:)` (silent), and `decidePolicyFor
    // navigationAction: Target frame nil, URL: nil` (nil for EVERY main-frame navigation, because
    // `currentFrame` is nilled one line above it; the real destination is `navigationAction.request.url`,
    // bound on the next line and never logged). Everything downstream that proposes to ship this log
    // depends on it naming the fault, and until this line it did not.
    //
    // THE BRANCH AND THE SCHEME, NEVER THE URL. `run-retry-navigation-test.sh:197` fails the build on
    // a recovery URL handled as a value, and its rule is the right one here: the home start URL
    // carries the DEVICE TOKEN, and a log line is exactly where a credential gets copied into a
    // support ticket. The agent scrubs http(s) URLs out of the log it uploads, but that is not the
    // argument for this line — the argument is that a token must not be written down in the first
    // place. A scheme is not a URL: it distinguishes the fault (`about`) from a healthy homework
    // refresh (`https`) and from a cover-scheme commit (`blinkered-offlinecover`) while carrying no
    // host, no path and no query.
    public func reload() {
        previousZoomLevel = 1
        ...
        WKWebsiteDataStore.default().removeData(ofTypes: websiteDataTypes as! Set<String>, modifiedSince:NSDate.distantPast, completionHandler:{
            if let url = self.sebWebView.url {
                DDLogInfo("Blinkered reload: branch=committed scheme=\(url.scheme ?? "none")")
                self.load(url)
            } else {
                if let currentPageURL = self.navigationDelegate?.currentURL {
                    if currentPageURL != nil {
                        DDLogInfo("Blinkered reload: branch=delegate-current scheme=\(currentPageURL!.scheme ?? "none")")
                        self.load(currentPageURL!)
                    } else {
                        DDLogInfo("Blinkered reload: branch=none scheme=none")
                    }
                } else {
                    DDLogInfo("Blinkered reload: branch=none scheme=none")
                }
...
//        searchSessionIdentifiers()
    ...
    public func webView(_ webView: WKWebView,
        ...
        // Forward the COMMIT, not just the finish: the window needs to know the moment the main
        // frame holds a document, so it can drop the empty-content backdrop (§5 C7′). didFinish
        // can be arbitrarily later on a page with a hung subresource.
        navigationDelegate?.sebWebViewDidCommitLoad?()
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        if !url.hasDirectoryPath && ((allowDownloads && !WKDownloadSupported) || (urlIsPDF && (self.downloadFilename ?? "").isEmpty)) {
            // This interpolated a page-chosen URL into JS with NO escaping at all, into a doubly
            // nested context at that: a CSS attribute-selector string inside a JS string literal.
            // Not currently exploitable — WebKit percent-encodes `"` in absoluteString, and this
            // targets the navigating webview rather than the shell — but it is reachable inside a
            // lock (a kid opening a PDF on an allowed site) and no amount of escaping makes
            // building a selector by concatenation a good idea.
            //
            // So: pass the URL as a JS VALUE through the one escaping helper, and let JS do the
            // matching. Semantics are preserved exactly — first element whose href ATTRIBUTE equals
            // the absolute URL, then its `download` property, undefined if there is no match, which
            // `result as? String` reads as nil just as the optional chain did.
            let urlLiteral = SEBAbstractWebView.blinkeredJSStringLiteral(url.absoluteString)
            let downloadAttributeScript = """
                (function (u) {
                    try {
                        var els = document.querySelectorAll('[href]');
                        for (var i = 0; i < els.length; i++) {
                            if (els[i].getAttribute('href') === u) { return els[i].download; }
                        }
                    } catch (e) {}
                    return undefined;
                })(\(urlLiteral))
                """
            webView.evaluateJavaScript(downloadAttributeScript) {(result, error) in
                if error == nil {
        ...
        if navigationAction.targetFrame == nil {
            ...
            if newNavigationAction?.policy == SEBNavigationActionPolicyJSOpen {
                ...
                if openedAbstractWebView != nil { // Excluding special case: Open in same window
                    ...
                    // Blinkered: set the clean custom UA on the new window's customUserAgent PROPERTY
                    // (which is what navigator.userAgent returns), not just the request header. Without
                    // this, a site window's navigator.userAgent is the bare WebKit string with no
                    // "Safari" token, so JS-based browser checks (e.g. Canva) show "update your browser"
                    // even though the request header is a clean Safari UA.
                    let newWindowUA = newAbstractWebView.customSEBUserAgent
                    if !newWindowUA.isEmpty {
                        newWKWebView?.customUserAgent = newWindowUA
                    }
                    // Blinkered: two-finger trackpad swipe = back/forward, like a normal browser.
                    // Only on these site windows (not the main wrapper, which has no history).
                    // Safe in a lockdown: history only contains pages that already passed the
                    // allow-list filter, so a swipe can never reach a blocked site.
                    newWKWebView?.allowsBackForwardNavigationGestures = true
                    return newWKWebView
                }
            }
#endif
