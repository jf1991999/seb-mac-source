...
// Transient opener handoff: set by SEBBrowserWindow just before it forwards a page-initiated
// window.open/target=_blank to openNewTabWithURL:, consumed (and reset) by
// openAndShowWebViewWithURL: to stamp blinkeredOpenerWindowNumber on the new window.
// Main-thread only, so a plain property is safe.
@property (nonatomic) NSInteger blinkeredPendingOpenerWindowNumber;
...
- (void)blinkeredSetWebViewAudioSuspended:(BOOL)suspended;   // Blinkered: idle → release web audio so the Mac can sleep
...
// Blinkered split screen: end the split. Pass the window being torn down, or nil.
