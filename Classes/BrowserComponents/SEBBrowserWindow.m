...
+ (void) blinkeredAddBrandBackdropContentToView:(NSView *)view
{
    ...
    NSTextField *label = [NSTextField labelWithString:NSLocalizedString(@"Locked by Blinkered", @"")];
    ...
    // Centre the icon-over-label block; flexible margins keep it centred across resizes and the
    // differing cover/browser window geometries.
    NSRect b = view.bounds;
    CGFloat blockHeight = iconSide + gap + NSHeight(label.frame);
    CGFloat labelY = NSMidY(b) - blockHeight / 2;
    label.frame = NSMakeRect(NSMidX(b) - NSWidth(label.frame) / 2, labelY,
                             NSWidth(label.frame), NSHeight(label.frame));
    iconView.frame = NSMakeRect(NSMidX(b) - iconSide / 2, labelY + NSHeight(label.frame) + gap,
                                iconSide, iconSide);
...
- (void) blinkeredHoldContentUntilFirstPaint
{
    ...
    // identical in both branches (review condition 5). Fail-toward-black: no session info yet
    // (e.g. very first lock, file not written) simply keeps today's behaviour.
    BOOL homeLock = [self.browserController.sebController blinkeredHomeSessionInfo] != nil;
