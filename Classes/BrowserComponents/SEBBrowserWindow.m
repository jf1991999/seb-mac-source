...
- (void) blinkeredHoldContentUntilFirstPaint
{
    ...
    [self performSelector:@selector(blinkeredRevealContent) withObject:nil afterDelay:blinkeredContentRevealSafetyTimeout];
}
...
- (void) blinkeredInstallEmptyContentBackdropIfNeeded
{
    ...
    if (_blinkeredContentCommitted) return;                    // signal 1: a document committed
    if (self.webView.url.absoluteString.length > 0) return;    // signal 2: the frame holds a URL
...
// C4 (review R6.3). The installer reads the webview's URL exactly ONCE, at reveal, and nothing
// re-reads it — so any failure of the removal path is permanent, and because this view sits above
// the content view it also swallows every mouse event over the window. A cheap re-read on the
// paint timer closes that without suppressing anything: it is a plain observation, it changes no
// flag the recovery paths consult, and it cannot fire on a device that has nothing to show.
//
...
{
    if (!_blinkeredEmptyContentBackdrop) return;
    if (self.webView.url.absoluteString.length == 0) return;   // still nothing to show — stay up
