...
- (BOOL)windowShouldClose:(id)sender
{
    ...
    // ONLY when this window is actually half of a split. The clear used to fire for ANY site
    // window closing, so closing a DOCUMENT from its chip x ended the split — the document has
    // nothing to do with the pairing. Reported from the field before anyone managed to open one.
    if (self.blinkeredSplitRole != SEBBlinkeredSplitRoleNone) {
...
- (void)reload
{
    if (self.webView.isReloadAllowed) {
        // NO APP-MODAL ALERT WHILE A LOCK IS ON SCREEN.
        //
        // `runModalAlert:` reaches `[alert runModal]` on macOS 12+ — an APP-MODAL NESTED RUN LOOP,
        // the pattern already characterised in this repo as MAJOR: with one up, 12-13 recovery
        // navigations produced ZERO commits against 137 HTTP 200s, and it swallows ⌘Q. Over a kiosk
        // lock that is not a nuisance, it is a containment failure — it stalls the recovery rail AND
        // takes away the parent's escape hatch, on a screen a child cannot leave.
        //
        // DEVICE-PROVEN 13 Sep 2026 on Kid's Mac, 3.6.222, on the offline cover: a right-click →
        // Reload produced
        //     Reloading current browser window: <SEBBrowserWindow>
        //     Show Reload Current Page warning.
        //     Adding modal alert window <_NSAlertPanel>
        //     BrowserWindow did resign key / Current key window is modal panel
        // It is reachable fleet-wide: showReloadWarning defaults YES and NO live .seb sets it.
        //
        // WHAT IS SUPPRESSED IS THE WARNING, NOT THE RELOAD. The `else` branch below already calls
        // -unconditionallyReload, so this cannot silently stop the flow — the failure mode of the
        // two fixes that were rejected. It is deliberately NOT fixed in `runModalAlert:` (51
        // callers, and beginSheetModalForWindow: on a non-visible window never calls its completion
        // handler, which would convert ~51 flows into silent stops), and NOT by switching this one
        // to a sheet (that permanently promotes the browser window to NSMainMenuWindowLevel+6,
        // where the SEB Dock and the password dialog live).
        //
        // The warning exists to stop a child losing form state to a stray reload. On a locked
        // screen that trade goes the other way: a wedged lock and a swallowed ⌘Q cost more than a
        // re-typed form, and the cover itself has no state to lose at all.
        // capWindows holds the kiosk COVER SCREENS and nothing else. The lockdown alert and the
        // modal dimmer live in separate arrays — lockdownWindows (SEBController.m:10579,
        // coveringWindowLockdownAlert) and lockModalWindows (:5990, :10688,
        // coveringWindowModalAlert). This is also the house predicate for "a kiosk is up":
        // -runModalAlert: itself branches on capWindows.count > 0.
        //
        // A NOTE BECAUSE THIS COMMENT WAS ONCE WRONG IN BOTH DIRECTIONS. An earlier version of it
        // said capWindows "also holds the red lockdown-alert and the modal dimmer covers", citing
        // SEBController.m:2952 — which is itself a comment, and is itself wrong (its likely origin
        // is -changeWindowLevels:, which declares `CapWindow *capWindow` at :11310 while iterating
        // self.capWindows, a mistyped loop variable ObjC never checks). Trusting it turned an
        // accurate comment here into an inaccurate one. Read the three @property declarations at
        // SEBController.h:316-319 before believing any prose about which array holds what.
        //
        // IT SUPPRESSES THE WARNING ON EVERY LOCKED PAGE, not only the cover, and that is
        // deliberate. The reason is NOT "the cover has no state to lose" — that is true only of the
        // cover. It is that the modal's cost applies on every locked page: an app-modal nested run
        // loop stalls the recovery rail and swallows ⌘Q on a screen a child cannot leave. A stuck
        // content page under lock is exactly where a child reaches for Reload, so the narrow
        // predicate would remove the guard where it is most needed.
        //
        // The warning only ever intercepts a DELIBERATE gesture: every entry point to this method is
        // one — the ⌘R/menu action, SEBBrowserWindowController.m:607, and the Dock button. The
        // recovery rail cannot raise it on itself: -blinkeredReloadWindow: (SEBController.m:3738)
        // calls -[SEBAbstractWebView reload], whose delegate is assigned downward to the concrete
        // web view (SEBAbstractWebView.m:605).
        BOOL lockOnScreen = self.browserController.sebController.capWindows.count > 0;
        if (self.webView.showReloadWarning && !lockOnScreen) {
            ...
            // Reload page without displaying warning — either because the config says so, or
            // because a lock is on screen and an app-modal alert there is worse than the reload.
            if (lockOnScreen && self.webView.showReloadWarning) {
                DDLogInfo(@"Reload warning SUPPRESSED — a lock is on screen (%lu cap window(s)); an app-modal alert there stalls the recovery rail and swallows ⌘Q.", (unsigned long)self.browserController.sebController.capWindows.count);
            }
            [self unconditionallyReload];
        }
    }
...
// allowBrowsingBackForward is NO, which a lock session is, and that
...
//  * FIX 2's RETRY GATE (G3, plan §4.5 condition 1) — a true answer ISSUES A NAVIGATION. That is a
//    third call site with a third safe direction, so it gets its own predicate rather than a
//    negation of either of the two above, which would silently inherit the wrong fail-safe:
//      !MayHoldContent      == Absent  — the same test, but reading it as "not(may hold)" invites a
//                                        later editor to "simplify" it to !DefinitelyHolds, which
//                                        would make Unknown FIRE.
//      !DefinitelyHolds     == Absent OR Unknown — and Unknown here means a navigation over a page
//                                        we could not rule out, i.e. reloading a working lock page
//                                        under a child on a 30-second timer. Never that.
//    Same signal, not a second notion of "has content" — the condition is explicit that Fix 2 must
//    reuse this one and invent nothing.
- (BOOL) blinkeredWebViewDefinitelyHoldsNoCommittedDocument
{
    return [self blinkeredCommittedDocumentSignal] == BlinkeredCommittedDocumentAbsent;
}
...
- (void)sebWebViewDidCommitLoad
{
    ...
    [self blinkeredDropEmptyContentBackdrop];
    // Fix 2 (OFFLINE_RETRY_DEAD_END_PLAN §4). A PROMPT, not a verdict: it carries no claim about
    // WHICH window committed, because the 19 Aug rig run tagged both of its commits "(site window)"
    // and settled that isMainBrowserWindow is not reliable here (§6.2 anomaly). SEBController's
    // observer re-reads the main window's own commit signal and disarms only on that, so a site
    // window committing cannot switch off the retry the lock shell still needs — and a MISSED
    // notification costs nothing either, because the retry's own fire path disarms on the same
    // signal. It only makes the disarm PROMPT, which is what keeps an armed retry from firing into
    // a navigation the child started.
    [[NSNotificationCenter defaultCenter] postNotificationName:@"BlinkeredContentCommitted" object:self];
}
