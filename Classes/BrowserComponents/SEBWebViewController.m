...
// Invoked when the main frame's document has COMMITTED — there is now something in the webview,
// which is not the same as the load having finished. SEBBrowserWindow drops its empty-content
// backdrop here (OFFLINE_RETRY_DEAD_END_PLAN §5 C7'/C4): on this legacy path in particular,
// "commits and paints but never finishes" is a real page state, and waiting for the finish would
// leave a rendered page behind an opaque curtain indefinitely.
- (void)webView:(SEBWebView *)sender didCommitLoadForFrame:(WebFrame *)frame
{
    if (frame == [sender mainFrame]) {
...
- (NSURLRequest *)webView:(SEBWebView *)sender resource:(id)identifier willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)redirectResponse fromDataSource:(WebDataSource *)dataSource
{
    ...
    NSString *absoluteRequestURLTrimmed = [[absoluteRequestURL stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"/"]] lowercaseString];
