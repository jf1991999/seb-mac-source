...
// ┌──────────────────────────────────────────────────────────────────────────────────────────────┐
// │ §6 TAHOE DEVICE TEST: RUN 21 Aug 2026. Gate raised 0 -> 1 (LOG ONLY). 1 -> 2 IS NOT CLEARED. │
// └──────────────────────────────────────────────────────────────────────────────────────────────┘
//
// Ran on a MacBook Air, macOS 26.6.2 (real Tahoe). A WKWebView harness mirroring
// blinkeredTrustedQuitNavigation: read navigationAction.sourceFrame.securityOrigin
// UNCONDITIONALLY — a superset of the production path, which short-circuits before that read —
// across: baseline load; same-origin 302 redirect hop (both hops); cross-origin 302 redirect hop;
// sub-frame self-navigation; and session restore via -interactionState into a fresh webview
// (navType=backForward). NO TRAP ON ANY. Teeth proven: injecting a fake trap at the read point
// killed the process, so the clean pass is a real negative and not a harness that never ran.
// Caveat recorded rather than hidden: origins were http/127.0.0.1, not https/blinkered.com.au.
// The trap is about frame-object NULLABILITY — a navigation-shape property — not the domain.
//
// The window.open / targetFrame==nil case (a)/(b) was NOT re-triggered in that harness. It is
// covered instead by code and config, which is stronger than it sounds:
//   - the predicate returns before the sourceFrame read when targetFrame is nil (see below), and
//   - the ALREADY-SHIPPED URL-filter guard further down this file does exactly that read on every
//     main-frame navigation in a lock, INCLUDING window.open (that is how the research lane opens
//     its browser), and has not crashed in the field;
//   - the synthesized SEBWKNavigationAction the comment below used to worry about is unreachable
//     in Blinkered: it is created only by SEBWebViewController, i.e. the CLASSIC WebView, and the
//     macOS default browserWindowWebView = webViewSelectForceModernInForeignNewTabs with
//     sendBrowserExamKey = NO selects the MODERN WebView for every tab. Blinkered's server-issued
//     .seb sets neither key, so both take those defaults. Confirm on device from the one log line
//     this decision already emits: "Opening modern WebView".
//
...
//
//   0 = OFF        no predicate is computed. Pre-21-Aug-2026 behaviour exactly.
//   1 = LOG ONLY   compute, log and report a refusal, but still quit. Requires the §6 test. <- NOW
...
//
// AT 1 THE HOLE IS STILL OPEN. Log-only still quits. What this release buys is the ONE piece of
// evidence gate 2 requires and cannot be reasoned into: whether the predicate ever refuses a
// LEGITIMATE unlock in the field. Watch for "Blinkered SECURITY: /seb-quit from an UNTRUSTED
// navigation" against the eight legitimate quit flows (L1-L8). Zero of those over a real release
// is the precondition for 2. Any of them is a design bug found before it could strand a child.
//
// The §6 test, as originally specified (kept for the 1 -> 2 review): on Tahoe, inside a real lock, confirm targetFrame is
...
#define BLINKERED_QUIT_FRAME_GATE 1
...
// later failed belongs back where it was. Emptiness is not enough of a test, and the reason is worth
// keeping even though the specific example has changed: a `committed ?: configured` rule would send a
// parent's Retry to whatever non-empty thing the webview happened to hold. It USED to hold
// about:blank, because the offline cover was injected with loadHTMLString — a white window with
// chrome, precisely the artifact reported. The cover is now SERVED from
// blinkered-offlinecover://<untilMs> (SEBOSXWKWebViewController), so its committed URL is a real one
// and Retry on a cover would now be sent back to the cover, correctly. data:, file: and any future
// non-https state still fall out for the same reason, so the https+host test stands unchanged. The
...
{
    NSString *safeReason = (reason.length ? reason : @"?");
    NSString *safePath = (url.path.length ? url.path : @"?");
    NSString *key = [NSString stringWithFormat:@"%@|%@", safePath, safeReason];
    ...
    [self blinkeredPostControlURLRefused:safeReason path:safePath attempt:1];
}

// [B1] DELIVERY IS CHECKED AND RETRIED. This used to be one fire-and-forget
// `[[NSURLSession sharedSession] dataTaskWithRequest:req] resume]` with no completion handler,
// followed by a DDLog saying "reported" — which claimed the action before the no-op was ruled out.
// That is this workstream's most-repeated lesson, and it is the same shape the 3.6.198 device rig
// caught in SEBController's reporter (see tools/lockdown-tests/device-security-event-types.txt).
//
// WHY IT MATTERS MORE HERE THAN ANYWHERE ELSE. This event is the ONLY evidence the /seb-quit frame
// gate soak produces, and the gate 1 -> 2 decision turns on the ABSENCE of it. A refusal lost to an
// outage, a captive portal, an SSL-intercepting school network, a 5xx or a rotated-token 403 is
// indistinguishable from "the predicate never refused anything" — which is the conclusion that
// would license enforcing. A dropped report here does not lose a diagnostic; it manufactures the
// wrong answer. See blinkered-classroom docs/DEVICE_TELEMETRY_DURABLE_PLAN.md §4.1 [B1].
//
// The 300 s throttle above is left exactly as it was and deliberately NOT cleared on failure: it
// bounds how often a NEW report is started, so a continuously-refusing device during a long outage
// cannot turn this into a POST flood. The retries below bound the delivery of a report already
// begun. Bounded and in-memory: a teardown loses a pending retry, which is accepted — the
// alternative is on-disk queueing inside a locked browser, for a soak signal.
+ (void)blinkeredPostControlURLRefused:(NSString *)reason path:(NSString *)path attempt:(NSInteger)attempt
{
    static const NSInteger kMaxAttempts = 5;
    // 15 s, 60 s, 120 s, 240 s — a short blip and a longer one, ~7 minutes total.
    static const NSTimeInterval kBackoff[] = { 15.0, 60.0, 120.0, 240.0 };
    ...
    // [B2] These used to return SILENTLY. An unpaired, reinstalled or orphaned device refused
    // invisibly, and its silence was then counted as "no refusals". Say so in the log instead: it is
    // the only place a device outside the reporting population can be seen at all.
    if (![creds isKindOfClass:[NSDictionary class]]) {
        DDLogWarn(@"Blinkered: control_url_refused (%@, %@) CANNOT be reported — no readable agent.json. "
                  @"This device refuses invisibly and is outside the soak population.", path, reason);
        return;
    }
    ...
    if (![devId isKindOfClass:[NSString class]] || ![devTok isKindOfClass:[NSString class]]) {
        DDLogWarn(@"Blinkered: control_url_refused (%@, %@) CANNOT be reported — agent.json carries no "
                  @"usable id/token. This device refuses invisibly.", path, reason);
        return;
    }
    NSURL *endpoint = [NSURL URLWithString:[NSString stringWithFormat:@"%@/api/home/devices/%@/security-event", server, devId]];
    if (!endpoint) {
        DDLogWarn(@"Blinkered: control_url_refused (%@, %@) CANNOT be reported — bad server URL %@", path, reason, server);
        return;
    }
    NSMutableURLRequest *req = [NSMutableURLRequest requestWithURL:endpoint];
    req.HTTPMethod = @"POST";
    [req setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    // [F1] `reason` now travels. All three call sites of this method post the SAME type, so without
    // it the server cannot tell the /seb-quit frame gate ('untrusted-quit-navigation') from the
    // §3.3 start-URL anchor ('not-start-url') — which makes any NON-ZERO count unreadable. The
    // strings are pinned on both sides by hand: tools/lockdown-tests/device-control-url-reasons.txt
    // here, docs/device-control-url-reasons.json there. A one-character drift lands every real
    // refusal in the server's 'other' bucket, alongside junk.
    req.HTTPBody = [NSJSONSerialization dataWithJSONObject:@{ @"token": devTok,
                                                              @"type": @"control_url_refused",
                                                              @"reason": reason }
                                                   options:0 error:nil];
    if (!req.HTTPBody) {
        DDLogWarn(@"Blinkered: control_url_refused (%@, %@) CANNOT be reported — body would not serialise", path, reason);
        return;
    }
    [[[NSURLSession sharedSession] dataTaskWithRequest:req
        ...
        if (delivered) {
            DDLogInfo(@"Blinkered: control_url_refused (%@, %@) DELIVERED on attempt %ld", path, reason, (long)attempt);
            return;
        }
        if (attempt < kMaxAttempts) {
            NSTimeInterval delay = kBackoff[MIN((NSUInteger)attempt - 1, (NSUInteger)3)];
            DDLogWarn(@"Blinkered: control_url_refused (%@, %@) did not land (status %ld, error %@) — "
                      @"retrying in %.0fs (attempt %ld/%ld)", path, reason, (long)status,
                      error.localizedDescription ?: @"none", delay, (long)attempt + 1, (long)kMaxAttempts);
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [SEBAbstractWebView blinkeredPostControlURLRefused:reason path:path attempt:attempt + 1];
            });
            return;
        }
        // GIVING UP IS ITSELF THE FINDING. Anyone reading a zero out of the server must be able to
        // find this line and know the zero is incomplete for this device and window.
        DDLogError(@"Blinkered: control_url_refused (%@, %@) GAVE UP after %ld attempts (status %ld, error %@). "
                   @"The server has NOT been told about this refusal — a zero on the server is incomplete.",
                   path, reason, (long)kMaxAttempts, (long)status, error.localizedDescription ?: @"none");
    }] resume];
    DDLogInfo(@"Blinkered: control_url_refused (%@, %@) dispatched (attempt %ld)", path, reason, (long)attempt);
}
