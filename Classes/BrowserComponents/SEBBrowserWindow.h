...
/// Fix 2 (OFFLINE_RETRY_DEAD_END_PLAN §4.5 condition 1). "Nothing has committed", POSITIVELY
/// established — the same commit-derived signal the empty-content backdrop reads, exposed with the
/// third safe direction rather than a second notion of "has content". A true answer here TRIGGERS a
/// navigation, so Unknown must read as NO: navigating over content we could not rule out would
/// reload a working lock page under a child. See the note above -blinkeredWebViewMayHoldContent.
- (BOOL) blinkeredWebViewDefinitelyHoldsNoCommittedDocument;
/// Companion, already used by the backdrop re-check: a document is POSITIVELY committed.
- (BOOL) blinkeredWebViewDefinitelyHoldsCommittedDocument;
/// "present" / "absent" / "unknown" for logging. Exposed for impl-review R3-F5: the commit-prompt
/// disarm must be able to report the MAIN window's signal on BOTH paths, because the same value
/// logged from -sebWebViewDidCommitLoad is a different window's read.
