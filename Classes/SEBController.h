...
// Blinkered atomic teardown (MAC_ATOMIC_TEARDOWN_PLAN.md): YES once exitSEB has begun. Read-only
// outside SEBController — the bridge's `quit` case uses it to dedupe the page's double trigger
// (bridge quit + the 2 s /seb-quit fallback). Defined in SEBController.m.
BOOL BlinkeredTeardownStarted(void);
...
@interface SEBController : NSObject <NSApplicationDelegate, SEBLockedViewControllerDelegate, ProcessListViewControllerDelegate, AssessmentModeDelegate, ServerControllerDelegate, ServerLoggerDelegate, SEBDockItemButtonDelegate, ScreenProctoringDelegate, SPSControllerUIDelegate, SPUUpdaterDelegate>
{
...
// The active home session's identity ({id, token, base, sessionId}), or nil when not in a home
// lock. Exposed (Proposal B, MAC_WAKE_EDGE_RECOVERY_PLAN.md §2) as THE home-lock predicate the
// brand backdrop is scoped by — exam/class sessions have no home_session.json and stay black.
- (NSDictionary *_Nullable) blinkeredHomeSessionInfo;
