          ...
          @{
              @"prohibitedProcesses" :
                  @[
                      ...
                      // NOTE: The upstream SEB default prohibits Firefox's "plugin-container"
                      // (org.mozilla.plugincontainer) to stop in-browser screen sharing during exams.
                      // On modern Firefox that bundle hosts EVERY child process — content tabs, GPU,
                      // extensions, thumbnails — so force-killing it SIGKILLs the whole browser: the
                      // kid's Firefox tab dies ("Gah, your tab just crashed", no crash report) on every
                      // lock. Blinkered is a home browser where Firefox is the kid's everyday browser and
                      // kiosk mode already blocks switching to it, so this rule only destroys their
                      // session for no security gain. Entry intentionally removed. See memory:
                      // reference_blinkered_firefox_plugincontainer.
                      @{
