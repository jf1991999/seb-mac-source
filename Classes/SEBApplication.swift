...
class SEBApplication: NSApplication {
    override init() {
         ...
         */
        // libSEBSupport.a normally sets _mainTLS in the main thread dictionary
        // before init runs; since we replaced that library with an empty placeholder,
        // we set it here so setSEBDefaults doesn't call exit(0).
        Thread.main.threadDictionary["_mainTLS"] = true
