...
// Blinkered: rebuild the filter's allow/block lists at RUNTIME from a passed rules array (same dict
// shape as the org_safeexambrowser_SEB_URLFilterRules config: {action, active, expression, regex}),
// WITHOUT touching NSUserDefaults / the encrypted config / the Browser Exam Key. Lets the server push
// an updated allow-list mid-session (a parent/teacher adding a site) and have it apply on the next
// navigation with no re-lock. testURLAllowed: reads permittedList/prohibitedList fresh each call, so
// no cache to invalidate for the plain URL filter (content filter is off in the home config).
- (NSError *) updateFilterRulesWithArray:(NSArray *)URLFilterRules enable:(BOOL)enable startURL:(NSURL *)startURL
{
    ...
    for (URLFilterRule in URLFilterRules) {
        if ([URLFilterRule[@"active"] boolValue] == YES) {
            ...
            if (expressionString.length > 0) {
                ...
                if (error) {
                    // A single bad expression must not leave a half-built list. Bail without changing
                    // the (already-cleared) lists further; caller keeps the filter enabled — same
                    // fail-behaviour as the config path.
                    [self.prohibitedList removeAllObjects];
                    [self.permittedList removeAllObjects];
                    return error;
                }
    ...
    // Keep the start URL reachable (same guard as the config path), so a live update can never strand
    // the running session on our own shell/content page.
    if (self.enableURLFilter && startURL && [self testURLAllowed:startURL] != URLFilterActionAllow) {
        NSArray *expressions = [SEBURLFilterRegexExpression regexFilterExpressionWithString:startURL.absoluteString error:&error];
        if (error) {
            [self.prohibitedList removeAllObjects];
            [self.permittedList removeAllObjects];
            return error;
...
- (URLFilterRuleActions) testURLAllowed:(NSURL *)URLToFilter
{
    ...
    // App-internal schemes are not real external navigations — never subject them to the host-based
    // content filter. about: is inert; blob: URLs are origin-scoped runtime artifacts (blob:https://<origin>/
    // <uuid>), only creatable by JS already running on an allowed page, un-typeable, and their ONWARD
    // navigations stay filtered — so containment holds. Blocking blob: breaks WebAssembly/canvas apps
    // (e.g. GeoGebra navigates to blob: internally), shown to the child as "Access to '' is not allowed".
    // NOT data: — that can carry arbitrary top-level content (a real bypass vector); apps only use data:
    // for sub-resources, which the filter doesn't check anyway.
    if ([URLToFilter.scheme isEqualToString:@"about"] || [URLToFilter.scheme isEqualToString:@"blob"]) {
