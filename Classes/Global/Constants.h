...
// Desktop tint (the macOS menu-bar strip over the aquarium). PERSISTED, not session state: if a lock
// dies without unwinding — crash, force quit, power loss — these are what let the next launch hand the
// child's own wallpaper back instead of leaving Blinkered's colour on their Mac for good.
static NSString __unused *blinkeredDesktopTintActiveKey = @"blinkeredDesktopTintActiveKey";
static NSString __unused *blinkeredDesktopTintOriginalURLKey = @"blinkeredDesktopTintOriginalURLKey";
static NSString __unused *blinkeredDesktopTintOriginalOptionsKey = @"blinkeredDesktopTintOriginalOptionsKey";
