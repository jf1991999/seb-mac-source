...
// Central log-hygiene formatter: identical to the default file format, but the finished line is passed
// through [MyGlobals blinkeredScrub:] so URLs / home paths / emails are redacted at the single point
// every persisted log line flows through. See docs/CRASH_RESILIENCE_PLAN.md.
@interface BlinkeredScrubbingLogFormatter : DDLogFileFormatterDefault
@end
@implementation BlinkeredScrubbingLogFormatter
- (NSString *)formatLogMessage:(DDLogMessage *)logMessage {
    return [MyGlobals blinkeredScrub:[super formatLogMessage:logMessage]];
}
@end
...
+ (NSString *)blinkeredScrub:(NSString *)s
{
    if (!s) return @"";
    // Order matters: URL (can carry a token) before the path/email rules. One place, three patterns —
    // kept in lockstep with the agent's Swift scrub and the crash-report server scrub.
    // AN ARRAY, NOT A DICTIONARY. The comment above says the order matters, and until 8 Sep 2026 this
    // iterated an NSDictionary — whose key order is unspecified, so the stated order was not actually
    // in force. No realistic string was found where it leaked (the URL rule needs at least one
    // character after "://", which only fails if another substitution lands immediately after the
    // scheme), but an ordering guarantee that the code does not make is exactly the kind of comment
    // that gets cited in review. The agent's Swift twin already uses an ordered array; this matches it.
    static NSArray<NSArray<NSString *> *> *subs;
    ...
    for (NSArray<NSString *> *pair in subs) {
        NSRegularExpression *re = [NSRegularExpression regularExpressionWithPattern:pair[0] options:NSRegularExpressionCaseInsensitive error:nil];
        if (re) [re replaceMatchesInString:out options:0 range:NSMakeRange(0, out.length) withTemplate:pair[1]];
    }
    return out;
}
...
+ (DDFileLogger *)initializeFileLoggerWithDirectory:(NSString *)logPath
{
    ...
    // Scrub every persisted log line centrally: same default file format, but the final string is run
    // through blinkeredScrub so no single log call can leak a child's home path / a URL token.
    myLogger.logFormatter = [[BlinkeredScrubbingLogFormatter alloc] initWithDateFormatter:dateFormatter];
