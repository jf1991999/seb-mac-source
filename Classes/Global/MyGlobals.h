...
// Log hygiene (docs/CRASH_RESILIENCE_PLAN.md): strip URLs / home paths / emails from a string so a
// child's username or a token can never reach a persisted log line. Applied to every file-log line
// via the scrubbing formatter, and reused by any Blinkered log/telemetry site.
+ (NSString *)blinkeredScrub:(nullable NSString *)s;
