...
//            DDLogDebug(@"Couldn't obtain certificate info from executable %@", executablePath);
        ...
        // "Is this Apple's own software?" must never be answered by pinning a certificate fingerprint:
        // Apple rotates the OS signing leaf (macOS 26.6.2 moved every system binary to a new "macOS
        // Software Signing" leaf), and a stale pin makes this return NO for ALL system software. This
        // module is dormant today (deploy.sh builds only the "Safe Exam Browser" scheme, not the
        // Verificator), but it carried the SAME landmine signedSystemExecutable: shipped with until
        // 3.6.209 — the identical two hard-coded "Software Signing" CA fingerprints with an "old
        // certificate" fallback — so it is fixed identically here. See docs/FOREIGN_APP_TERMINATION_PLAN.md
        // §5/M3 and docs/OS_UPDATE_RESILIENCE_PLAN.md §4 (M-B). `anchor apple` alone is not enough —
        // Apple's Mac App Store apps are signed "Apple Mac OS Application Signing" and FAIL it — so this
        // is the same union requirement the shipped check uses. Fail closed on any error.
